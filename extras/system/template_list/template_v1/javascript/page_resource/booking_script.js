$(document).ready(function() {
	...
	*/
	...
	$('[type="submit"]').on('click', function(e) {
		...
		if(! validate_mobile($('#passenger-contact').val())) {
			...
			$('#passenger-contact').after('<p id="invalid_mobile_msg">Mobile Number Should be 10 digits and This Field is mandatory</p>');
			_status = false;
			_focus = '';
			$('#passenger-contact').focus();
		}
		...
		$('#pre-booking-form').find(':input[required]:visible').each(function(){
			...
			if($(this).is(':visible') && $(this).val().trim() == ''){
				_status = false;
				$(this).after("<p class='text-danger'>This Field is mandatory</p>");
				$(this).focus();
			}
		});
	...
	*/
	function build_confirm_modal(module_name){
		...
		if(module_name == 'bus'){
			...
			popup_html =  build_hotel_modal_content();
		}
	...
	function build_flight_modal_content(){
		...
		popup_html += '<div class="table-responsive"> ';
			popup_html += '<table class="table" id="passenger-confirm-table">';
				popup_html += '<thead>';
					popup_html += '<tr>';
						popup_html += '<th>Passenger Type</th>';
				...
				$('#pre-booking-form').find('.pasngr_input').each(function(){
					...
					popup_html += '<tr>';
						...
						popup_html += '<td>';
							popup_html += $(this).find('.pax_type_for_popup').val();
						popup_html += '</td>';
		...
		popup_html +='<div class="total_price_pd"><b>Total Price : ';
	...
	*/
	...
	function build_bus_modal_content(){
		...
		$('#pre-booking-form').find('.pasngrinput_secnrews').each(function(){
					popup_html += '<tr>';
						...
						popup_html += '<td>';
							popup_html += crazy_sex[$(this).find('.name_title:checked').val()];
						popup_html += '</td>';
		...
		popup_html +='<p><b style="font-size: 16px;">Total Price : ';
		if($('.amnterbig').is(':visible')){
			popup_html += $('#sp_t').text();		
		}else if($('.t_fare').is(':visible')){
			popup_html += $('#t_fare').text();
		}else{
	...
	/*
		builds passenger confirmation popup content for Hotel
	*/

	function build_hotel_modal_content(){

		var popup_html = '';

		popup_html += '<div class="table-responsive">';
		popup_html +=$('.hotelistrowhtl').html();
		popup_html +='<table class="table table-condensed "><tbody><tr class="rmdtls"><th colspan="2">Room Details</th></tr><tr><th>Room Type</th><td>'+$('.modal_room_type').text()+'</td></tr><tr class="">'+$('.aminitdv').html()+'</tr><tr><th>No of Guest</th><td>'+$('.modal_no_guest').text()+'</td></tr><tr class="grd_tol"><th>Total Price</th><td><span class="grand_total_amount">'+$('.grand_total_amount').text()+'</span></td></tr></tbody></table>'
			popup_html += '<table class="table" id="passenger-confirm-table">';
				popup_html += '<thead>';
					popup_html += '<tr>';
				...
				$('.pasngrinput').each(function(){
					popup_html += '<tr>';

						popup_html += '<td>';
							popup_html += $(this).find('.adltnom').html();
						popup_html += '</td>';

						popup_html += '<td>';
							popup_html += $(this).find('.guest_name').val();
						popup_html += '</td>';
