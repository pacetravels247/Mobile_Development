...
class Currency extends Master_currency {
	 ...
	 * Set Commission
	 */
	function set_commission($fd, $override=true)
	{	
		//debug($fd); exit;
		$CI = &get_instance();
		if ($override === true) {
			
			$this->commission_fees_row = $CI->private_management_model->get_commission($this->module_name, $fd);
	 ...
	 * Get Commission
	 */
	function get_commission($fd=0)
	{
		$this->set_commission($fd, true);
