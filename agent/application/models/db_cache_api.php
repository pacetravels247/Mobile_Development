	...
	function set_current_balance() {
		...
		if (isset($this->cache[$hash_key]) == false) {
			...
			if ($domain_details['status'] == FAILURE_STATUS) {
				...
				$booking_balance = $domain_details['data'][0]['balance']+$domain_details['data'][0]['credit_limit']+$domain_details['data'][0]['due_amount'];
				$this->cache[$hash_key] = array('value' => $domain_details['data'][0]['balance'],'credit_limit' => $domain_details['data'][0]['credit_limit'],'due_amount' => $domain_details['data'][0]['due_amount'], 'booking_balance' => $booking_balance);
			}
