...
Class Private_Management_Model extends Abstract_Management_Model
{
	...
	function admin_b2b_bus_commission_list($fd,$user_id, $group_id, $booking_source)
	{
		...
		if((empty($fd) || $com['commission_value'][0]['value'] == 0)){
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" AND group_fk="0" AND ISNULL(operator_name) ORDER BY type DESC';
		...
		if(!empty($fd) && $com['commission_value'][0]['value'] == 0){
			if($com['commission_value'][0]['value'] == 0){
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		group_fk IN ('.intval($group_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" AND operator_name = "'.$fd.'" ORDER BY type DESC';
			//debug($query1);die();
			$com['commission_value'] = $this->db->query($query1)->result_array();
		...
		if((empty($fd) || $com['commission_value'][0]['value'] == 0)){
			...
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		group_fk IN ('.intval($group_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" ORDER BY type DESC';
			//debug($query1);die();
			$com['commission_value'] = $this->db->query($query1)->result_array();
			/*if($_SERVER['REMOTE_ADDR'] == '182.156.244.142'){
            	debug($this->db->last_query());
            	debug($fd);
            	debug($com['commission_value']);die('111');
