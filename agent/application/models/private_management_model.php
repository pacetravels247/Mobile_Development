...
Class Private_Management_Model extends Abstract_Management_Model
{
	...
	function airline_markup($is_domestic='')
	{
		...
		if (empty($this->airline_markup) == true) {
			...
			$response['specific_markup_list'] = $this->specific_airline_markup('b2b_flight', $is_domestic);//Airline-Wise Markup
	...
	function generic_domain_markup($module_type, $is_domestic='')
	{
		...
		if($is_domestic == 1){
			...
			$query = 'SELECT ML.origin AS markup_origin, ML.type AS markup_type, ML.reference_id, ML.int_value as value, ML.int_value_type as value_type,  ML.markup_currency AS markup_currency
			FROM markup_list AS ML where ML.int_value != "" and ML.module_type = "'.$module_type.'" and
			ML.markup_level = "'.$this->markup_level.'" and ML.type="generic" and ML.domain_list_fk='.get_domain_auth_id();
		}
	...
	function specific_domain_markup($module_type)
	{
		...
		FROM domain_list AS DL JOIN markup_list AS ML where ML.value >= 0.00 and
	...
	function specific_airline_markup($module_type, $is_domestic='')
	{
		...
		if($is_domestic == 1){
			...
			FROM airline_list AS AL JOIN markup_list AS ML where ML.int_value >= 0.00 and
			ML.module_type = "'.$module_type.'" and ML.markup_level = "'.$this->markup_level.'" and AL.origin=ML.reference_id and ML.type="specific"
			and ML.domain_list_fk != 0  and ML.domain_list_fk='.get_domain_auth_id().' order by AL.name ASC';
		}
	...
	function admin_b2b_airline_commission_list($fd, $user_id, $group_id, $booking_source, $type)
	{
		...
		if(empty(trim($airline_code))){
			$airline_code = $fd['SegmentSummary'][0]['AirlineDetails']['AirlineCode'];
		}
	...
	function admin_b2b_bus_commission_list($fd,$user_id, $group_id, $booking_source)
	{
		...
		if(!empty($fd)){
			$get_trip_id = $this->custom_db->single_table_records('booking_source', 'origin', array('travel_id'=>$fd));
			if($get_trip_id['status'] == SUCCESS_STATUS){
				$bk_origin = $get_trip_id['data'][0]['origin'];
			}else{
				$bk_origin = 0;
			}
		...
		agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$bk_origin.' AND type="specific" AND is_master= "1" ORDER BY type DESC';
			/*$query = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		...
		agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" AND group_fk="0" AND operator_name = "'.$fd.'" ORDER BY type DESC';*/
		$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$bk_origin.' AND type="specific" AND is_master= "0" AND group_fk="0" ORDER BY type DESC';
			$com['commission_value'] = $this->db->query($query1)->result_array();
		}

		if((empty($fd) || $com['commission_value'][0]['value'] == 0)){
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where	agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" AND group_fk="0" AND ISNULL(operator_name) ORDER BY type DESC';
			$com['commission_value'] = $this->db->query($query1)->result_array();
		}
		...
		if($com['commission_value'][0]['value'] == 0){
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		agent_fk IN (0, '.intval($this->entity_user_id).') AND domain_list_fk = '.$domain_origin.' AND type="generic" AND group_fk="0" AND is_master= "0" ORDER BY type DESC';
			$com['commission_value'] = $this->db->query($query1)->result_array();
		}

		//debug($com['commission_value']);die();

		//for groupwise SPECIFIC start
		if(!empty($fd) && $com['commission_value'][0]['value'] == 0){
			//debug($fd);echo '--------';
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		group_fk IN ('.intval($group_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$bk_origin.' AND type="specific" AND is_master= "0" ORDER BY type DESC';
			/*if($fd == "Sai"){
				debug($query1);die();
			}*/
			$com['commission_value'] = $this->db->query($query1)->result_array();
		...
		if(($com['commission_value'][0]['value'] == 0)){
			...
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		group_fk IN ('.intval($group_id).') AND domain_list_fk = '.$domain_origin.' AND booking_source_origin = '.$booking_source_origin.' AND type="specific" AND is_master= "0" AND ISNULL(operator_name) ORDER BY type DESC';
			//debug($query1);die();
			$com['commission_value'] = $this->db->query($query1)->result_array();
		}
		...
		if($com['commission_value'][0]['value'] == 0){
			$query1 = 'select value, value_type, commission_currency, commission_currency as def_currency, value as def_value,is_master,tds,operator_name From b2b_bus_commission_details where
		group_fk IN ('.intval($group_id).') AND domain_list_fk = '.$domain_origin.' AND type="generic" AND is_master= "0" ORDER BY type DESC';
