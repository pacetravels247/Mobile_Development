...
function get_table($table_data, $total_rows)
{
	...
	$report_data .= '<table class="table table-condensed table-bordered rigid_actions">
		<tr>
			<th>Sno</th>
		...
		if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
				...
				$action .= hotel_voucher_sms($app_reference, $booking_source, $status,$parent_v['lead_pax_phone_number']);
				// $action.='<br/>';
...
function hotel_voucher_email($app_reference, $booking_source,$status,$recipient_email)
{
	...
	return '<a class="btn btn-sm btn-primary send_email_voucher" data-app-status="'.$status.'"   data-app-reference="'.$app_reference.'" data-booking-source="'.$booking_source.'"data-recipient_email="'.$recipient_email.'"><i class="far fa-envelope"></i></a>';
}
function hotel_voucher_sms($app_reference, $booking_source, $status, $recipient_phone)
{
