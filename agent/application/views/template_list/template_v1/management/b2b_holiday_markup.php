...
<div class="bodyContent">
	<div class="table_outer_wrper"><!-- PANEL WRAP START -->
		<div class="panel_custom_heading"><!-- PANEL HEAD START -->
            ...
            <div class="pull-left">
				<i class="fa fa-edit"></i> Manage Holiday Markup
            </div>	
            &nbsp;&nbsp;&nbsp;&nbsp;
            <span style="">Maximum Markup Value : 
            <strong><?echo $markup_limits[0]['value'];
		...
		<div class="panel_bdy"><!-- PANEL BODY START -->
			<fieldset><legend><i class="fa fa-hotel"></i> International Package - Markup</legend>
			<?php echo $this->session->flashdata("msg"); ?>
				<form action="<?php echo base_url();?>index.php/management/b2b_holiday_markup" class="form-horizontal" method="POST" autocomplete="off">
					<div class="hide">
						<input type="hidden" name="domain_origin" value="<?=get_domain_auth_id()?>" />
						<input type="hidden" name="form_values_origin" value="generic" />
						<input type="hidden" name="markup_origin" value="<?=@$i_generic_markup_list[0]['markup_origin']?>" />
						<input type="hidden" name="module_type" value="international_package" />
					</div>
					<?php
					$default_percentage_status = $default_plus_status = '';
					if (isset($i_generic_markup_list[0]) == false || $i_generic_markup_list[0]['value_type'] == 'percentage') {
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" id="generic_value" name="generic_value" class=" generic_value numeric" placeholder="Markup Value" required="" value="<?=@$i_generic_markup_list[0]['value']?>" />
							</div>
						</div>
					</div>
			...
			<fieldset><legend><i class="fa fa-hotel"></i> Domestic Package - Markup</legend>
			<?php echo $this->session->flashdata("msg"); ?>
				<form action="<?php echo base_url();?>index.php/management/b2b_holiday_markup" class="form-horizontal" method="POST" autocomplete="off">
					<div class="hide">
						<input type="hidden" name="domain_origin" value="<?=get_domain_auth_id()?>" />
						<input type="hidden" name="form_values_origin" value="generic" />
						<input type="hidden" name="markup_origin" value="<?=@$d_generic_markup_list[0]['markup_origin']?>" />
						<input type="hidden" name="module_type" value="domestic_package" />
					</div>
					<?php
					$default_percentage_status = $default_plus_status = '';
					if (isset($d_generic_markup_list[0]) == false || $d_generic_markup_list[0]['value_type'] == 'percentage') {
						$default_plus_status = 'checked="checked"';
						//$default_percentage_status = 'checked="checked"';
					} else {
						$default_plus_status = 'checked="checked"';
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								<label for="generic_value" class="col-sm-4 control-label">Markup Value<span class="text-danger">*</span></label>
								<input type="text" id="generic_value" name="generic_value" class=" generic_value numeric" placeholder="Markup Value" required="" value="<?=@$d_generic_markup_list[0]['value']?>" />
							</div>
						</div>
					</div>
					<div class="well well-sm">
		...
		<div class="panel-body"><!-- PANEL BODY START -->
			<fieldset><legend><i class="fa fa-hotel"></i> Holiday - Specific Domain Markup</legend>
