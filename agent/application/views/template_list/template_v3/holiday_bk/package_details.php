...
<div class="row" >
  ...
  <div class="col-sm-6 nopad">
   <!-- <a class="text-right id-back-s">< Back</a>-->
	...
	?>	
	 <a href="<?php echo base_url()?>menu/dashboard/package?default_view=VHCID1433498322" class="pull_right id-back-s"><i class="fa fa-chevron-left"></i>Back</a>
	<?php	
		}else{
			$prev_arr = explode('-',$prev_page);
			if($prev_arr[0]=="country"){
			?>
				<a href="<?php echo base_url()?>tours/holiday_country_package_list/<?=$prev_arr[1]?>" class="pull_right id-back-s"><i class="fa fa-chevron-left"></i>Back</a>
			<?php				
			}else{
			?>
				<a href="<?php echo base_url()?>tours/holiday_city_package_list/<?=$prev_arr[1]?>" class="pull_right id-back-s"><i class="fa fa-chevron-left"></i>Back</a>
			<?php
			}
			?>
	<?php
...
<script type="text/javascript">
  ...
  $( function() {
	...
	$(document).on('click','.btn-pluss',function(e){
		...
		var total_per_room = parseInt(child)+parseInt(adlt);
