...
foreach ($raw_bus_list as $__bk => $__bv) {
   	$b2b_price_details = $__bv['b2b_PriceDetails'];
   	//debug($b2b_price_details);die();
    ...
    // $bus_fare_details = $this->bus_lib->update_markup_currency($__bv, $currency_obj, 1); //Has all converted currency
   	...
   	$Adm_Markup = $__bv['Fare'] - $__bv['API_Raw_Fare'] - $b2b_price_details["_AgentMarkup"];
   	$cusm_comm_show = ($b2b_price_details['_admin_Commission_org_pct'] / 100)*$b2b_price_details['_Commission_org_pct'];
   	...
   	if($Adm_Markup > 0 ){
        $gst_details = $GLOBALS['CI']->custom_db->single_table_records('gst_master', '*', array('module' => 'bus'));
            ...
            if($gst_details['data'][0]['gst'] > 0){
                $gst_value = ($Adm_Markup/100) * $gst_details['data'][0]['gst'];
            }
        }
    }
	...
	//debug($nf_without_makrup); exit;
    $bus_net_fare = @$nf_without_makrup['Net']['Fare']; //If Zero seats available, we would not get this tag
    $bus_net_fare_commission = (@$nf_without_makrup['b2b_PriceDetails']['_Commission']-@$nf_without_makrup['b2b_PriceDetails']['_tdsCommission']); //If Zero seats available, we would not get this tag
    ...
    // $route_id = $__bv['RouteScheduleId'].'*'.$search_id.'*'.$__bv['RouteCode'].'*'.$booking_source; //old
    $route_id = $__bv['RouteScheduleId'].'*'.$search_id.'*'.$__bv['RouteCode'].'*'.$booking_source.'*'.$__bv['booking_source'];//new
    $max_fare = round($__bv['Max_Fare']+$bus_total_markup+$b2b_price_details['_GST']);
    ...
    $busses .= '
		...
		<div class="col-xs-5 nopad fiftywirdt">
			...
			<div style="display:none" class="net-fare-tag snf_hnf" title="C ' . $bus_net_fare_commission . '('.$cusm_comm_show.'%)+M ' . $b2b_price_details['_AgentMarkup'] . ' = ' . $b2b_price_details['_AgentEarning'] . '">' . $cur_Currency . ' <span>' . $b2b_price_details['_AgentBuying'] . '
			<span class="subtitle"> (C ' . $bus_net_fare_commission . '('.$cusm_comm_show.'%)+M ' . $b2b_price_details['_AgentMarkup'] . ' = ' . $b2b_price_details['_AgentEarning'].')</span>
			</div>
		</div>
		<div class="col-xs-3 nopad full_bus_prices">
			...
			<div class="col-xs-7 nopad">
				<div class="inpadbus">
					<div class="bustprice"><strong>' . $cur_Currency . '</strong>' . number_format($b2b_price_details['_CustomerBuying']) . '-'.number_format($max_fare).'</div>
					<div class="bookbuss">
						<form action="" method="GET" class="book-form">
								<input type="hidden" name="route_schedule_id" class="route-schedule-id" value="' . $route_id . '">
								<input type="hidden" name="gst_value" class="gst_value" value="' . $gst_value . '">
