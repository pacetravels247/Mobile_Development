...
if (valid_array($CUR_Layout) == true and ($sl_max_rows > 0 || $sl_max_cols > 0)) {
	...
	foreach ($sl_bus_deck['deck_config'] as $deck_k => $deck_v) {
		...
		for ($col = $deck_v['max_col']; $col >= $deck_v['min_col']; $col--) {			
			...
			// if($col == 1 || $col == 0){
			if(in_array($col, $sl_bus_deck['gang_way']) == true){
				// $class = 'spcetr';
				$class = '';
			}
			else{
			...
			for($row = $deck_v['min_row']; $row <= $deck_v['max_row']; $row++) {
				...
				$gangway_flag = 0;
				if($deck_k == 'Upper'){
					if(isset($sl_matrix['Upper'][$row][$col]['BlockType']) &&  $sl_matrix['Upper'][$row][$col]['BlockType'] == 3){
						$gangway_flag = 1;
					}
					if (isset($sl_matrix['Upper'][$row][$col]) == true && empty($sl_matrix['Upper'][$row][$col]['seat_no']) == false && $sl_matrix['Upper'][$row][$col]['decks'] == $deck_k && $sl_matrix['Upper'][$row][$col]['BlockType'] == 0) 
					{
					 ...
					 //    }
						...
						if ($sl_matrix['Upper'][$row][$col]['IsAvailable'] == '1') {
							...
							if (intval($sl_matrix['Upper'][$row][$col]['height']) == 2 && intval($sl_matrix['Upper'][$row][$col]['width']) == 1) {
								...
								$colspan = 1;
								// if($sl_matrix['Upper'][$row][$col]['ColumnSpan'] > 0){
								// 	$colspan = $sl_matrix['Upper'][$row][$col]['ColumnSpan'];	
								// }
								...
								$availed_seat_icon = $seat_type.'-A.png';
								$rowspan = 1;
								// if($sl_matrix['Upper'][$row][$col]['RowSpan'] > 0){
								// 	$rowspan = $sl_matrix['Upper'][$row][$col]['RowSpan'];	
								// }
							} else {
								//seater
								$seat_type = 'seat';
					...
					if(isset($sl_matrix['Lower'][$row][$col]['BlockType']) &&  $sl_matrix['Lower'][$row][$col]['BlockType'] == 3){
						$gangway_flag = 1;
					}					
					if (isset($sl_matrix['Lower'][$row][$col]) == true && empty($sl_matrix['Lower'][$row][$col]['seat_no']) == false && $sl_matrix['Lower'][$row][$col]['decks'] == $deck_k && $sl_matrix['Lower'][$row][$col]['BlockType'] == 0) 
					{
						...
						if ($sl_matrix['Lower'][$row][$col]['IsAvailable'] == '1') {
							...
							if (intval($sl_matrix['Lower'][$row][$col]['height']) == 2 && intval($sl_matrix['Lower'][$row][$col]['width']) == 1) {
								...
								$availed_seat_icon = $seat_type.'-A.png';
								$colspan = 1;
								// if($sl_matrix['Lower'][$row][$col]['RowSpan'] > 0){
								// 	$colspan = $sl_matrix['Lower'][$row][$col]['RowSpan'];	
								// }
								...
								$availed_seat_icon = $seat_type.'-A.png';
								$rowspan = 1;
								// if($sl_matrix['Lower'][$row][$col]['ColumnSpan'] > 0){
								// 	$rowspan = $sl_matrix['Lower'][$row][$col]['ColumnSpan'];
								// }
							} else {
								//seater
								$seat_type = 'seat';
							...
							if (intval($sl_matrix['Lower'][$row][$col]['height']) == 2 && intval($sl_matrix['Lower'][$row][$col]['width']) == 1) {
								...
								$colspan = 1;
								// if($sl_matrix['Lower'][$row][$col]['ColumnSpan'] > 0){
								// 	$colspan = $sl_matrix['Lower'][$row][$col]['ColumnSpan'];
								// }								
							} else if (intval($sl_matrix['Lower'][$row][$col]['height']) == 1 && intval($sl_matrix['Lower'][$row][$col]['width']) == 2) {
								//vertical sleeper
								$seat_type = 'sleeper_v';
								...
								$rowspan = 1;
								// if($sl_matrix['Lower'][$row][$col]['RowSpan'] > 0){
								// 	$rowspan = $sl_matrix['Lower'][$row][$col]['RowSpan'];
								// }
							} else {
								//seater
								$seat_type = 'seat';
