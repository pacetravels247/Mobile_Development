...
foreach ($flights_data as $__tirp_indicator => $__trip_flights) {
	...
	foreach ($__trip_flights as $__trip_flight_k => $__trip_flight_v) {
		...
		$outer_summary .= '<div class="f-s-d-w col-xs-8 nopad wayeght full_same">';
			...
			foreach ($__segment_v as $__stop => $__segment_flight) {
				...
				$outer_summary .= '<div class="allsegments outer-segment-'.$__segment_k.'">';
					...
					$outer_summary .= '<div class="col-xs-4 nopad quarter_wdth">
											<div class="insidesame">
												...
												<span class="dep_dt hide" data-category="'.time_filter_category($__segment_flight['OriginDetails']['DateTime']).'" data-datetime="'.(number_format((strtotime($__segment_flight['OriginDetails']['DateTime'])*1000), 0, null, '')).'"></span>
											</div>
										</div>';
					...
					$outer_summary .= '</div>';
					//Between Content -----
					$outer_summary .= '<div class="col-xs-2 nopad5">
										<div class="ritstop">
										<lable>Duration :</lable>
										<div class="termnl">'.$__segment_flight['SegmentDuration'].'</div>
										<div class="termnl1 hide">Stop : '.($__stop).'</div>';
					/*if(empty($Baggage) == false){
						$outer_summary .= '<div class="termnl1 flo_w"><em><i class="fas fa-suitcase bag_icon"></i>'.($Baggage).'</em></div>';
					}
					if(empty($AvailableSeats) == false){
						$outer_summary .= '<div class="termnl1 flo_w"><em><i class="air_seat timings icseats" ></i>'.$AvailableSeats.'</em></div>';
					}*/
					$outer_summary .= '</div>
										</div>';
					...
					<div class="col-xs-4 nopad wayfour full_same">
						...
						<div class="priceanbook">
							...
							<div class="col-xs-6 nopad waybook">
								...
								<div style="display:none" class="snf_hnf net-fare-tag" title="C '.($agent_commission-$tds_oncommission).'+M '.$agent_markup.' = '.$agent_earning.'"> '.$cur_Currency.' '.$agent_buying_fare.'
										
										<span class="subtitle"> (C '.($agent_commission-$tds_oncommission).'+M '.$agent_markup.' = '.$agent_earning.')</span>	
									</div>
		...
		$outer_summary .= '<div class="mrinfrmtn">
									<a class="detailsflt iti-btn" data-id="fdp_'.$api_code.'_'.$__root_indicator.$__trip_flight_k.'">Flight Details '.$remark_separator.'</a>
									<i>'.$cur_AirlineRemark.' | Baggage - '.$Baggage.' / Seat';
		if(!empty($cur_AirlineRemark) && !empty($oth_AirlineRemark))
		{
			if($cur_AirlineRemark == $oth_AirlineRemark)
				$oth_AirlineRemark="";
			else
				$outer_summary .= ' | ';
		}

		$outer_summary .= $oth_AirlineRemark.'</i>
							</div>';
		//Outer Summary - END
		$outer_summary .= '</div>';
