        ...
        <div class="cancellation_page">
            ...
            ?>
                ...
                <form name="passenger_ticket_form" id="passenger_ticket_form" class="passenger_ticket_form" method="post" action="<?=base_url().'index.php/flight/cancel_booking'?>"><!-- Passenger Form Starts -->
                    ...
                    <input type="hidden" name="is_void" value="<?=$void?>">
                ...
                ?>
                 ...
                 <div class="col-xs-6 col-xs-offset-3 text-center nopad">
                    <div class="contbk">
                        <div class="contcthdngs">Reason : </div>
                        <input type="radio" name="cancel_res" value="No Show,Refund expected in 30 to 45 working days" checked="checked"> No Show - Refund expected in 30 to 45 working days.</br>
                        <input type="radio" name="cancel_res" value="Already Cancelled with airline,Refund expected inn 24 hours"> Already Cancelled with airline - Refund expected inn 24 hours.</br>
                        <input type="radio" name="cancel_res" value="other">Other</br>
                        <div class="contcthdngs">Comments : </div>
                        <textarea name="agent_cmt" id="agent_cmt" rows="4" class="form-control" required></textarea>
                    </div>
                </div>
...
<div class="modal fade" tabindex="-1" role="dialog" id="cancel_error_modal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title">System found some errors</h4>
      </div>
      <div class="modal-body"></div>
      <div class="modal-footer">
...
$(document).ready(function() {
    ...
    $('#pre_cancel_button').click(function () {
		...
		var agent_cmt = $("#agent_cmt").val();
		var no_of_errors = 0;
		var errors = "";
		if(pax_count <= 0) {
			errors += "<h4>Please Select Passenger(s) to Cancel.</h4>";
			no_of_errors++;
		}
		if(agent_cmt.trim()==""){
			errors += "<h4>Comments is mandatory.</h4>";
			no_of_errors++;
		}			
		if(no_of_errors>0){
			$('#cancel_error_modal .modal-body').html(errors);
