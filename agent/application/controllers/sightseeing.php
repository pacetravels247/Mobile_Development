		...
		if ($domain_balance_status || $is_paid_by_pg) {
			...
			if ($temp_booking != false) {
				...
				if ($booking['status'] == SUCCESS_STATUS) {
					...
					$this->domain_management_model->update_transaction_details('sightseeing', $book_id, $data['fare'], $data['admin_markup'], $data['agent_markup'], $data['convinence'], $data['discount'],$data['transaction_currency'], $data['currency_conversion_rate'], $is_paid_by_pg);
