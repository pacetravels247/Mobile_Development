...
Class Custom_Db extends CI_Model
{
	...
	function insert_multiple_records($table_name, $data)
	{
	 ...
	 *@return array has status of insertion and insert id
	 */
	function update_record ($table_name='', $data=array(), $condition=array())
	{
		$status = '';
	...
	function generate_static_response($data)
	{
		...
		$insert_id = $this->custom_db->insert_record('test', $insert_data);
		return $insert_id['insert_id'];
	}
	function get_state_list(){
		$query = 'SELECT origin,en_name FROM  state_list ';
		$resutl = $this->db->query($query)->result_array();
		$res = array();
		foreach ($resutl as $key => $value) {
			$res[$value['origin']] = $value['en_name'];
		}
		return $res;
	}
	/**
