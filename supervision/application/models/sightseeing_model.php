...
function b2b_sightseeing_report($condition=array(), $count=false, $offset=0, $limit=100000000000)
	{
		...
		if ($count) {
			...
			$booking_customer_details	= array();
			$cancellation_details = array();
			$bd_query = 'select BD.* ,U.agency_name, U.uuid AS agency_id, U.first_name,U.last_name from sightseeing_booking_details AS BD
					     left join user U on BD.created_by_id =U.user_id 					     
	...
	public function update_cancellation_refund_details($AppReference, $cancellation_details)
	{
		...
		if($cancel_details_exists['status'] == true) {
			...
			$sightseeing_cancellation_details['cancellation_processed_on'] = date('Y-m-d');
            $sightseeing_cancellation_details['refund_amount'] = '0.00';
            $sightseeing_cancellation_details['cancellation_charge'] = '0.00';
            $sightseeing_cancellation_details['refund_status'] = 'INPROGRESS';
            $sightseeing_cancellation_details['refund_comments'] = '';
            $sightseeing_cancellation_details['refund_date'] = date('Y-m-d H:i:s');
            $sightseeing_cancellation_details['attributes'] = '';
