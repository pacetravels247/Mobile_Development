		...
		<?php 
			if(isset($value['booking_customer_details'])){
			 foreach($value['booking_customer_details'] as $cus_k => $cus_v){
			 	$action = '';
			 	$action .= get_cancellation_details_button($app_reference, $booking_source, $cus_v['status'], $cus_v['origin'], $_GET["status"]);
...
<?php 
function get_cancellation_details_button($app_reference, $booking_source, $passenger_status, $passenger_origin, $booking_status)
{
	if($passenger_status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'flight/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&passenger_status='.$passenger_status.'&passenger_origin='.$passenger_origin.'&booking_status='.$booking_status.'" class="btn btn-sm btn-info "><i class="fa fa-info"></i> Cancellation Details</a>';
	}
}
?>
