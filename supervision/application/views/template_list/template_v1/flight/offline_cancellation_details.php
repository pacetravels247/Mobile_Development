					...
					<tr>
						<td style="padding: 10px; width: 100%;">
                            ...
                            <?= generate_options($supliers_list, (array) @$booking_details['booking_source']) ?>                   
												...
												<td><input type="text" name="pnr" value="<?=@$booking_transaction_details[0]['pnr']?>"></td>
												<td><?=app_friendly_absolute_date(@$booking_details['booked_date'])?></td>
												<td>
													
													<select class="form-control" name="status" required>
														<?= generate_options(get_enum_list('booking_status_options'), array('BOOKING_CANCELLED')) ?>
													</select>
									...
									<td colspan="5" style="border: 1px solid #cccccc;">
									 ...
									 <?php
										...
										<?php if (strtolower($cus_v['passenger_type']) == 'infant') { ?>
		                                   <td><input type="checkbox" name="check_pax[<?=$cus_v['origin'];?>]" value="<?=$cus_v['origin'];?>" class="check_pax">&nbsp;<?php echo $cus_v['first_name'].'  '.($cus_v['middle_name']!==''? $cus_v['middle_name'].' ':'').$cus_v['last_name'];?>(Infant)</td>
		                                 <?php }else{?>
		                                 <td><input type="checkbox" name="check_pax[<?=$cus_v['origin'];?>]" value="<?=$cus_v['origin'];?>" class="check_pax"><?php echo $cus_v['title'].'.'.$cus_v['first_name'].'  '.($cus_v['middle_name']!==''? $cus_v['middle_name'].' ':'').$cus_v['last_name'];?></td>
		                                 <?php } ?>
										<!--  <td><input type="text" name="ticket_number[<?=$cus_v['origin'];?>]" value="<?=@$cus_v['TicketNumber'];?>" ></td> -->
							...
							<tr>
								...
								<div id="cancellation_details" >
								<tr>
									<td
										style="padding: 10px; border: 1px solid #cccccc; font-size: 14px; font-weight: bold;">Cancellatio Details</td>
								...
								<tr>
									<td style="border: 1px solid #cccccc;">
										<table width="100%" cellpadding="5"
											...
											<tr style="font-size: 15px;">
												<td>
													<label>API RefundedAmount : </label>
													<input type="number" name="api_r_amount">
												</td>
											</tr>
											<tr style="font-size: 15px;">
												<td>
											...
											<tr style="font-size: 15px;">
												<td>
													...
													<input type="number" name="cancellation_charge">
												</td>
											</tr>
											...
											<tr style="font-size: 15px;">
												<td>
													...
													<select name="refund_status" required=""><option value="">Please select</option><option value="INPROGRESS" >INPROGRESS</option><option value="PROCESSED" selected="selected">PROCESSED</option><option value="REJECTED">REJECTED</option></select>
												</td>
											</tr>
										</table>
									</td>
	...
	if (isset ( $booking_transaction_details_value ['booking_customer_details'] )) {
		foreach ( $booking_transaction_details_value ['booking_customer_details'] as $cus_k => $cus_v ) {
			...
			?>
										<tr>
										<?php if (strtolower($cus_v['passenger_type']) == 'infant') { ?>
		                                   <td><?php echo $cus_v['first_name'].'  '.($cus_v['middle_name']!==''? $cus_v['middle_name'].' ':'').$cus_v['last_name'];?>(Infant)</td>
		                                 <?php }else{?>
		                                 <td><?php echo  $cus_v['title'].'.'.$cus_v['first_name'].'  '.($cus_v['middle_name']!==''? $cus_v['middle_name'].' ':'').$cus_v['last_name'];?></td>
		                                 <?php } ?>
										 <td><?=@$cus_v['TicketNumber'];?></td>
										 <?php $cus_v['status'] = 'BOOKING_CANCELLED' ?>
												<td><strong
