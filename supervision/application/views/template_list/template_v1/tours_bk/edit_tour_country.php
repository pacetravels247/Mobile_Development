<?php error_reporting(E_ALL); // debug($tour_country_details); exit; ?>
<div id="Package" class="bodyContent col-md-12">
    <div class="panel panel-default">
            ...
            <div class="panel-title">
                <ul class="nav nav-tabs nav-justified" role="tablist" id="myTab">
                    ...
                    <li role="presentation" class="active" id="add_package_li"><a
                            ...
                            data-toggle="tab">Edit Package Country </a></li>          
                </ul>
            </div>
        </div>
        <!-- PANEL HEAD START -->
        <div class="panel-body">
            <!-- PANEL BODY START -->
            <form
                action="<?php echo base_url(); ?>index.php/tours/edit_tour_country_save"
                    ...
                    <div role="tabpanel" class="tab-pane active" id="add_package">
                        <div class="col-md-12">
                            ...
                            <div class='form-group'>
                                <label class='control-label col-sm-3' for='validation_current'>Package Country
                                </label>
                                <div class='col-sm-8 controls'>
                                    <input type="text" name="tour_country" id="tour_country"
                                           placeholder="Enter Country" data-rule-required='true'
                                           class='form-control add_pckg_elements' required value="<?php echo string_replace_encode($tour_country_details['name']);?>">
                                </div>
                            </div>
                            <div class='form-group ini_banner_img'>
								...
								<div class='col-sm-8 controls'>
									<?php
									if($tour_country_details['banner_image']){
									echo '<img class="banner_imgs" src="'.$this->template->domain_images( $tour_country_details['banner_image']) . '" style="width:200px;height:130px;">';
									}
									?>										
								</div>
                            </div>
                            ...
                            <div class='' style='margin-bottom: 0'>
                                <div class='row'>
                                    <div class='col-sm-9 col-sm-offset-3'>  
                                        <input type="hidden" name="id" value="<?= $tour_country_details['id'] ?>">                       
                                        <button class='btn btn-primary sub_form' type='submit'>Update</button>
                                        <a href="<?php echo base_url(); ?>index.php/tours/tour_country" class='btn btn-primary' style="color:white;">Cancel</a>
...
<script type="text/javascript">  
		...
		$("#banner_image").change(function() {
		  readURL(this);
			...
			$(document).on('keyup','#tour_country',function(){
				var name=$('#tour_country').val();
				$.post('<?=base_url();?>tours/check_unique_data',{'name':name,'table':'tours_country'},function(data)
				{
					//	alert(data);
						if(data>1){
							$('.is_unique').val('0');
