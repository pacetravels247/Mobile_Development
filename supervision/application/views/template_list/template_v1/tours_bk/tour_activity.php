...
<div id="Package" class="bodyContent col-md-12">
	<div class="panel panel-default">
			...
			<div class="panel-title">
				<ul class="nav nav-tabs nav-justified" role="tablist" id="myTab">
					...
					<li role="presentation" class="active" id="add_package_li"><a
						...
						data-toggle="tab">Package Activity </a></li>
		...
		<div class="panel-body">
			<!-- PANEL BODY START -->
			<form
				action="<?php echo base_url(); ?>index.php/tours/tour_activity_save"
						   ...
						   <div class="col-md-12">
							...
							<div class='form-group'>
								...
								<div class='col-sm-8 controls'>
									<input type="text" name="tour_activity" id="tour_activity"
										placeholder="Enter Package Activity" data-rule-required='true'
	...
	<!-- PANEL WRAP END -->
        ...
        foreach ($tour_activity as $key => $data) {
        if($data['status']==1)
        {
        ...
        echo '<tr>
              <td>'.$sn.'</td>
              <td>'.$data['tour_activity'].'</td>';
        //echo '<td>'.$galleryImages.'</td>';
        echo '<td>'.$status.'</td>';
        if($data['status']==1)
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/activation_tour_activity/'.$data['id'].'/0"
        ...
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/activation_tour_activity/'.$data['id'].'/1"
              data-original-title="Activate Tour Destination"> <i class="glyphicon glyphicon-th-large"></i></i> Activate
              </a>
              </td>';
        }      
        echo '<td class="center">
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/edit_tour_activity/'.$data['id'].'"
		...
		$(document).ready(function()
		{
          ...
          $(".callDelete").click(function() { 
            $id = $(this).attr('id'); //alert($id);
		        ...
		        if($response==true){ window.location='<?=base_url()?>index.php/tours/delete_tour_activity/'+$id; } else{}
           });
		});
        </script>
<?php
