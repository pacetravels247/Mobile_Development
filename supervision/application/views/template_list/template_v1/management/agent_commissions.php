   ...
   <div class="panel panel-primary">
      <div class="panel-heading">
         ...
         <div class="panel-title">
            <i class="fa fa-edit"></i> Agent Commission
         </div>
      </div>
      <div class="panel-body">
         ...
         <div class="tab-content">
            <div class="org_row">
               <div class="col-md-6">
                  <label for="new_airline_value" class="col-sm-3 control-label">Select Agent<span class="text-danger">*</span></label>                                
                  <div class="col-md-9">
                     <div class="form-group">
                        <select name="group" id="group" required="" class="group form-control mb10 form_group">
                           <option value=""> -- Select Agent -- </option>
                           <?php foreach($agent as $group_key => $group_val){ ?>
                           <option value="<?php echo $group_val['user_id']; ?>"><?php echo $group_val['agency_name'].' - '. provab_decrypt($group_val['uuid']); ?></option>
                           <?php }?>
                        </select>
                     </div>
                  </div>
            ...
            <div class="col-md-6">
               ...
               <div class="col-md-10 mtb5">  
                  ...
                  <input type="radio" <?php if($active_value['course_id'] == 'VHCID1420613784'){ echo 'checked=checked'; } ?> class="module" name="module" value="<?php echo $active_value['course_id']; ?>"> <?php echo $active_value['name']; ?> </label>
                  <?php } } ?>
               </div>
            </div>
         </div>
         <div class="col-xs-12 nopad" id="display_supplier_list">
         </div>
      </div>
   </div>
</div>
<script type="text/javascript">
   $(document).ready(function(){
   	...
   	/*$.post( base_url+'index.php/management/get_suppliers_for_agent', { module: $("input[name='module']:checked").val(), group_fk:$('#group').val() })
   		  .done(function( data ) {
   	...
   	$(document).on('change','.trip_type, #tp_supplier, .module, .group', function(){
         ...
         $('.selected_supplier_specific').val(supplier);
   		$.post( base_url+'index.php/management/get_suppliers_for_agent', { module: $("input[name='module']:checked").val(), user_oid:$('#group').val() , trip_type : tirp_type, tp_supplier : supplier })
   		  .done(function( data ) {
   			$('#display_supplier_list').html(data);		    
   		  });
   	});
