...
//debug($export_data[$k]['booked_date']);exit;
	...
	{	
		...
		$date_obj->modify('-90 day');
		$date = $date_obj->format('Y-m-d');
		
		$common_report_data = $this->domain_management_model->get_common_booking_details($date);
		...
		// $this->load->library('pagination');
		// if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
		// $config['base_url'] = base_url().'index.php/report/b2b_common_report/';
		// $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		// $page_data['total_rows'] = $config['total_rows'] = $total_records;
		// $config['per_page'] = RECORDS_RANGE_2;
		// $this->pagination->initialize($config);
