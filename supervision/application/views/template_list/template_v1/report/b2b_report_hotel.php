            ...
            <div class="clearfix"></div>
                <div class="dropdown col-xs-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="excel_imp_drop">
                        <li >
                            <a href="<?php echo base_url(); ?>index.php/report/export_confirmed_booking_hotel_report_b2b/excel/confirmed_cancelled<?= !empty($_SERVER["QUERY_STRING"])?'?'.$_SERVER["QUERY_STRING"]:''?>">Confirmed & Cancelled Booking</a>
                        </li>
                        <li >
...
function get_table($table_data, $total_rows)
{
	...
	$report_data .= '<table class="table table-condensed table-bordered rigid_actions" id="b2b_report_hotel_table">
		<thead>
			<tr>
				<th>Sno</th>
				<th>Agency_name</th>
		    ...
		    foreach($booking_details as $parent_k => $parent_v) { 
		       //debug($parent_v);exit;
		    	...
		    	if($status == 'BOOKING_CONFIRMED') {
					...
					$action .='<a href="'.base_url().'index.php/hotel/offline_hotel_cancellation/'.$app_reference.'/'.$booking_source.'/'.$status.'" class="btn btn-sm btn-warning" title="Offline Cancellation" target="_blank"><i class="fa fa-ban" aria-hidden="true"></i></a>';
				}
		...
		$report_data .= '<tr>
					...
					<td>'.$agency_name.' - '.provab_decrypt($agency_id).'</td>
					<td>'.$supp_name.'</td>
					<td>'.$app_reference.'</td>
