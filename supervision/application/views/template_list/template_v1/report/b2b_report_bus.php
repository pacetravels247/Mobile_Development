						...
						<?php
							if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
								...
								foreach($booking_details as $parent_k => $parent_v) {
									...
									$do_det = $GLOBALS["CI"]->custom_db->single_table_records("booking_source", "*", array("travel_id" => $travel_id));
									if($travel_id != 0 && $do_det["status"] == 1)
									{
										$direct_operator = $do_det["data"][0]["name"];
									}
									else
										$direct_operator = "NA";
									...
									if($status == 'BOOKING_CONFIRMED'){
										$action .='<a href="'.base_url().'index.php/bus/offline_bus_cancellation/'.$app_reference.'/'.$booking_source.'/'.$status.'" class="btn btn-sm btn-warning" title="Offline Cancellation" target="_blank"><i class="fa fa-ban" aria-hidden="true"></i></a>';
									}
									...
									$action .= get_cancellation_details_button($parent_v['app_reference'], $parent_v['booking_source'], $parent_v['status'], $parent_v["booking_customer_details"]);
									$profit = ($admin_markup+$admin_commission)-$agent_commission;
									$total_profit += $profit;
								?>
									<tr>
										<td><?php echo ($current_record++)?></td>
										<td><?php echo $agency_name." - ".provab_decrypt($agency_id); ?></td>
										...
										<td><?php echo @$direct_operator?></td>
										<td><?php echo @$pnr?></td>
								...
								?>
									...
									<td colspan="4"><?php echo $total_profit; ?></td>
							<?php
							} else {
...
function bus_voucher_email($app_reference, $booking_source,$status,$recipient_email)
{
	...
	return '<a class="btn btn-sm btn-primary send_email_voucher" data-app-status="'.$status.'"   data-app-reference="'.$app_reference.'" data-booking-source="'.$booking_source.'"data-recipient_email="'.$recipient_email.'" title="Email Ticket"><i class="far fa-envelope"></i></a>';
}
function get_cancellation_details_button($app_reference, $booking_source, $status, $bcds)
{
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'bus/ticket_cancellation_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$status.'" class="col-md-12 btn btn-sm btn-primary "
			title="Cancellation Details"><i class="far fa-info"></i></a>';
	}
	foreach($bcds AS $bcd)
	{
