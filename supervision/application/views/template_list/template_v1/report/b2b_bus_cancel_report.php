	...
	<div class="panel panel-default clearfix"><!-- PANEL WRAP START -->
			...
			<div class="panel-body">
				...
				<div class="clearfix">
					<div class="col-xs-4">
						...
						<select class="form-control" name="status" onchange="check_supplier(this.value);">
							<option>All</option>
							<?//=generate_options($supplier_option, array(@$status))
							if(isset($_GET['source_id'])){
								$sr_id = $_GET['source_id'];
							}
							
							foreach ($supplier_option['data'] as $key => $value) {
								
							?>
							<option value="<?=$value['source_id']?>" <?php if($sr_id==$value['source_id']) echo "selected" ?>><?=$value['name']?></option>
							<?php } ?>
						</select>
					</div>
				</div>
			...
			<!-- <div id="show-search" class="collapse">
				<form method="GET" autocomplete="off">
					...
					<a href="<?php echo base_url().'index.php/report/b2b_bus_cancel_report?' ?>" id="clear-filter" class="btn btn-primary">Clear Filter</a>
					</div>
				</form>
			</div> -->
						...
						<?php
							if (isset($table_data) == true and valid_array($table_data['booking_details']) == true) {
								...
								foreach($booking_details as $parent_k => $parent_v) {
									...
									$profit = $admin_markup-$agent_commission;
									$total_profit += $profit;
								?>
									<tr>
...
function get_cancellation_details_button($app_reference, $booking_source, $status)
{
	if($status == 'BOOKING_CANCELLED'){
		return '<a target="_blank" href="'.base_url().'bus/cancellation_refund_details?app_reference='.$app_reference.'&booking_source='.$booking_source.'&status='.$status.'" class="col-md-12 btn btn-sm btn-primary "
...
<script type="text/javascript">
	function check_supplier(source_id){
		var url = app_base_url + 'index.php/report/b2b_bus_cancel_report?source_id='+source_id;
		window.location = url;
	}
</script>