...
<div id="Package" class="bodyContent col-md-12">
	<div class="panel panel-default">
			...
			<div class="panel-title">
				<ul class="nav nav-tabs nav-justified" role="tablist" id="myTab">
					...
					<li role="presentation" class="active" id="add_package_li"><a
						...
						data-toggle="tab">Tour Inclusions </a></li>
		...
		<div class="panel-body">
			<!-- PANEL BODY START -->
			<form
				action="<?php echo base_url(); ?>index.php/tours/tour_inclusions_save"
					...
					<div role="tabpanel" class="tab-pane active" id="add_package">
						   <div class="col-md-12">													
							<div class='form-group'>
								<label class='control-label col-sm-3' for='validation_current'>Tour Inclusion
								</label>
								<div class='col-sm-8 controls'>
									<input type="text" name="inclusion" id="inclusion"
										placeholder="Enter Tour Inclusion" data-rule-required='true'
										class='form-control' required>									
								</div>
							</div>						
							<div class='form-group'>
								<label class='control-label col-sm-3' for='validation_current'>Tour Inclusion Icon
								</label>
								<div class='col-sm-4 controls'>
									<input type="file" name="inclusion_image" id="inclusion_image" class='form-control' data-rule-required='true' required>									
								</div>
							</div>																
	...
	<!-- PANEL WRAP END -->
        ...
        foreach ($tour_inclusions as $key => $data) {
        if($data['status']==1)
        {
        ...
        echo '<tr>
              ...
              <td><img src="/airliners/extras/custom/keWD7SNXhVwQmNRymfGN/images/'.$data['inclusion_image'].'" style="width:100%"></td>';
        //echo '<td>'.$galleryImages.'</td>';
        echo '<td>'.$status.'</td>';
        if($data['status']==1)
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/activation_tour_inclusion/'.$data['id'].'/0"
              data-original-title="Deactivate Tour Inclusion"> <i class="glyphicon glyphicon-th-large"></i></i> De-activate
              </a>
              </td>';
        }
        else
        {
        echo '<td>
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/activation_tour_inclusion/'.$data['id'].'/1"
              data-original-title="Activate Tour Inclusion"> <i class="glyphicon glyphicon-th-large"></i></i> Activate
              </a>
              </td>';
        }      
        echo '<td class="center">
              <a class="" data-placement="top" href="'.base_url().'index.php/tours/edit_tour_inclusion/'.$data['id'].'"
              data-original-title="Edit Tour Inclusion"> <i class="glyphicon glyphicon-pencil"></i> Edit
              </a> 
		...
		{          
          $(".callDelete").click(function() { 
            $id = $(this).attr('id'); //alert($id);
		        ...
		        if($response==true){ window.location='<?=base_url()?>index.php/tours/delete_tour_inclusion/'+$id; } else{}
           });
		});
        </script>
<?php
