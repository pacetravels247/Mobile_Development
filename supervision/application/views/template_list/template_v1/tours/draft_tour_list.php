    ...
    <table class="table table-bordered">
      <thead>
          ...
          <th>Top Deals</th>-->
            ...
            <tbody>
              ...
              foreach ($tour_list as $key => $data) {
				...
				if($data['package_status']=='CREATED')
                {
					$itinerary_link = 'Add itinerary';
				}
				else
				{
					$itinerary_link = 'Edit Itinerary';
				}
			 ...
			 // <td>'.$dep_dates_list.'</td>
              // <td>'.$top_deals_list.'</td>
               ...
               if($data['status']==4)
               {
                ...
                <a class="" data-placement="top" href="'.base_url().'index.php/tours/price_management/'.$data['id'].'/draft"
            ...
            <a class="" data-placement="top" href="'.base_url().'index.php/tours/tour_itinerary_p2/'.$data['id'].'"
              data-original-title="Edit Tour Itinerary"> <i class="glyphicon glyphicon-pencil" ></i> Add Itinerary
            </a> &nbsp; <br>
        ...
        <a class="callDelete" id="'.$data['id'].'"> 
          ...
          &nbsp; <br>
  <a class="btn btn-primary send_verify" data-tour_id="'.$data['id'].'" id="'.$data['id'].'"> 
  <i class="glyphicon glyphicon-trash"></i> Send for Verification</a>
...
<a class="" data-placement="top" href="'.base_url().'index.php/tours/edit_tour_package/'.$data['id'].'"
			...
			<a class="" data-placement="top" href="'.base_url().'index.php/tours/price_management/'.$data['id'].'/B2B/draft"
                data-original-title="Price Management"><i class="fa fa-usd" aria-hidden="true"></i>B2B Price Management
              </a> &nbsp; <br>
			<a class="" data-placement="top" href="'.base_url().'index.php/tours/price_management/'.$data['id'].'/B2C/draft"
                data-original-title="Price Management"><i class="fa fa-usd" aria-hidden="true"></i>B2C Price Management
              </a> &nbsp; <br>
   <a class="" data-placement="top" href="'.base_url().'index.php/tours/tour_itinerary_p2/'.$data['id'].'"
              data-original-title="Edit Tour Itinerary"> <i class="glyphicon glyphicon-pencil" ></i> '.$itinerary_link.'
            </a> &nbsp; <br>
			<a class="" data-placement="top" href="'.base_url().'index.php/tours/tour_dep_dates/'.$data['id'].'/draft_list"
            data-original-title="Edit Tour Destination"> <i class="fa fa-calendar"aria-hidden="true"></i> Dep Dates
          </a> &nbsp; <br>
    ...
    $('.book_tourid').click(function() {
      $('#tour_id').val($(this).data('tourid'));
        ...
        $(".callDelete").click(function() { 
            ...
            if($response==true){ window.location='<?=base_url()?>index.php/tours/delete_tour_package/'+$id+'/draft_list'; } else{}
          });
		...
		//	alert(tour_id);
			$.ajax({
				url: '<?php echo base_url();?>index.php/tours/ajax_tour_send_verification/',
				method: 'post',
				...
				success:function(data){
					...
					console.log(obj);
					var html = '';
					if ( typeof obj['first'][0] !== "undefined") {
						html += obj['first'][0]+'\n';
					}
					if ( typeof obj['first'][1] !== "undefined") {
						html += obj['first'][1]+'\n';
					}
				...
				var res_multiple_date=$(this).find('.res_multiple_date').val();
					...
					if(res_from_to_date !=='null'){
						...
						$.each(res_from_to_date, function(index, item) {
							//var d1=new Date(item['valid_from']);
						//	var d2=new Date(item['valid_to']);
							//var d3=new Date(tour_to_date);
							//if(item['valid_from']>=tour_to_date && item['valid_to']<=tour_to_date){
