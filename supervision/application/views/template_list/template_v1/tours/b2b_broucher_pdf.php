...
p {margin: 0px;}
.sm_tab.table>tbody>tr>td, .sm_tab.table>thead>tr>th {padding: 0px;font-size: 13px;}
@media print
   {
...
<?php } ?>
  ...
  <table cellspacing="0" cellpadding="0" border="0" style="border: 2px solid #eeeeee; background: #ffffff; font-size: 12px; line-height: 20px; width: 100%; max-width: 600px; margin: 0px auto; font-family: cambria, serif; color:#222;text-align: justify;word-spacing: 5px;">
  <tbody>
  <tr>
  <td>
  <table cellspacing="0" cellpadding="0" border="0" style="border-collapse: collapse; width: 100%; margin: 0px auto; font-family: cambria, serif; color:#222;text-align: justify;word-spacing: 5px;">
    <tbody>
      <tr>
        <td style="border-bottom:2px solid #eee;padding:30px 0px 0px 10px;height: 50px;line-height: 20px;">
          <span>&nbsp;</span><br>
        <img height="40px" src="<?php echo SYSTEM_IMAGE_FULL_PATH_TO_EXTRA.$GLOBALS['CI']->template->domain_images($GLOBALS['CI']->application_domain_logo); ?>" alt="logo">
        </td>
      </tr>    
      <tr>
        <td style="margin-left: auto;margin-right: auto;text-align: center;">
          <span style="margin:0;font-size: 16px;font-weight:600"><?= $tour_data['package_name']; ?></span><br>
          <span style="margin:0 0 2px;font-size: 13px;"><?=($tour_data['duration']+1);?> Days / <?=$tour_data['duration'].(($tour_data['duration']==1)? 'Night': 'Nights');?> </span>
        </td>
      </tr>
      <tr>
        <td style="margin-left: auto;margin-right: auto;text-align: center;">
      ...
      <tr>
        <td>
          <span style="font-size: 12px;">Quote Reference : <?=$quotation_details['quote_reference']?></span><br>
          <span style="font-size: 12px;">Fare Breakdown (<?=$quotation_details['currency_code']?>)</span><br>
          <?php 
          ...
          ?>
          <span style="font-size: 12px;font-weight:bold">Total : <?=$quotation_details['currency_code']?> <?=$quotation_details['quoted_price']?></span>          
        </td>
      </tr>
      <?php 
      ...
      ?>
	   <tr>
        <td>
          ...
          <span style="margin: 0px auto;text-align:center;font-size: 12px;font-weight:600;">
           <span><?php 
            ...
            ?></span> <br>
            <span>Tour Type – <?=$tour_type?> || Tour Code - <?=$tour_data['tour_code']?></span><br>
            <span>Departure Dates</span><br>
            <span><?php 
			if($tour_data['package_type']=="fit"){
    ...
    <!--  <tr>
	   ...
	   <tr>
        <td>
            ...
            <span style="text-align: left;font-size: 16px;font-weight: 600;border-bottom: 1px solid #ccc;">Detailed Itinerary</span>
	 ...
	 // debug($tours_itinerary_dw);
      foreach ($tours_itinerary_dw as $key => $itinary) {
        ...
        <tr>
          <td>
            <span><b>Day <?php echo $key+1; ?> - <?php echo  $itinary['visited_city_name']; ?></b> </span>
            <span>
      ...
      <tr><td style="margin-right:auto;margin-left:auto;text-align:center;">

              
			  <?php $imag_array= explode(',',$itinary['banner_image']);
			  foreach($imag_array as $im){
				  if($im!=''){
				echo '<img style="display: inline-block;width:100px;height:100px;border:3px solid #fff;" src="'.SYSTEM_IMAGE_FULL_PATH_TO_EXTRA.$this->template->domain_images($im).'">';
				  }
			  }
		...
		<tr>
			<td style="padding:0px 10px 10px 0">
			  <p style="margin:0;">
				<h1 style="text-align:left;font-size: 16.5px;line-height:20px;font-weight:600;">Detailed Holiday Price: B2B Pricing</h1>
			  </p>
			  <ul>
			  ...
			  <table class="tabel table-bordered" cellpadding="5" border="1" style="margin-right: auto;margin-left: auto;">
				<tr>
					<th style="padding: 5px;text-align: center;">Room Type</th>
					<th style="padding: 5px;text-align: center;">Per Person</th>
				</tr>
				...
				?>
								<tr>
					<td style="padding: 5px;text-align: center;"><?=$fetch_x['occupancy_name']?></td>
					<td style="padding: 5px;text-align: center;"><?=$tour_price_fly['market_price']?></td>
					</tr>
				<?php } ?>
	...
	<!-- 	<tr>
			<td style="padding:0px 10px 10px 0">
				<h2 style="margin:5px 0;font-size: 16.5px; line-height:20px; font-weight:600">B2B TOUR COST</h2>
				<?php
				$arr_occ = array();
...
//debug($b2b_tour_price);					  
		...
		<tr>
			<td style="padding:0px 10px 10px 0">
				<h2 style="margin:5px 0;font-size: 16.5px; line-height:20px; font-weight:600">B2C TOUR COST</h2>
				<?php
				$arr_occ = array();
...
//debug($b2c_tour_price);					  
      ...
      <tr>
		...
		<tr>
          <td style="border-bottom: 2px solid #eeeeee;">
            <span style="font-size: 14px;font-weight:600">ACCOMMODATION</span>
            <ul>
				<?php 
    ...
    <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
				 <span style="font-size: 14px;font-weight:600">PACKAGE PRICE INCLUDES:</span>
            <span>
            	 <?php 
        ...
        <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
             <span style="font-size: 14px;font-weight:600">PACKAGE PRICE DOES NOT INCLUDES:
</span>
<span>
	<?php 
...
<tr>
          <td style="border-bottom: 2px solid #eeeeee;">
             <span style="font-size: 14px;font-weight:600"> GENERAL CONDITIONS & REMARKS:</span>
            <span><?php
            ...
            <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
                <span style="font-size: 14px;font-weight:600">TRIP NOTE:</span>
              <span style="margin:0;white-space: normal;font-size: 13px;">
                <?php $tours_itinerary['trip_notes'] = str_replace('\n', '', $tour_data['trip_notes']);

                echo htmlspecialchars_decode($tours_itinerary['trip_notes']); ?></span>

              </td>
            </tr>
            <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
                 <span style="font-size: 14px;font-weight:600">VISA PROCEDURES</span>
              <span style="margin:0;white-space: normal;font-size: 13px;">
                <?php $tours_itinerary['visa_procedures'] = str_replace('\n', '', $tour_data['visa_procedures']);

                echo htmlspecialchars_decode($tours_itinerary['visa_procedures']); ?></span>
              </td>
            </tr>
            <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
                <span style="font-size: 14px;font-weight:600">PAYMENT POLICY:</span>
              <span style="margin:0;white-space: normal;font-size: 13px;">
                <?php $tours_itinerary['canc_policy'] = str_replace('\n', '', $tour_data['canc_policy']);

                echo htmlspecialchars_decode($tour_data['b2b_payment_policy']); ?></span>
              </td>
            </tr>
            <tr>
          <td style="border-bottom: 2px solid #eeeeee;">
                <span style="font-size: 14px;font-weight:600">CANCELLATION POLICY:</span><br>
              <span style="margin:0;white-space: normal;font-size: 13px;">
                <?php $tours_itinerary['canc_policy'] = str_replace('\n', '', $tour_data['canc_policy']);

                echo htmlspecialchars_decode($tours_itinerary['canc_policy']); ?></span>
          ...
          <tr>
            <td style="padding:10px 20px 10px 0">
              <table>
                <tbody>
                  <tr>
                    <td colspan="3" width="100%"><span style="line-height:20px;">&copy; <?=date("Y")?> Pace Travels. All rights reserved.
					</span></td> 
                  </tr>
                </tbody>
              </table>
