...
class Utilities extends CI_Controller {
	...
	function get_bus_stations(){
        ...
        foreach($memData as $member){
            $i++;
            $member['action'] = "<a href='#' class='btn btn-info edit_details' data-id='".$member['origin']."' data-c_name='".$member['ets_city_name']."' data-vrl_id='".$member['vrl_id']."' data-bitla_id='".$member['bitla_id']."'>Edit</a>&nbsp;<a href='#' class='btn btn-warning delete_details' data-id='".$member['origin']."'>Delete</a>";

            $data[] = array($i, $member['ets_city_name'], $member['vrl_id'], $member['bitla_id'],$member['action']);
        }
	...
	function agent_amount_collection($id = ''){
		...
		if (valid_array ( $page_data ['form_data'] ) == true) {
			...
			//debug($_FILES["upload_reciept"]);
			//debug($page_data ['form_data']);die();
			$cheque_issued_date = '0000-00-00';
			...
			$insert_form_data = array(
					'agent_id' =>$page_data['form_data']['agency_name'],
					'deposite_type' =>$page_data['form_data']['deposite_type'],
					'beneficiary_bank_name' =>$page_data['form_data']['beneficiary_bank_name'],
					'payer_bank_name' =>$page_data['form_data']['payer_bank_name'],
					'cheque_dd_num' =>$page_data['form_data']['cheque_dd_no'],
					'cheque_issued_date' => $cheque_issued_date,
					'payment_reciept' => $upload_files,
					'payment_date' => $payment_date,
					'collected_amount' =>$page_data['form_data']['collected_amount'],
					'collected_date' =>$page_data['form_data']['collected_date'],
					'status' =>'Pending',
					'created_by' =>$this->entity_user_id,
					'updated_by' =>$this->entity_user_id,
					'created_date' =>date('Y-m-d h:i:s'),
					'collection_ref' => 'EAC-'.$this->entity_user_id.time()
				);


			$temp_name = $_FILES["upload_reciept"]['name'];
			if($temp_name != ''){
				$config['upload_path'] = '../../'.$this->template->domain_uploads().'agent_amount_reciept/';
			...
			$this->custom_db->insert_record('agent_amount_collection', $insert_form_data);
		...
		if (isset($params['created_date_from']) == true and empty($params['created_date_from']) == false) {
			$data_list_filt[] = array('date(AAC.created_date)', '>=', $this->db->escape(db_current_datetime(date('Y-m-d',strtotime($params['created_date_from'])))));
		}
		if (isset($params['created_date_to']) == true and empty($params['created_date_to']) == false) {
			$data_list_filt[] = array('date(AAC.created_date)', '<=', $this->db->escape(db_current_datetime(date('Y-m-d',strtotime($params['created_date_to'])))));
		}

		//debug($data_list_filt);die();
		$page_data['agency_list'] = $this->domain_management_model->get_agent_list();
		$page_data['search_params'] = $params;
		$page_data ['table_data'] = $this->domain_management_model->amount_collection_list($data_list_filt);
