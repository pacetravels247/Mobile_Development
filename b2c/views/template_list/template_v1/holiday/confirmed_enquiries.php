	...
	<table class="table table-bordered">
		...
		<?php
			...
			foreach($tours_enquiry AS $key=>$te){
				echo $GLOBALS['CI']->template->isolated_view('holiday/passanger_data_form',$te);
				echo $GLOBALS['CI']->template->isolated_view('holiday/enq_upload_details',$te);
				echo $GLOBALS['CI']->template->isolated_view('holiday/enq_payment_details',$te);
				if($te['te_status']==1)
		...
		?>
			<tr> 
				...
				<td><?=$te["phone"]?></td>
				<td><?php if($te["package_details"]['package_type']=='group'){echo "Grouped / Fixed";}else{echo "FIT / Customised";}?></td>
				...
				<td><?=$te["status"]?></td>
				<td>
					<a class="btn btn_sm" data-toggle="modal" data-target="#quot_<?=$te["id"]?>" >Received Quote</a>
					<a class="btn btn_sm" data-toggle="modal" data-target="#passanger_data_form_<?=$te["enquiry_reference_no"]?>">Passanger Data</a>
					<a class="btn btn_sm" data-toggle="modal" data-target="#payment_details_<?=$te["enquiry_reference_no"]?>">Payment Details</a><br>
					<a class="btn btn_sm" data-toggle="modal" data-target="#upload_details_<?=$te["enquiry_reference_no"]?>" >Downloads</a>
				</td>
			</tr>
	  <?php } ?>
		</tbody>
