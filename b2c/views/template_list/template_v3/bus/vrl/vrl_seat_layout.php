...
if (valid_array($CUR_Layout) == true and ($sl_max_rows > 0 || $sl_max_cols > 0)) {
				 ...
				 //    }
					...
					if ($sl_matrix['Lower'][$row][$col]['IsAvailable'] == '1') {
						...
						if($gender == 'not'){
							...
							$sl_title = array('Seat1 : '.$seat_number, ' Fare : '.$sl_matrix['Lower'][$row][$col]['Fare'].' ,Service Tax : '.$sl_matrix['Lower'][$row][$col]['service_tax']);
	...
	//debug($seat_layout_stamp);die('1111');
	echo '
	<div class="row">
		<div class="col-lg-12 nopad"><div class="layoutonly">'.$seat_layout.'</div>
         ...
         <table style="width:70%;">
			...
			<td style="width:35%;" align="left" valign="top" class="seat_set setag"><strong>Service Tax :</strong><span class="selected-seat-price colordstybg" id="selected-seat-tax-price-'.$seat_layout_stamp.'"></span></td>
...
$(document).ready(function() {
	...
	function update_seat_summary_layout()
	{
		...
		$('#selected-seat-tax-price-'+<?=$seat_layout_stamp?>).empty().text(seat_tax_cost_summary.toFixed(2));
	}
