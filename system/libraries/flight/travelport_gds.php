 ...
 * @version    V1
 */
class Travelport_gds extends Common_Api_Grind {
    ...
    function credentials($service) {
        switch ($service) {
            ...
            case 'UpdateFareQuoteRoundtrip':
                $this->service_url = $this->Url . 'UpdateFareQuoteRoundtrip';
                break; 
            case 'ExtraServices':
     ...
     * @param $search_key   session id of the search  -  session identifies each search
     */
    function fare_quote_request($data_key, $search_key, $fare_type) {
        $response['status'] = SUCCESS_STATUS;
        $response['data'] = array();
        $request_params = array();
        if($fare_type == 'FareQuote'){
              $this->credentials('UpdateFareQuote');
        }
        else{
             $this->credentials('UpdateFareQuoteRoundtrip');
        }
        if (empty($data_key) == false) {
            $request_params['ResultToken'] = $search_key;
        } else {
    ...
    function get_flight_list($search_id = '',$booking_source='') {
        ...
        if ($search_data['status'] == true) {
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                //get request
                $search_request = $this->flight_search_request($search_data['data'],$booking_source);
                //get data
                if ($search_request['status']) {
                    $search_response = $this->CI->api_interface->get_json_response($search_request['data']['service_url'], $search_request['data']['request'], $header_info);
                    //debug($search_response);die('==');
                    //$this->CI->custom_db->generate_static_response(json_encode($search_response));
    ...
    function fare_quote_details($flight_booking_details) {
        ...
        if (count($unique_search_access_key) == 1) {
            ...
            if (count($flight_booking_details['search_access_key']) == 1) {
                $tmp_token = $this->run_fare_quote(array($flight_booking_details['token'][0]), $flight_booking_details['search_access_key'][0],'FareQuote');
            ...
            //     }
            // }
            $tmp_token = $this->run_fare_quote(array($flight_booking_details['token']), $flight_booking_details['search_access_key'],'FareQuoteReturn');
            $this->update_fare_quote_details($flight_booking_details, 0, $tmp_token['data'], $tmp_token['status'], $response);
        }
    ...
    function update_fare_quote_details(& $flight_booking_details, $index, $new_quote_details, $process_quote_request, & $response) {
        ...
        unset($flight_booking_details['token'][1]);
        unset($flight_booking_details['token_key'][1]);
        unset($flight_booking_details['search_access_key'][1]);
    }
     ...
     */
    private function run_fare_quote($data_key, $search_access_key, $fare_type) {
        $response['data'] = array();
        $response['status'] = FAILURE_STATUS;
        $api_request = $this->fare_quote_request($data_key, $search_access_key, $fare_type);
        //get data
        if ($api_request['status']) {
            $header_info = $this->get_header();
            ...
            if ($this->valid_api_response($api_response)) {
                 if(isset($api_response['UpdateFareQuoteRoundtrip'])){
                    $response['data'] = $api_response['UpdateFareQuoteRoundtrip']['FareQuoteDetails']['JourneyList'];
                }
                else{
...
//debug($pax_ticket_ids);exit;
             ...
             // debug($cancel_booking_request); die;
                ...
                // client said no api hit for Cancellation
    ...
    private function WSPassenger($passenger) {
        ...
        for ($i = 0; $i < $total_pax_count; $i++) {
            ...
            /* if(isset($passenger['billing_email']) && !empty($passenger['billing_email'])){
                $tmp_passenger[$i]['Email'] = $passenger['billing_email'];
            }else{
    ...
    function update_pax_markup_currency(& $price_summary, & $currency_obj, $level_one_markup = false, $current_domain_markup = true, $pax_count = 0, $search_id = 0) {
        ...
        $multiplier = ($total_pax * $way_count);
        return $this->update_markup_currency($price_summary, $currency_obj, $level_one_markup, $current_domain_markup, $multiplier, $is_domestic);
    }
     ...
     */
    function get_commission(& $__trip_flight, & $currency_obj, $type) {
        #cadebug($__trip_flight['FareDetails']);
        $group_id = 2;
        $CI =& get_instance();
        $user_oid = $CI->entity_user_id;
        $this->commission = $currency_obj->get_commission($__trip_flight,$user_oid, $group_id, TRAVELPORT_GDS_BOOKING_SOURCE, $type);
        if (valid_array($this->commission) == true && intval($this->commission['admin_commission_list']['value']) > 0) {
            ...
            $core_agent_commision = $__trip_flight['FareDetails']['AgentCommission'];
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '', $promotional_plan_type = '', $booking_source = TRAVELPORT_GDS_BOOKING_SOURCE) {
        $booking_form = '';
    ...
    public function search_data_in_preferred_currency($search_result, $currency_obj, $search_id) {
        ...
        if($search_params['data']['is_domestic'] == 1){
            $type = 'domestic';
        }else{
            $type = 'international';
        }
        $flights = $search_result['JourneyList'];
        $bs = $GLOBALS['CI']->custom_db->single_table_records('booking_source', 'origin', array('source_id' => TRAVELPORT_GDS_BOOKING_SOURCE));
        ...
        foreach ($flights as $fk => $fv) {
            foreach ($fv as $list_k => $list_v) {
                /*debug($list_v);
                exit();*/
                 //$class = $list_v['FlightDetails']['Details'][0][0]['OperatorName'];
                 $class = $list_v['FlightDetails']['Details'][0][0]['CabinClass'];
                 $operator = $list_v['FlightDetails']['Details'][0][0]['OperatorCode'];
                 $master_commission = $GLOBALS['CI']->flight_model->master_tp_commission($bs_origin, $class, $operator, $type);

                 if(empty($master_commission) || (@$master_commission[0]['value'] == '0.00')){
                    $master_commission = $GLOBALS['CI']->flight_model->master_commission_generic_tp(TRAVELPORT_GDS_BOOKING_SOURCE);
                    ...
                    $commission_value = ($list_v['Price']['PriceBreakup']['BasicFare']) * ($commission / 100);;
                    $commisssion_tds = ($commission_value) * ($tds/100);
                    $percent_to_pass = $commission;
                 }else{
                    ...
                    $basic_commission_value = ($list_v['Price']['PriceBreakup']['BasicFare']) * ($basic_commission / 100);
                    if(isset($list_v['Price']['PriceBreakup']['Tax_Details']['YQ'])){
                        $yq_commission_value = ($list_v['Price']['PriceBreakup']['Tax_Details']['YQ']) * ($yq_commission / 100);
                    ...
                    $percent_to_pass = $basic_commission;
                 }
                ...
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommissionPerc'] = $commission;
                ...
                debug($flight_list[$fk][$list_k]['Price']['PriceBreakup']['BasicFare']);
                debug($commission_value);
                $agent_commission = ($flight_list[$fk][$list_k]['Price']['PriceBreakup']['BasicFare']) * ($commission_value/100);
                debug($agent_commission);
                $agent_tds = $agent_commission *($commission_tds / 100);
                debug($agent_tds);
                exit;*/
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommission'] = $commission_value;
                $flight_list[$fk][$list_k]['FareDetails']['AgentTdsOnCommision'] = $commisssion_tds;
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommissionPerc'] = $percent_to_pass;
            }
        }
    ...
    public function extract_flight_details($flights, $currency_obj, $search_id, $module, $ins_token = false) {
        ...
        foreach ($flights as $fk => $fv) {
            $fight_data = $this->extract_flight_segment_fare_details($fv, $currency_obj, $search_id, $module, $ins_token, $token, $fk);
          
            if(valid_array($fight_data)){
                 $formatted_flight_list[$fk] = $fight_data;
            }
        }
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
        ...
        $blocked_airlines = $this->CI->flight_model->manage_blocked_airlines(TRAVELPORT_GDS_BOOKING_SOURCE);
        ...
        foreach ($flights as $list_k => $list_v) {
            ...
            $flight_list[$list_k]["FareType"] = $list_v['Price']["Fare_Type"];
            ...
            $flight_list[$list_k]['booking_source'] = TRAVELPORT_GDS_BOOKING_SOURCE;
    ...
    public function merge_flight_segment_fare_details($flight_details) {
        ...
        $flight_pre_booking_summery['Attr'] = $flight_details[0]['Attr'];
        ...
        foreach ($tmp_fare_breakdown as $k => $v) {
            foreach ($v as $pax_k => $pax_v) {
                ...
                if (isset($PassengerFareBreakdown[$pax_type]) == false) {
                    ...
                    $PassengerFareBreakdown[$pax_type]['Tax'] = $pax_v['Tax'];
                } else {
                    $PassengerFareBreakdown[$pax_type]['BaseFare'] += $pax_v['BaseFare'];
                    $PassengerFareBreakdown[$pax_type]['Tax'] += $pax_v['Tax'];
                }
    ...
    private function get_fare_object($flight_details, $currency_obj, $search_id, $module) {
        ...
        if ($module == 'b2c') {
           ...
           // echo $o_Total_Fare;exit;
            ...
            $search_data = $this->search_data($search_id);
            if($search_data['data']['is_domestic'] == 1){
                $type = 'domestic';
            }else{
                $type = 'international';
            }
            $this->get_commission($flight_details, $currency_obj, $type);
            ...
            $agent_comm  = $this->commission["admin_commission_list"];
...
//            debug($admin_price_details);debug($agent_price_details);exit;
            ...
            $b2b_price_details["_Commission_perc"] = $agent_comm["value"];
    ...
    private function update_segment_details(& $segments) {
        foreach ($segments as $k => & $v) {
            $airline_name = $GLOBALS['CI']->db_cache_api->get_airline_code_list(array(
            'code' => $v['OperatorCode']
            ));
            ...
            $AirlineDetails['AirlineName'] = $airline_name[$v['OperatorCode']];
  ...
  //       echo "<br/>";
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
       ...
       // error_reporting(E_ALL);
        ...
        foreach ($token as $token_index => $token_value) {
            ...
            $pass_break_down = $token_value['PassengerFareBreakdown'];
            $tmp_domain_markup = 0;
            ...
            $tranaction_attributes['Breakdown'] = $pass_break_down;
    ...
    public function update_booking_details($book_id, $book_params, $ticket_details, $module = 'b2c') {
        ...
        // echo $master_transaction_status;exit;
        ...
        if (valid_array($ticket_details) == true) {
            ...
            foreach ($ticket_details as $ticket_index => $ticket_value) {
                ...
                if ($this->valid_flight_booking_status($ticket_value['Status']) == true) {//IF Ticket is HOLD/CONFIRMED
                    ...
                    $update_transaction_data['gds_pnr'] = $gds_pnr;
                   ...
                   // $update_transaction_data['agent_markup'] = $agent_markup;
                    ...
                    foreach ($segment_details as $seg_k => $seg_v) {
                        foreach ($seg_v as $ws_key => $ws_val) {
                            ...
                            $update_segement_data['status'] = $master_transaction_status;
