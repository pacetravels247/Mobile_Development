 ...
 * @version    V1
 */
class Travelport_ach extends Common_Api_Grind {
    ...
    function update_fare_quote_details(& $flight_booking_details, $index, $new_quote_details, $process_quote_request, & $response) {
        ...
        unset($flight_booking_details['token'][2]);
        unset($flight_booking_details['token_key'][2]);
        unset($flight_booking_details['search_access_key'][2]);
    }
...
//debug($pax_ticket_ids);exit;
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            ...
            if ($clean_search_details['status'] == true) {
                ...
                if(valid_array($clean_search_details['data']['carrier'])){
     ...
     */
    function update_markup_currency(& $price_summary, & $currency_obj, $level_one_markup = false, $current_domain_markup = true, $multiplier = 1, $specific_markup_config = array(), $module='', $isdomestic='') {
        ...
        foreach ($price_summary as $__k => $__v) {
            if (is_numeric($__v) == true) {
                ...
                if (in_array($__k, $markup_list)) {
                    $temp_price = $currency_obj->get_currency($__v, true, $level_one_markup, $current_domain_markup, $multiplier, $specific_markup_config, $isdomestic);
    ...
    function get_commission(& $__trip_flight, & $currency_obj, $type) {
        ...
        $this->commission = $currency_obj->get_commission($__trip_flight,$user_oid, $group_id, TRAVELPORT_ACH_BOOKING_SOURCE, $type);
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '', $promotional_plan_type = '', $booking_source = TRAVELPORT_ACH_BOOKING_SOURCE) {
        $booking_form = '';
    ...
    public function search_data_in_preferred_currency($search_result, $currency_obj, $search_id) {  
        ...
        $bs = $GLOBALS['CI']->custom_db->single_table_records('booking_source', 'origin', array('source_id' => TRAVELPORT_ACH_BOOKING_SOURCE));
        ...
        foreach ($flights as $fk => $fv) {
            foreach ($fv as $list_k => $list_v) {
                 ...
                 if(empty($master_commission) || (@$master_commission[0]['value'] == '0.00')){
                    $master_commission = $GLOBALS['CI']->flight_model->master_commission_generic_tp(TRAVELPORT_ACH_BOOKING_SOURCE);
                    if(empty($master_commission) || ($master_commission[0]['value'] == '0.00')){
    ...
    public function format_search_response($search_result, $currency_obj, $search_id, $module, $from_cache = false, $search_hash = '') {
        ...
        //Start Addition
        $search_params = $this->search_data($search_id);
        if($search_params["data"]["trip_type"]=="multicity")
        {
            $from_seg_count = count($search_params["data"]["from"]);
            foreach($formatted_flight_list[0] AS $key => $ffl)
            {
                if(count($ffl["SegmentSummary"]) < $from_seg_count)
                    unset($formatted_flight_list[0][$key]);
            }
           
            if(valid_array($formatted_flight_list[0])){

                $formatted_flight_list[0] = array_values($formatted_flight_list[0]);
            }
            else{
                 $formatted_flight_list = array();
            }
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
        ...
        foreach ($flights as $list_k => $list_v) {
          //debug($list_v); exit;
            ...
            $flight_list[$list_k]['booking_source'] = TRAVELPORT_ACH_BOOKING_SOURCE;
  ...
  //       echo "<br/>";
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
       ...
       // echo $journey_from;exit;
        ...
        if ($safe_search_data['is_domestic_one_way_flight'] == true && strtolower($from_to_trip_type) == 'circle') {
        ...
        $GLOBALS['CI']->flight_model->add_extra_service_price_to_published_fare($app_reference, $gst_value);
		...
		$response['extra_service_price'] = $extra_services_total_price;
		//debug($response); exit;
        return $response;
    }
            ...
            foreach ($booking_transaction_details as $tr_k => $tr_v) {
                ...
                foreach ($book_params["meal_$meal_index"] as $meal_k => $meal_v) {
                    ...
                    if (empty($meal_v) == false && isset($meal_details[$meal_v]) == true && $meal_details[$meal_v]['JourneyType'] == $journy_type) {
                        ...
                        $description = str_replace(',', ' ', $meal_details[$meal_v]['Description']);
