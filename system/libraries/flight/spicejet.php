 ...
 * @version    V1
 */
class Spicejet extends Common_Api_Grind {
    ...
    function credentials($service) {
        ...
        $token = $computerName.$_SERVER['HTTP_USER_AGENT'];
        $token = md5($token);
        switch ($service) {
            case 'Search':
                $this->service_url = $this->Url . 'Search/'.$token;
                break;
            case 'FareRule':
                $this->service_url = $this->Url . 'FareRule/'.$token;
                break;
            case 'UpdateFareQuote':
                $this->service_url = $this->Url . 'UpdateFareQuote/'.$token;
                break;
            case 'UpdateFareQuoteRoundtrip':
                $this->service_url = $this->Url . 'UpdateFareQuoteRoundtrip';
                break;  
            case 'ExtraServices':
                $this->service_url = $this->Url . 'ExtraServices/'.$token;
                break;
            case 'HoldTicket':
                $this->service_url = $this->Url . 'HoldTicket/'.$token;
                break;
            case 'CommitBooking':
                $this->service_url = $this->Url . 'CommitBooking/'.$token;
                break;
            case 'IssueHoldTicket':
                $this->service_url = $this->Url . 'IssueHoldTicket/'.$token;
                break;
            case 'CancelBooking':
...
// debug($response['data']);exit;
    ...
    public function get_extra_services($flight_booking_details) {
        ...
        if (count($unique_search_access_key) == 1) {
            ...
            /*debug('DATA______________________________________________');
            debug($data);
            exit;*/
    ...
    function update_fare_quote_details(& $flight_booking_details, $index, $new_quote_details, $process_quote_request, & $response) {
        // echo $process_quote_request;exit;
...
//debug($pax_ticket_ids);exit;
    ...
    function get_commission(& $__trip_flight, & $currency_obj) {
        ...
        $this->commission = $currency_obj->get_commission($__trip_flight ,$user_oid, $group_id, SPICEJET_BOOKING_SOURCE);
     ...
     * return booking form
     */
    function booking_form($isDomestic, $token = '', $token_key = '', $search_access_key = '', $promotional_plan_type = '', $booking_source = SPICEJET_BOOKING_SOURCE) {
        $booking_form = '';
    ...
    public function search_data_in_preferred_currency($search_result, $currency_obj) {
        ...
        $flight_list = array();
        $master_commission = $GLOBALS['CI']->flight_model->master_commission(SPICEJET_BOOKING_SOURCE);
        ...
        foreach ($flights as $fk => $fv) {
            foreach ($fv as $list_k => $list_v) {
                ...
                $base_fare = $flight_list[$fk][$list_k]['FareDetails']['BaseFare'];
                $commisssion_value = ($base_fare) * ($commission / 100);
                $tds_value = ($commisssion_value) * ($tds / 100);
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommissionPerc'] = $commission;
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommissionTds'] = $tds;
                $flight_list[$fk][$list_k]['FareDetails']['AgentCommission'] = $commisssion_value;
                $flight_list[$fk][$list_k]['FareDetails']['AgentTdsOnCommision'] = $tds_value;
            }
        }
    ...
    public function extract_flight_segment_fare_details($flights, $currency_obj, $search_id, $module, $ins_token = false, & $token = array(), $flight_index = 0) {
        ...
        foreach ($flights as $list_k => $list_v) {
            ...
            $flight_list[$list_k]['FareType'] = $list_v['Attr']['FareType'];
            //debug($list_v['Attr']);
            ...
            $flight_list[$list_k]['booking_source'] = SPICEJET_BOOKING_SOURCE;
            ...
            if (isset($list_v['Attr']) == true) {
                ...
                $flight_list[$list_k]['Attr']["ClassOfService"] = $list_v['Price']['ClassOfService'];
            }
        }
        return $flight_list;
    }
  ...
  //       echo "<br/>";
    ...
    function save_booking($app_booking_id, $book_params, $currency_obj, $module = 'b2c') {
        ...
        foreach ($token as $token_index => $token_value) {
            /*$token_value['FareDetails']['api_PriceDetails']['OfferedFare'] = $token_value['FareDetails']['api_PriceDetails']['PublishedFare'] + $token_value[$module.'_PriceDetails']['_AgentEarning']; 
            $token_value['FareDetails']['api_PriceDetails']['AgentCommission'] = $token_value['FareDetails'][$module.'_PriceDetails']['_Commission'];
            $token_value['FareDetails']['api_PriceDetails']['AgentTdsOnCommision'] = $token_value[$module.'_PriceDetails']['_tdsCommission'];*/
            ...
            for ($i = 0; $i < $total_pax_count; $i++) {
                ...
                $is_lead = (empty($book_params['lead_passenger'][$i])) ? 0 : $book_params['lead_passenger'][$i];
        ...
        if (valid_array($ticket_details) == true) {
            ...
            foreach ($ticket_details as $ticket_index => $ticket_value) {
                ...
                if ($this->valid_flight_booking_status($ticket_value['Status']) == true) {//IF Ticket is HOLD/CONFIRMED
                    ...
                    $pnr = '';
                    if(empty($ticket_value['PNR'])){
                        $pnr = $ticket_value['GDSPNR'];
                    }else{
                        $pnr = $ticket_value['PNR'];
                    }
