    ...
    function credentials($service) {
        switch ($service) {
            ...
            case 'TicketRefundDetails':
                ...
                break;
            case 'GetBalance':
                $this->service_url = $this->Url . 'GetBalance';
                break;
        }
    }
...
//debug($pax_ticket_ids);exit;
     ...
     */
    function get_commission(& $__trip_flight, & $currency_obj, $booking_source, $search_id) {
        ...
        if (valid_array($this->commission) == true && intval($this->commission['admin_commission_list']['value']) > 0) {
            ...
            /*if($__trip_flight["FlightDetails"]["Details"][0][0]["OperatorCode"] == 'SG'){
                debug($__trip_flight);
                exit();
            }*/
     ...
     */
    public function format_search_response($search_result, $currency_obj, $search_id, $module, $from_cache = false, $search_hash = '', $booking_source='') {
        ...
        $ins_token = true;
        $formatted_flight_list = $this->extract_flight_details($flights, $currency_obj, $search_id, $module, $ins_token, $booking_source);
     ...
     *
     */
    public function extract_flight_details($flights, $currency_obj, $search_id, $module, $ins_token = false, $booking_source='') {
        $formatted_flight_list = array();
        //Token Details
        $token = array(); //This will be stored in local file so less data gets transmitted
        $this->ins_token_file = time() . rand(100, 10000);
        foreach ($flights as $fk => $fv) {
            $flight_data = $this->extract_flight_segment_fare_details($fv, $currency_obj, $search_id, $module, $ins_token, $token, $fk, $booking_source);
            if(valid_array($flight_data)){
                $formatted_flight_list[$fk] = $flight_data;
            }
...
//            debug($admin_price_details);debug($agent_price_details);exit;
  ...
  //       echo "<br/>";
    ...
    public function GettBalance(){
        
        $header_info = $this->get_header();
        $request['booking_source'] = 'PTBSID0000000002';
        $balance_request = json_encode($request);
        $this->credentials('GetBalance');
        $balance_response = $this->CI->api_interface->get_json_response($this->service_url, $balance_request, $header_info);
        $balance = '';
        // debug($balance_response);exit;
        if($balance_response['Status'] == SUCCESS_STATUS){
            $balance = $balance_response['GetBalance']['balance'];
        }
        return $balance;
       // echo $balance;exit;
    }
