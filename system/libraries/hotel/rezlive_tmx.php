        ...
        if(!empty($raw_hotel_lists)){
            ...
            foreach ($raw_hotel_lists as $r_hl_k => $r_hl_v) {
               ...
               //debug($hotel_info);
                ...
                $RoomDetails = $r_hl_v['RoomDetails'];
				
                $format_room_details = array();
				
				if (array_key_exists('0', $RoomDetails['RoomDetail'])) {
					
					foreach ($RoomDetails['RoomDetail'] as $k => $v) {
						$format_room_details['BoardBasis'][] = $v['BoardBasis']['@cdata'];
						$format_room_details['Type'][] = $v['Type']['@cdata'];
					}
				}else{
					$format_room_details['BoardBasis'][] = $RoomDetails['RoomDetail']['BoardBasis']['@cdata'];
					$format_room_details['Type'][] = $RoomDetails['RoomDetail']['Type']['@cdata'];
				}
//debug($format_room_details);exit;

                $list_data['RoomDetails'] = $format_room_details;
