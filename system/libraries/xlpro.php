...
// debug($emp_dat);//exit();
	...
	public function insert_bus_booking_details($booking_data=''){
		...
		if(!empty($booking_data)){
			...
			foreach ($booking_data['booking_customer_details'] as $cust_k => $cust_v) {
					...
					$admin_markup = $cust_v['admin_markup'];
					...
					$tblSalesDetails_data = array(
						...
						'decSalesDetailBasicMarkupAmt' => $admin_markup,
						'decSalesDetailTAXIAmt' => $tax,
						/*'decSalesDetailTAXIMarkupAmt' =>'',*/
						//'decSalesDetailTAXIIAmt' => ,
						/*'decSalesDetailTAXIIMarkupAmt' =>'',
						'decSalesDetailTAXIIIAmt' =>'',
						'decSalesDetailTAXIVAmt' => '',
						'decSalesDetailProcessingAmt' => '',
	...
	public function get_hotel_booking_details($booking = '',$temp_booking = ''){

		$status = 'BOOKING_CONFIRMED';
		//$temp_booking['book_id'] = 'HB15-173833-58733';
		//$temp_booking['booking_source'] = 'PTBSID0000000034';
		
		$hotel_details = $this->CI->hotel_model->get_booking_details($temp_booking['book_id'],$temp_booking['booking_source'],$status);
		//debug($hotel_details);die('++');
		if($hotel_details['status'] == SUCCESS_STATUS){
			$clean_data = $this->insert_hotel_booking_details($hotel_details['data']);
		}
	}

	public function insert_hotel_booking_details($booking_data=''){
		$response = array();
		$response['status'] = FAILURE_STATUS;
		$DB2 = $GLOBALS['CI']->load->database('second_db', TRUE);
		...
		if(!empty($booking_data)){
			...
			$admin_commission = 0;
			$agent_commission = 0;
			foreach ($booking_data['booking_itinerary_details'] as $key => $value) {
				$total_fare += $value['total_fare'];
				$admin_markup += $value['admin_markup'];
				$agent_markup += $value['agent_markup'];
				$admin_tds += $value['TDS'];
			...
			$tblSalesHead_data = array(
				...
				//'strSalesHeadPlaceofSupplyID' => '',
				'tintSalesHeadType' => 0,
				'intSalesHeadServiceID' =>  14,
				'intSalesDetailServiceID' => 5,
				/*'tintSalesHeadDiscount' => 0,
				'strSalesHeadDiscountAon' => '',
				'decSalesHeadDiscountAon' => 0,
				'decSalesHeadDiscountAPrc' => 0,
			...
			// insert customer details data
			$pax_count = count($booking_data['booking_customer_details']);
			$count = 1;
			foreach ($booking_data['booking_customer_details'] as $cust_k => $cust_v) {

					//debug($booking_details);
					//debug($cust_v);die('99999999');
					$hotel_name = $booking_details['hotel_name'];
					$p_fare = $total_fare/$pax_count;
					$admin_markup = $admin_markup/$pax_count;
					
					$check_in = date('d/m/Y',strtotime($booking_data['booking_itinerary_details'][0]['check_in']));
					$check_out = date('d/m/Y',strtotime($booking_data['booking_itinerary_details'][0]['check_out']));


					$pax_config = '';
					$adult_count = 0;
					$child_count = 0;
					foreach ($booking_data['booking_customer_details'] as $c_k => $c_v) {
						if($c_v['pax_type'] = 'Adult'){
							$adult_count++;
						}else{
							$child_count++;
						}
					...
					if($adult_count != 0){
						$pax_config .= $adult_count.' Adult';
					}else if($child_count != 0){
						$pax_config .= '|'.$child_count.' Child';
					}

						
					$DetailNotes = $hotel_name.'|'.'Check In '.$check_in.' to Check Out '.$check_out.'|'.'Room Type'.'|'.$pax_config.'| MAP Plan';

					$tblSalesDetails_data = array(
						'intBookingReferenceID' => $last_insert_id,
						'intBookingReferenceDetailID' => $count++,
						'strSalesDetailPassenger' => $cust_v['title'].' '.$cust_v['first_name'].' '.$cust_v['last_name'],
						'strSalesDetailNotes' => $DetailNotes,
						//'strAccountAirPrefix' => $bus_code,
						//'strAccountAirCode' => '',
						//'strAirlineAccountID' =>'',
						//'tintSalesDetailFromStock' => $_is_bsp,
	...
	public function get_bus_sales_return_details($app_reference,$booking_details,$cancellation_details){

		//$DB2 = $GLOBALS['CI']->load->database('second_db', TRUE);
		//tblSalesReturnHead
		$status = '';
		$app_reference = 'BB07-150928-852507';
		$booking_details['booking_source'] ='PTBSID0000000033';

		$bus_cancel_details = $GLOBALS['CI']->db->where(array('app_reference'=>$app_reference))->get('bus_cancellation_details')->result_array();

		$bus_booking_details = $this->CI->bus_model->get_booking_details($app_reference,$booking_details['booking_source'],$status);
		$booking_data = $bus_booking_details['data'];
		//debug($booking_data);
		//die('++++++');

		$tblSalesReturnHead_data = array(

				//'tblSalesReturnHead' => 'Primary key',
				'strCancellationReferenceNo' => $bus_cancel_details[0]['origin'],
				'intBookingReferenceID' => $bus_cancel_details[0]['app_reference'],
				'strSalesReturnHeadPrefix' => 'PBB',
				'dtSalesReturnHeadDate' => date('Y-m-d',strtotime($bus_cancel_details[0]['created_datetime'])),
				'dtSalesReturnHeadDueDate' => date('Y-m-d',strtotime($bus_cancel_details[0]['created_datetime'])),
				//'strIncomeAccountID' => '',
				'strCustomerAccountID' => $bus_cancel_details[0]['created_by_id'],
				//'strSalesReturnHeadPlaceofSupplyID' => '',
				'tintSalesReturnHeadType' => 0,
				'intSalesReturnHeadServiceID' => 4, //for bus 
				//'intSalesReturnDetailServiceID' => '',
				/*'tintSalesReturnHeadDiscount' => '',
				'strSalesReturnHeadDiscountAon' => '',
				'decSalesReturnHeadDiscountAon' => '',
				'decSalesReturnHeadDiscountAPrc' => '',
				'decSalesReturnHeadDiscountAAmt' => '',
				'strSalesReturnHeadDiscountBon' => '',
				'decSalesReturnHeadDiscountBon' => '',
				'decSalesReturnHeadDiscountBPrc' => '',
				'decSalesReturnHeadDiscountBAmt' => '',*/
				/*'decSalesReturnHeadRefundablePrc' => '',
				'decSalesReturnHeadRefundableAmt' => '',
				'decSalesReturnHeadTDSPrc' => '',
				'decSalesReturnHeadTDSAmt' => '',
				'strSalesReturnHeadServiceTaxIon' => '',
				'decSalesReturnHeadServiceTaxIon' => '',
				'decSalesReturnHeadServiceTaxITaxPrc' => '',
				'decSalesReturnHeadServiceTaxICessPrc' => '',
				'decSalesReturnHeadServiceTaxIPrc' => '',
				'decSalesReturnHeadServiceTaxIAmt' => '',
				'tintSalesReturnHeadServiceRentACabTaxon' => '',
				'bitSalesReturnHeadReverseCharge' => '',
				'decSalesReturnHeadIGSTTaxPrc' => '',
				'decSalesReturnHeadIGSTTaxAmt' => '',
				'decSalesReturnHeadCGSTTaxPrc' => '',
				'decSalesReturnHeadCGSTTaxAmt' => '',
				'decSalesReturnHeadSGSTTaxPrc' => '',
				'decSalesReturnHeadSGSTTaxAmt' => '',
				'decSalesReturnHeadGSTCessPrc' => '',
				'decSalesReturnHeadGSTCessAmt' => '',
				'decSalesReturnHeadGSTTaxPrc' => '',
				'decSalesReturnHeadGSTAmt' => '',*/
				//'decSalesReturnHeadRoundOffAmt' => '',
				'decSalesReturnHeadNetAmt' => $bus_cancel_details[0]['api_refund_amount'],
				/*'decCustomerCardBasicAmt' => '',
				'strSalesReturnHeadNarration' => '',
				'intSalesReturnHeadAdult' => '',
				'intSalesReturnHeadChild' => '',
				'intSalesReturnHeadInfant' => '',*/
				'strPurchaseReturnReferenceNumber' => $bus_cancel_details[0]['app_reference'],
				//'strExpensesAccountID' => '',
				'strSupplierAccountID' => $newstring = substr($booking_details['booking_source'], -3),
				'strPurchaseReturnPlaceofSupplyID' => 29,
				/*'strPurchaseReturnNarration' => '',
				'decPurchaseReturnProcessingAmt' => '',
				'decPurchaseReturnOtherAmt' => '',
				'tintPurchaseReturnTAC' => '',
				'strPurchaseReturnTACAon' => '',
				'decPurchaseReturnTACAon' => '',
				'decPurchaseReturnTACAPrc' => '',
				'decPurchaseReturnTACAAmt' => '',
				'strPurchaseReturnTACBon' => '',
				'decPurchaseReturnTACBon' => '',
				'decPurchaseReturnTACBPrc' => '',
				'decPurchaseReturnTACBAmt' => '',*/
				//'decPurchaseReturnRefundablePrc' => '',
				//'decPurchaseReturnRefundableAmt' => '',
				/*'decPurchaseReturnTDSPrc' => '',
				'decPurchaseReturnTDSAmt' => '',
				'strPurchaseReturnServiceTaxon' => '',
				'decPurchaseReturnServiceTaxon' => '',
				'decPurchaseReturnServiceTaxTaxPrc' => '',
				'decPurchaseReturnServiceTaxCessPrc' => '',				
				'decPurchaseReturnServiceTaxPrc' => '',
				'decPurchaseReturnServiceTaxAmt' => '',
				'bitPurchaseReturnReverseCharge' => '',
				'decPurchaseReturnIGSTTaxPrc' => '',
				'decPurchaseReturnIGSTTaxAmt' => '',
				'decPurchaseReturnCGSTTaxPrc' => '',
				'decPurchaseReturnCGSTTaxAmt' => '',
				'decPurchaseReturnSGSTTaxPrc' => '',
				'decPurchaseReturnSGSTTaxAmt' => '',
				'decPurchaseReturnGSTCessPrc' => '',
				'decPurchaseReturnGSTCessAmt' => '',
				'bitPurchaseReturnCenvatCredit' => '',*/
				//'decPurchaseReturnRoundOffAmt' => '',
				'decPurchaseReturnNetAmt' => $bus_cancel_details[0]['api_refund_amount'],
				/*'tintPurchaseReturnCard' => '',
				'strPurchaseReturnCardAccountID' => '',
				'strPurchaseReturnCardNumber' => '',
				'decPurchaseReturnCardBasicAmt' => '',
				'strPurchaseReturnCardChargesAccountID' => '',
				'decPurchaseReturnCardChargesAmt' => '',
				'strDatabaseName' => '',
				'intBranchID' => '',
				'strAction' => '',
				'strImportStatus' => '',
		...
		// insert into 'tblSalesReturnHead';
		//$DB2->insert('tblSalesReturnHead',$tblSalesHead_data);
		//$last_insert_id = $DB2->insert_id();
		$last_insert_id = '123';
		debug($tblSalesReturnHead_data);
		//die('===========');

		$total_pax = count($booking_data['booking_customer_details']);
		$charge_attr = json_decode($bus_cancel_details[0]['attributes'],1);
		$charge_amount = $charge_attr['data']['CancelSeats']['ChargeAmt'];
		$count = 1;
		foreach ($booking_data['booking_customer_details'] as $cust_k => $cust_v) {
				
				$ticket_no = $booking_data['booking_details'][0]['ticket'];
				$pnr_explode = explode(',', $booking_data['booking_details'][0]['pnr']);
				$pnr = '';
				if(count($pnr_explode) > 1){
					$pnr = $pnr_explode[$cust_k];
				}else{
				...
				$tblSalesReturnDetail_data = array(
						'intCancellationReferenceID' => $last_insert_id,
						'intCancellationReferenceDetailID' => $count++,
						'strSalesReturnDetailPassenger' => $cust_v['title'].' '.$cust_v['name'],
						//'strSalesReturnDetailNotes' => '',
						'strAccountAirPrefix' => $bus_code,
						/*'strAccountAirCode' => '',
						'strAirlineAccountID' => '',
						'tintSalesReturnDetailFromStock' => '',
						'intSalesReturnDetailPNRFromID' => '',
						'strSalesReturnDetailTicketPrefix' => '',*/
						//'intSalesReturnDetailTicketNo' => '',
						'strSalesReturnDetailAirlinePNRNo' => $pnr,
						/*'strSalesReturnDetailCRSPNRNo' => '',
						'strSalesReturnDetailSectorDetail' => '',*/
						'strSalesReturnDetailJRNDetail' => rtrim($jr_details,'$'),
						//'strSalesReturnDetailFareBasis' => '',
						'decSalesReturnDetailBasicAmt' => $base_f, 
						'decSalesReturnDetailBasicMarkupAmt' => $admin_markup,
						'decSalesReturnDetailTAXIAmt' => $tax,
						/*'decSalesReturnDetailTAXIMarkupAmt' => '',
						'decSalesReturnDetailTAXIIAmt' => '', 
						'decSalesReturnDetailTAXIIMarkupAmt' => '', 
						'decSalesReturnDetailTAXIIIAmt' => '',
						'decSalesReturnDetailTAXIVAmt' => '',*/
						'decSalesReturnDetailCancellationAmt' => $charge_amount/$total_pax, 
						/*'decSalesReturnDetailCancellationMarkupAmt' => '', 
						'decSalesReturnDetailProcessingAmt' => '',
						'decSalesReturnDetailOtherAmt' => '',*/
					);

					debug($tblSalesReturnDetail_data);//die();
					//$DB2->insert('tblSalesReturnDetail',$tblSalesDetails_data);
		}	
		
		die('55555555555555555');
	}
