...
class Srs_private extends Common_Api_Grind {
    ...
    public function __construct() {
     ...
     private function set_api_credentials() {
        ...
        $credentials = $this->CI->custom_db->single_table_records("supplier_credentials", "config", array("booking_source" => SRS_BUS_BOOKING_SOURCE, "mode"=>$bus_engine_system));
	...
	function get_all_operators()
	{
		...
		$this->opts_cache = $this->CI->cache->file->get("opts_".$this->CI->bus_engine_system."_".SRS_BUS_BOOKING_SOURCE);
		if(empty($this->opts_cache) || !isset($this->opts_cache)){
			...
			$cache_exp = "3600";
			$this->CI->cache->file->save("opts_".$this->CI->bus_engine_system."_".SRS_BUS_BOOKING_SOURCE, $bitla_opts, $cache_exp);
		}
	}
            ...
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                if ($request['status']) {
					...
					for($i = 0; $i < $city_arr_len; $i++){
						...
						if($id_to==0)
							$id_to = $bitla_to_id;
						$newest_arr[] = $id_form."-".$id_to;
					}
					$new_arr_search_unique = array_unique($newest_arr);
					$new_arr_search_unique = array_values($new_arr_search_unique);
					$new_city_arr_len = count($new_arr_search_unique);
                    for($i = 0; $i < $new_city_arr_len; $i++){
						/* $srs = 0;
						if($i==($new_city_arr_len-1))
							$srs = 1; */
                        $explode_data = explode('-',$new_arr_search_unique[$i]);
                        $id_form = $explode_data[0];
                        $id_to = $explode_data[1];
						//debug($id_form.' - '.$id_to);
                        if(($id_form != $bitla_from_id) || ($id_to != $bitla_to_id) || ($i==0)){
						//if($srs==1){
    ...
    function block_seats($search_id, $block_params) {
        ...
        $response['data'] = array();
        $response['status'] = SUCCESS_STATUS;
        //debug($block_params); exit("In SRS"); 
        if (valid_array($block_params)) {
            ...
            if ($request['status']) {
				...
				//debug($response_data1); exit("SRS Block");
    ...
    function process_booking($book_id, $booking_params) {
        echo "<pre>book_id SRS===== ";print_r($book_id);
        echo "<pre>booking_params SRS===== ";print_r($booking_params); die;
        $response['data'] = array();
        $response['status'] = SUCCESS_STATUS;
    ...
    function b2b_price_details_1($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $this->commission = $currency_obj->get_commission($__op_name,$user_oid,$group_id, SRS_BUS_BOOKING_SOURCE,'generic');
    ...
    function b2b_price_details($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        //$__op_name = $api_price_details['CompanyName'];
        if($api_price_details['booking_source'] == SRS_BUS_BOOKING_SOURCE){
    ...
    public function search_data($search_id) {
        ...
        $this->search_hash = md5(serialized_data($search_id.$response['data'].SRS_BUS_BOOKING_SOURCE));
        return $response;
    }
        ...
        if(is_array($combine_details) && !empty($combine_details)){
            if($api_balance['balance_amount'] >= 0){
                foreach ($combine_details as $k => $v__seg_details) {
                    ...
                    $_each_bus_details['booking_source'] = SRS_BUS_BOOKING_SOURCE;
    ...
    private function Tentative_Booking_request($block_params){
        ...
        /* $bitla_from_to_id = $this->bitla_search_data($block_params['token']['departure_from'],$block_params['token']['arrival_to']);
