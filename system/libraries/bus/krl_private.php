...
class Krl_private extends Common_Api_Grind {
    ...
    public function __construct() {
     ...
     private function set_api_credentials() {
        ...
        $credentials = $this->CI->custom_db->single_table_records("supplier_credentials", "config", array("booking_source" => KRL_BUS_BOOKING_SOURCE, "mode"=>"live"));
        $this->details = json_decode($credentials["data"][0]["config"], true);
        //$this->details = $this->CI->config->item ('krl_bus_live');
        $this->Url = $this->details ['api_url'];
        $this->Api_key = $this->details ['api_key'];
        //debug($this->details);die();
    }
     ...
     *
     */
    function GetService_Details_request($route_schedule_id, $journey_date, $route_code, $ResultToken, $booking_source) {
        $response['status'] = SUCCESS_STATUS;
        $response = array();
        $api_key = $this->Api_key;
    ...
    function get_bus_list($search_id = '') {
        ...
        if ($search_data['status'] == true) {
            ...
            if ($cache_search === false || ($cache_search === true && empty($cache_contents) == true)) {
                ...
                //debug($request);//exit('777');
                //get data
                if ($request['status']) {

                    $response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($request['request_url'],$request['api_key']);
                    //debug($response_data);exit('response');
                    if ($this->valid_search_result($response_data) && !empty($response_data['routes'])) {
    ...
    function get_bus_details($route_schedule_id, $journey_date, $route_code, $ResultToken, $booking_source) {
        ...
        if (empty($route_schedule_id) == false and empty($journey_date) == false) {
            //get request
            $header_info = $this->get_header();
            $request = $this->GetService_Details_request($route_schedule_id, $journey_date, $route_code, $ResultToken, $booking_source);
       ...
       // debug($block_params); exit;
        if (valid_array($block_params)) {
            ...
            if ($request['status']) {
                ...
                //$response_data1 = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($request['url'],$this->Api_key,$request['data']);

                //use static data, because it is live credentials.
                //$response_data1 = file_get_contents('comp_bitla_ets/kukkeshree_live_tent_booking_log');

                $response_data1 = '{"result":{"ticket_details":{"pnr_number":"TS200108152113175979YAMP","operator_pnr":"761507"}}}';
                //debug($response_data1);exit;
                $response_data1=json_decode($response_data1,1);
    ...
    function process_booking($book_id, $booking_params) {
        echo "<pre>book_id KRL===== ";print_r($book_id);
        echo "<pre>booking_params KRL===== ";print_r($booking_params); die;
        $response['data'] = array();
        $response['status'] = SUCCESS_STATUS;
        ...
        if (valid_array($booking_params)) {
            ...
            if ($request['status']) {
                ...
                //$response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($request['request_url'],$request['api_key'],$request['data']);

                //use static data, because it is live credentials.
                //$response_data = file_get_contents('comp_bitla_ets/kukkeshree_live_booking_log');

                $response_data = '{"result":{"ticket_details":{"ticket_status":"Confirmed","ticket_number":"TS200108161455724765OPAV","operator_pnr":"761553","travel_operator_pnr":"761553","fromcity":"Dharwad","to-city-id":"Bangalore","journey_date":"2020-01-09","sel-seats-count":"1","seat_nos":"5L","travels":"Kukkeshree Travels","service-no":"Dandeli-Haliyal-B-Lore","bus_type":"2+1 Sleeper Non-AC Video (30 seats)","dep_time":"10:30 PM","duration":"08:00","boarding_point_details":{"name":"Near Brindavan Hotel","dep_time":"10:30 PM","boarding_stage_address":"Beside brindavan hotel","landmark":"Beside Brindavan Hotel","contact_numbers":"0"},"agent_ref_number":"","total_fare":"600","service_tax_percent":"0","convenience_charge_percent":"0","seat_fare_details":[{"seat_detail":{"seat_number":"5L","fare":600,"our_commission":0,"service_tax":0,"convenience_charge":0,"offer_discount":0,"discount":0,"additional_fare":0,"tieup_agent_commission_percentage":90,"my_commission":90,"our_convenience_charge_amount":0}}],"commission":90,"passenger_details":{"title":"Mr","gender":"M","name":"Pacetravelstest","age":"26","mobile":"1234678965","email":"agent@pacetravel.com"},"commission_percent":0,"tieup_agent_commission_percent":90,"travel_id":32,"operator_reservation_id":68580,"operator_seat_wise_pnr":"","operator_seat_pnr_hash":[]}}}';
    ...
    function b2b_price_details_1($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $this->commission = $currency_obj->get_commission($__op_name,$user_oid,$group_id, KRL_BUS_BOOKING_SOURCE,'generic');
    ...
    function format_pre_cancel_data($is_cancellable_response_data)
    {
        ...
        $pre_cancel_data["refund_amount"] = $cancel_raw_data["refund_amount"];
    ...
    function pre_cancellation_data($booking_details, $app_reference)
    {
        ...
        if ($is_cancellable_request['status']) {
            ...
            $is_cancellable_response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree(
    ...
    function cancel_full_booking($booking_details, $app_reference ,$seat_to_cancel = "") {
        ...
        if ($is_cancellable_request['status']) {
            ...
            $is_cancellable_response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($is_cancellable_request['request_url'],$is_cancellable_request['api_key'],$is_cancellable_request['request_data']);
            //debug($is_cancellable_response_data); exit;
            ...
            if (valid_array($is_cancellable_response_data) == true) {
                ...
                //$IsCancellable2Result['is_cancellable'] = '1';
                if ($IsCancellable2Result['is_cancellable'] != '1') {
                    ...
                    //debug($cancell_ticket_request);die();
                    if ($cancell_ticket_request['status']) {
                        
                        //$cancell_ticket_response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($cancell_ticket_request['request_url'],$cancell_ticket_request['api_key'],$cancell_ticket_request['request_data']);

                    // use static data , because its having live credentials.
                        
                        $cancel_encode = '{"result": {"cancel_ticket": {"refund_amount": 498.75,"cancel-charges": 26.25,"seat_numbers": "D1","cancel_seat_details": [{"cancel_seat_detail": {"seat_number": "D1","cancel_percent": 5,"refund_amount":498.75,"cancelled_fare": 26.25,"base_cancelled_fare": 26.25,"cancelled_service_tax": 0}}],"operator_gst_details": {"category": "unregistered","sale_type": "Inter State","trans_type": "Ecommerce","registration_name": "PARSHWANATH TRAVELS PRIVATE LIMITED","gst_id": "08AAECP7257R1ZJ","cgst_percentage": 0,"sgst_percentage": 0,"igst_percentage": 5,"tcs_percentage": 0,"cancellation_charges": 26.25,"cgst_amount": 0,"sgst_amount": 0,"igst_amount": 23.75,"tcs_amount": 0}}}}';

                        $cancell_ticket_response_data = json_decode($cancel_encode,1);
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            ...
            if ($clean_search_details['status'] == true) {
                ...
                $bitla_from_to = $this->krl_search_data($response['data']['bus_station_from'],$response['data']['bus_station_to']);
        ...
        $this->search_hash = md5(serialized_data($search_id.$response['data'].KRL_BUS_BOOKING_SOURCE));
        return $response;
    }
    ...
    private function format_as_tmx_response($response_data,$search_data){
        /*debug($response_data);
        debug($search_data);
        die('HERE');*/
        $response = array();
        ...
        if(is_array($bus_route_details) && !empty($bus_route_details)){  
            foreach ($bus_route_details as $k => $v__seg_details) {
                ...
                $d_time = $v__seg_details['route']['departure-time'];
                ...
                $__exp_date = explode(':',$v__seg_details['route']['duration']);
                ...
                $_each_bus_details['booking_source'] = KRL_BUS_BOOKING_SOURCE;
    ...
    private function busLayout_formatting($response_data){
        ...
        if(isset($response_data['service_details'])){
            ...
            $seat_price = explode(',', $response_data['service_details']['fare']);
            $CompanyName = $response_data['service_details']['operator_service_name'];
            
            $is_st_applicable = $response_data['service_details']['is_stax_applicable'];
    ...
    private function Tentative_Booking_request($block_params){
        ...
        foreach ($no_of_seats as $k_seat => $v_seat) {
            ...
            $seat_detail['seat_number'] =  $k_seat;
            //fare changed Fare to base_fare
            $seat_detail['fare'] =  $v_seat['base_fare'];
        ...
        $bitla_from_to_id = $this->krl_search_data($block_params['token']['departure_from'],$block_params['token']['arrival_to']);
        ...
        $request_data['from_city_id'] = $from_id;
        $request_data['dest-city-id'] = $to_id;
        $request_data['pick-up-stage'] = $block_params['token']['PickUpID'];
        $request_data['total_seats'] = $count;
        $request_data['journeydate'] = $JourneyDate;
    ...
    public function format_booking_details_kukkeshree($booking, $temp_booking){
        /*debug($booking);
        debug($temp_booking);
        die('here');*/
        $response = array();
        ...
        //debug($ticket_details);die();
            ...
            $result['TotalSeats'] = $ticket_details['sel-seats-count'];
            ...
            $TotalSeats = $ticket_details['sel-seats-count'];
            $Passengers = array();
    ...
    private function krl_search_data($from_name,$to_name){

        $bus_station_from_id = $GLOBALS['CI']->custom_db->single_table_records('bus_stations_new1','krl_city_id', array('ets_city_name' => $from_name));
        $bus_station_to_id = $GLOBALS['CI']->custom_db->single_table_records('bus_stations_new1','krl_city_id', array('ets_city_name' => $to_name));

        $response['status'] = SUCCESS_STATUS;
        $response['from_id'] = $bus_station_from_id['data'][0]['krl_city_id'];
        $response['to_id'] = $bus_station_to_id['data'][0]['krl_city_id'];

        //$bus_station_from_id = '100307';//bangalore
        //$bus_station_to_id = '101915'; //Talikote
        //debug($response);die();
    ...
    private function get_balance_details($travel_id){
        if(!empty($travel_id)){
            ...
            $response_data = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($url,$api_key,$request_data);
