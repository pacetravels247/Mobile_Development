...
class Kukkeshree_private extends Common_Api_Grind {
    ...
    public function __construct() {
		...
		$canacel_policy = $this->get_cancel_policy();
		$this->canacel_policy = $this->format_cancel_policy($canacel_policy);
    }

     private function set_api_credentials() {
        $bus_engine_system = $this->CI->bus_engine_system;
        $this->system = $bus_engine_system;
        $credentials = $this->CI->custom_db->single_table_records("supplier_credentials", "config", array("booking_source" => KUKKESHREE_BUS_BOOKING_SOURCE, "mode"=>"live"));
        $this->details = json_decode($credentials["data"][0]["config"], true);
        //$this->details = $this->CI->config->item ('kukkeshree_bus_live');
	...
	function get_cancel_policy()
	{
		$url = $this->Url."/dir/api/cancellation_policies.json";
		$api_key = $this->Api_key;
		$res = $GLOBALS['CI']->api_interface->get_json_response_kukkeshree($url, $api_key);
		return $res["cancellation_policies"];
	}
    /**
     *  Balu A
       ...
       // debug($block_params); exit;
        if (valid_array($block_params)) {
            ...
            if ($request['status']) {
				...
				//debug($response_data1); exit("Kukk Block");
                ...
                // echo "<pre>response_data1===== "; print_r($response_data1); die;
    ...
    function process_booking($book_id, $booking_params) {
        echo "<pre>book_id KUKKESHREE===== direct ";print_r($book_id);
        echo "<pre>booking_params KUKKESHREE===== ";print_r($booking_params); die;
        $response['data'] = array();
        $response['status'] = SUCCESS_STATUS;
    ...
    function b2b_price_details_1($api_price_details, $admin_price_details, $agent_price_details, $currency_obj) {
        ...
        $this->commission = $currency_obj->get_commission($__op_name,$user_oid,$group_id, KUKKESHREE_BUS_BOOKING_SOURCE,'generic');
    ...
    public function search_data($search_id) {
        ...
        if (empty($this->master_search_data) == true and valid_array($this->master_search_data) == false) {
            ...
            if ($clean_search_details['status'] == true) {
                ...
                $bitla_from_to = $this->kukkeshree_search_data($response['data']['bus_station_from'],$response['data']['bus_station_to']);
        ...
        $this->search_hash = md5(serialized_data($search_id.$response['data'].KUKKESHREE_BUS_BOOKING_SOURCE));
        return $response;
    }
	...
	function format_cancel_policy($cp)
	{
		...
		{
			$new_cp[$cpk]["Mins"] = $cp[$cpk][1]*60;
			$new_cp[$cpk]["Pct"] = $cp[$cpk][2];
			$new_cp[$cpk]["Amt"] = 0;
		}
		return $new_cp;
	}
    ...
    private function format_as_tmx_response($response_data,$search_data){
        ...
        if(is_array($bus_route_details) && !empty($bus_route_details)){  
            foreach ($bus_route_details as $k => $v__seg_details) {
				...
				$_each_bus_details['CompanyName'] = "Kukkeshree Travels";
                ...
                $d_time = $v__seg_details['route']['departuretime'];
				...
				//debug($this->canacel_policy); exit;
                $_each_bus_details['CancPolicy']= $this->canacel_policy;
                ...
                $_each_bus_details['booking_source'] = KUKKESHREE_BUS_BOOKING_SOURCE;
    ...
    private function Tentative_Booking_request($block_params){
        ...
        $bitla_from_to_id = $this->kukkeshree_search_data($block_params['token']['departure_from'],$block_params['token']['arrival_to']);
        ...
        $request_data['from-city'] = $from_id;
        $request_data['destination_id'] = $to_id;
    ...
    private function kukkeshree_search_data($from_name,$to_name){

        $bus_station_from_id = $GLOBALS['CI']->custom_db->single_table_records('bus_stations_new1','kukkeshree_city_id', array('ets_city_name' => $from_name));
        $bus_station_to_id = $GLOBALS['CI']->custom_db->single_table_records('bus_stations_new1','kukkeshree_city_id', array('ets_city_name' => $to_name));

        $response['status'] = SUCCESS_STATUS;
        $response['from_id']=$bus_station_from_id['data'][0]['kukkeshree_city_id'];
        $response['to_id']=$bus_station_to_id['data'][0]['kukkeshree_city_id'];

        /*$bus_station_from_id = '115702';//bangalore
        $bus_station_to_id = '115819'; //chennai*/
        //debug($response);die();
        return $response;
    }
