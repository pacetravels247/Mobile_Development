...
function format_transaction_log($logs)
{
	...
	if(valid_array($logs) == true){
		...
		foreach($logs as $k => $v){
			...
			$transaction_log_data[$k]['credit_limit'] =		$credit_limit;
...
function format_account_ledger($logs)
{
	...
	if(valid_array($logs) == true){
		...
		foreach($logs as $k => $v){
			...
			$agent_tds = floatval($v['agent_tds']);
			...
			$credit_limit = $v['credit_limit'];
			$current_credit_limit = $v['current_credit_limit'];
			...
			$transaction_log_data[$k]['credit_limit'] =		$credit_limit;
			$transaction_log_data[$k]['current_credit_limit'] =	$current_credit_limit;
			$transaction_log_data[$k]['agent_commission'] =	$agent_commission;
			$transaction_log_data[$k]['agent_tds'] =	$agent_tds;
		}
		//debug($transaction_log_data); exit;
		$data['data'] = $transaction_log_data;
	}
	return $data;
}