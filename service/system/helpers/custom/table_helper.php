...
function get_account_link($datetime='', $id='', $name='')
{
	...
	if (empty($id) == true || empty($name) == true) {
		...
		return $link.'<a role="button" href="'.base_url().'general/account?aid='.urlencode($GLOBALS['CI']->encrypt->encode($id)).'&uid='.$id.'" class="btn btn-sm">'.$name.' <span class="glyphicon glyphicon-zoom-in"></span></a>';
	}
}
...
function get_profile_icon($icon, $size='', $alt='')
{
	if (empty($icon) == false) {
		switch ($size) {
			case THUMBNAIL : $image ='<img src="'.$GLOBALS['CI']->template->template_images('face.png').'" alt="'.$alt.'" width="35" height="35" class="img-rounded">';
			break;
			default : $image = '<img src="'.$GLOBALS['CI']->template->template_images('face.png').'" alt="'.$alt.'" width="75" height="75" class="img-rounded">';
			break;
		}
		return $image;
	}
