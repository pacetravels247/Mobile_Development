...
class Travelport extends Common_Api_Flight {
    ...
    private function format_search_data_response($low_fare_search_res_data, $search_data) {
        // debug($low_fare_search_res_data);exit;
        ...
        //debug($low_fare_search_res);exit;
     ...
     * array $pricing_array
     */
    function format_air_pricing_solution($pricing_array, $air_segment_list, $arrBaggageData, $arrFareruleData, $search_data) {
   ...
   //    debug($pricing_array);exit;
        ...
        if (isset($air_pricing_info) && valid_array($air_pricing_info)) {
            ...
            if(!empty($check_comm)){
                if($check_comm->value_type == 'percentage'){
                    $agent_commssion = ($approx_base_price*$check_comm->basic_value)/100;
                    $yq_value = ($yqvalue*$check_comm->YQ_value)/100;
                }
                else{
                   $agent_commssion = $approx_base_price+$check_comm->basic_value; 
                   $yq_value = $yqvalue+$check_comm->YQ_value;
                }
            }
            else{
                $agent_commssion = '0.00';  
                $yq_value = '0.00'; 
            }
            ...
            $flight_journey ['Price'] ['PriceBreakup'] ['AgentCommission'] = $agent_commssion+$yq_value;
            $flight_journey ['Price'] ['PriceBreakup'] ['AgentTdsOnCommision'] = $tds_commission;
           ...
           // debug($air_segment_list1);exit;
            ...
            //foreach ($journey_detail as $j_key => $journey_flight) {
                ...
                //if (isset($air_segment_ref) && valid_array($air_segment_ref)) {
                   ...
                   // foreach ($journey_detail as $seg_key => $segmnt) {
                           ...
                           // debug($air_segment_list);exit;
                            foreach ($air_segment_list1 as $seg_key => $segment) {
                               //debug($segment);exit;
                                ...
                                //if ($flight_key == $segment_ref) {
                                    if (isset($air_code_share_info) && valid_array($air_code_share_info)) {
                                        $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$seg_key] ['air_code_share'] = @$air_code_share_info ['@attributes'] ['OperatingCarrier'];
                                    }
                                    // booking_code
    ...
    private function flight_detail_format($segment, $flight_key, $trip_type, $is_leg) {
        ...
        $flight_detail_arr ['Origin'] = array(
            ...
            'DateTime' => @$depart_date." ".@$depart_time,
            'date' => $depart_date, // Derive
            'time' => $depart_time, // Derive H i/h i a
        ...
        $flight_detail_arr ['Destination'] = array(
            ...
            'DateTime' => @$depart_date." ".@$arrival_time,
            'date' => $arrival_date, // Derive
            'time' => $arrival_time, // Derive H i/h i a
    ...
    private function update_fare_quote_request($token, $search_id) {
       ...
       // debug($token);exit;
        ...
        $fp = fopen($path,"wb");fwrite($fp,$air_price_req);fclose($fp);
    ...
    //debug($arrPriceXML);exit;
        ...
        $fp = fopen($path,"wb");fwrite($fp,$AirCreateReservationReq);fclose($fp);
