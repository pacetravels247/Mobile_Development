...
function format_fare_rule_response($fare_rule_response) {
    ...
    private function format_search_data_response($low_fare_search_res_data, $search_data) {
        ...
        foreach ($low_fare_search_res_data as $key => $low_fare_search_res) {
           ...
           if(!empty($low_fare_search_res['air:LowFareSearchRsp']['air:FlightDetailsList']['air:FlightDetails'])){
                ...
                $trace_id = $low_fare_search_res['air:LowFareSearchRsp']['@attributes']['TraceId'];
                ...
                if($search_data['trip_type'] == 'return' && empty($search_data['is_domestic']) == false){
                    foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                        ...
                        // $air_flight_details = array();
                        $this->format_air_pricing_solution_domestic($pricing_array, $fare_info_key_arrays, $air_segment_list, $search_data, $airport_list, $airline_list, $air_flight_details, $flight_list_data, $arrHostTokenData, $trace_id);
                    ...
                    if (isset($air_pricing_solutions_data) && valid_array($air_pricing_solutions_data)) {
                        foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                            ...
                            $flight_data_list [] = $this->format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData,$airport_list,$airline_list,$hand_baggage_list, $arr_flight_details, $trace_id);
                        } 
                    }
     ...
     * array $pricing_array
     */
    function format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list1, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData,&$airport_list,&$airline_list,&$hand_baggage_list, $arr_flight_details = array(), $trace_id) {
        $CI = & get_instance();
       ...
       //debug($pricing_array); exit; 
                        ...
                        if (isset($air_segment_list) && !empty($air_segment_list)) {
                           foreach ($air_segment_list as $as_key => $segment) {
                                ...
                                if ($flight_key == $segment_ref) {
                                    ...
                                    // booking_code
                                    $flight_list_on_out ['flight_list'] [$j_key] ['flight_detail'] [$seg_key] ['TraceId']                     = @$trace_id;
        ...
        if (isset($pricing_array['air:AirPricingInfo']) and valid_array($AirPricingInfo)) {
          ...
          // debug($conversion_amount);exit;
            ...
            $flight_journey ['HoldTicket'] = false;
        }
...
private function flight_detail_format($segment, $flight_key, $trip_type, $is_leg,&$airport_list,&$airline_list, $arr_flight_details = array()) {
    ...
    function format_air_pricing_solution_domestic($pricing_array, $fare_info_key_array, $air_segment_list, $search_data, &$airport_list, &$airline_list, $air_flight_details, &$flight_list_data, $arr_host_token, $trace_id){
        // debug($arr_host_token);exit;
        ...
        foreach($flight_list_total as $flight_keyy => $flight_infos){
            ...
            foreach($air_booking_info as $b_key => $booking_info){
                ...
                $trip_type = '';
                $arr_flight_details = array();
                $details[0][] = $this->flight_detail_format_domestic($segment_data, $arr_key, $trip_type, $is_leg, $airport_list, $airline_list, $arr_flight_details, $search_data, $cabin_class, $booking_count, $origin_terminal, $destination_terminal);
                $is_leg = false;
                $connection = '';
            ...
            // debug($flight_list_on_out);exit;
            $fare_type = $fare_info_key_array[$fare_info_key]['FareFamily'];
            // $fare_type = '';
            // echo $fare_type;exit;
            ...
            $Refundable =1;
                ...
                $flight_list_data[$arr_key] ['HoldTicket'] = false;
                $token_data [0] = $flight_list_on_out;
                $token_data [0]['booking_source'] = $this->booking_source;
        ...
        // debug($flight_list_data);exit;
    }
    private function flight_detail_format_domestic($segment, $flight_key, $trip_type, $is_leg,&$airport_list,&$airline_list, $arr_flight_details = array(), $search_data, $cabin_class, $booking_count, $origin_terminal, $destination_terminal) {
        $FlightDetailsRef = @$segment['air:FlightDetailsRef']['@attributes']['Key'];
        $duration_sec = calculate_duration(@$segment ['@attributes'] ['DepartureTime'], @$segment ['@attributes'] ['ArrivalTime']);
        ...
        else if($search_data['from'] == 'CMB' || $search_data['from'] == 'DXB' || $search_data['from'] == 'MCT' || $search_data['from'] == 'SHJ' || $search_data['from'] == 'DOH' || $search_data['from'] == 'SIN' || $search_data['to'] == 'CMB' || $search_data['to'] == 'DXB' || $search_data['to'] == 'MCT' || $search_data['to'] == 'SHJ' || $search_data['to'] == 'DOH' || $search_data['to'] == 'SIN'){
 ...
 /*
    ...
    function flight_type_search_request($search_method,$segment_info,$adult,$child,$infant,$prefered_carrier, $search_data){
        $trace_id = $this->generateRandomString();
        ...
        $flight_type_search_request = '<?xml version="1.0" encoding="utf-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                ...
                <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                    <' . $search_method . ' SolutionResult="'.$solution_result.'" AuthorizedBy="user" TraceId="'.$trace_id.'" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v50_0">
    ...
    private function search_allowed($search_data)
    {
        ...
        $cabin_class_arr = array('economy','business','first');
       
        if( in_array($search_data['cabin_class'], $cabin_class_arr)){
            return TRUE;
        }
        else{
    ...
    public function format_update_fare_quote_response($air_price_response, $price_response, $search_id, $flight_details_key, $price_request,$flight_search_data, $flight_search_return_data='') {
        $search_data = $this->search_data($search_id);
        $search_data = $search_data['data'];
        $status = TRUE;
        if ($status = SUCCESS_STATUS) {
            /*$seg = $air_price_response['SOAP:Envelope']['SOAP:Body']['air:AirPriceRsp']['air:AirItinerary']['air:AirSegment'];
            $host = $air_price_response['SOAP:Envelope']['SOAP:Body']['air:AirPriceRsp']['air:AirItinerary']['common_v50_0:HostToken'];
            $seat_mapping_request = '<?xml version="1.0" encoding="utf-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                <SeatMapReq xmlns="http://www.travelport.com/schema/air_v50_0" TraceId="394d96c00971c4545315e49584609ff6" AuthorizedBy="Travelport" TargetBranch="'.$this->config ['Target'].'" ReturnSeatPricing="true" ReturnBrandingInfo="true">
  <BillingPointOfSaleInfo xmlns="http://www.travelport.com/schema/common_v50_0" OriginApplication="uAPI" />
  <AirSegment Key="'.$seg['@attributes']['Key'].'" Status="'.$seg['@attributes']['Status'].'" HostTokenRef="'.$seg['@attributes']['HostTokenRef'].'" Equipment="'.$seg['@attributes']['Equipment'].'" Group="'.$seg['@attributes']['Group'].'" Carrier="'.$seg['@attributes']['Carrier'].'" FlightNumber="'.$seg['@attributes']['FlightNumber'].'" Origin="'.$seg['@attributes']['Origin'].'" Destination="'.$seg['@attributes']['Destination'].'" DepartureTime="'.$seg['@attributes']['DepartureTime'].'" ArrivalTime="'.$seg['@attributes']['ArrivalTime'].'" FlightTime="'.$seg['@attributes']['FlightTime'].'" TravelTime="'.$seg['@attributes']['TravelTime'].'" ProviderCode="'.$seg['@attributes']['ProviderCode'].'" SupplierCode="'.$seg['@attributes']['SupplierCode'].'" ClassOfService="'.$seg['@attributes']['ClassOfService'].'">
    <CodeshareInfo OperatingCarrier="'.$seg['air:CodeshareInfo']['@attributes']['OperatingCarrier'].'" OperatingFlightNumber="'.$seg['air:CodeshareInfo']['@attributes']['OperatingFlightNumber'].'" />
  </AirSegment>
  <HostToken xmlns="http://www.travelport.com/schema/common_v50_0" Key="'.$host['@attributes']['Key'].'">'.$host['@value'].'</HostToken>
</SeatMapReq>
</s:Body></s:Envelope>';
echo 'Herer';
    debug($seat_mapping_request);
    $seat_response = $this->process_request($seat_mapping_request,'','Seat Mapping(Travelport Flight)');
    debug($seat_response);
    exit();*/
            $passenger = array();
            ...
            if(valid_array($flight_search_return_data)){
                // debug($flight_search_return_data);exit;
           ...
           // $air_segment = $air_price_response['SOAP:Envelope']['SOAP:Body']['air:AirPriceRsp']['']
            ...
            if($provider_code == 'ACH'){
                ...
                foreach ($optional_sevices as $opt_key => $opt_service) {
                    ...
                    if($opt_service['@attributes']['Type'] == 'Baggage'){
                        ...
                        // debug($opt_service_segment_baggage);exit;
                        if(count($common_sevice_data) > 1){
                            if(isset($opt_service['common_v50_0:ServiceInfo']['common_v50_0:Description'][0])){
                                $desc1 = $opt_service['common_v50_0:ServiceInfo']['common_v50_0:Description'][1];
                                if (strpos($desc1, 'Meal is available only on the first segment in a connecting flight') !== false) {
    ...
    public function get_update_fare_quote($request, $search_id, $return_request='') {
        ...
        // debug($update_fare_quote_request['request']);
        ...
        if ($update_fare_quote_request['status'] == SUCCESS_STATUS) {
            ...
            // $price_response = $this->CI->custom_db->get_static_response (219895);
            // debug($price_response);exit;
            ...
            if (valid_array($update_fare_quote_response) == true && isset($update_fare_quote_response['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                $response ['status'] = SUCCESS_STATUS;
                $response ['data']['FareQuoteDetails'] = $this->format_update_fare_quote_response($update_fare_quote_response, $price_response, $search_id, $flight_details_key, $update_fare_quote_request['request_opt'],$request, $return_request);
            } else {
                $response ['message'] = 'Not Available';
            }
        } else {
    ...
    private function update_fare_quote_request($token, $return_request, $search_id) {
      ...
      //  $search_id = '6045';
            ...
            foreach ($token['flight_list'] as $fl_ley => $flights_list) {
                if (isset($flights_list ['flight_detail']) && valid_array($flights_list ['flight_detail'])) {
                    foreach ($flights_list ['flight_detail'] as $flight_key => $flight) {
                        ...
                        $air_code_share = isset($flight ['air_code_share']) && !empty($flight ['air_code_share']) ? '<CodeshareInfo OperatingCarrier="6E" OperatingFlightNumber="'.$flight_number.'">Indigo</CodeshareInfo>' : '';
                        $class_of_service ='';
                        $status ='';
                        $apis_req_ref ='';
                        $suppliercode ='';
        ...
        if ($provider_code == '1G') {
            ...
            $all_price_modfier = '<AirPricingModifiers/>';
        }
        ...
        $air_price_req = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
            <s:Header/>
            <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                <AirPriceReq TraceId="'.$trace_id.'" CheckOBFees="All" AuthorizedBy="user" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v50_0" xmlns:common_v50_0="http://www.travelport.com/schema/common_v50_0">
        ...
        if(!empty($Age)){
         ...
         if ($pax_count != 0) {
            for ($i = 0; $i < $pax_count; $i ++) {
                $pax_xml .= '<SearchPassenger Key="' . $paxId . '" Code="'.$pax_code.'" '.$Age.' xmlns="http://www.travelport.com/schema/common_v50_0" ></SearchPassenger>';
                $paxId ++;
            }
        }
        return $pax_xml;
    ...
    public function seat_request($request) {
      // debug($request);
        ...
        $flight_details = $request['flight_list'];
        $segmentCnt = 0;
        $journeyCnt = 0;
        $dn = 0;
        for ($se = 0; $se < count($flight_details); $se++) {

            $flight_deta = $flight_details[$se];
            foreach($flight_deta['flight_detail'] as $fd_k => $flight_det){
                $seat_mapping_request = '<?xml version="1.0" encoding="utf-8"?>
                <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                    <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                    <SeatMapReq xmlns="http://www.travelport.com/schema/air_v50_0" TraceId="394d96c00971c4545315e49584609ff6" AuthorizedBy="Travelport" TargetBranch="'.$this->config ['Target'].'" ReturnSeatPricing="true" ReturnBrandingInfo="true">
                      <BillingPointOfSaleInfo xmlns="http://www.travelport.com/schema/common_v50_0" OriginApplication="uAPI" />
                      <AirSegment Key="'.$flight_det['flight_seg_key'].'" Status="'.$flight_det['status'].'" HostTokenRef="'.$flight_det['host_token_key'].'" Equipment="'.$flight_det['equipment'].'" Group="'.$flight_det['group'].'" Carrier="'.$flight_det['carrier'].'" FlightNumber="'.$flight_det['flight_number'].'" Origin="'.$flight_det['origin'].'" Destination="'.$flight_det['destination'].'" DepartureTime="'.$flight_det['departure_time'].'" ArrivalTime="'.$flight_det['arrival_time'].'" FlightTime="'.$flight_det['flight_time'].'" TravelTime="'.$flight_det['flight_time'].'" ProviderCode="'.$flight_det['provider_code'].'" SupplierCode="'.$flight_det['ProviderCode'].'" ClassOfService="'.$flight_det['booking_code'].'">
                        <CodeshareInfo OperatingCarrier="'.$flight_det['air_code_share'].'" OperatingFlightNumber="'.$flight_det['flight_number'].'" />
                      </AirSegment>
                      <HostToken xmlns="http://www.travelport.com/schema/common_v50_0" Key="'.$flight_det['host_token_key'].'">'.$flight_det['host_token_value'].'</HostToken>
                    </SeatMapReq>
                    </s:Body></s:Envelope>';
                $seatRes[$se] = $this->process_request($seat_mapping_request,'','Seating Request');
                ...
                if (isset($seatRes_arrar['SOAP:Body']['SOAP:Fault'])) {
                    $seatResformated[$dn] = $seatRes_arrar['SOAP:Body']['SOAP:Fault']['faultstring'];
                } else {
                    $seatResformated[$dn] = $this->seatFormat($seatRes_arrar, $segmentCnt, $journeyCnt, $search_id);
    ...
    public function seatFormat($seat_res, $segmentCnt, $journeyCnt, $search_id) {
        ...
        if(isset($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:OptionalServices']['air:OptionalService'])){
            $optional_service = array();
            foreach ($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:OptionalServices']['air:OptionalService'] as $optional_key => $optional_value) {
                $optional_service[$optional_value['@attributes']['Key']] = $optional_value;
            }
        }
        if (isset($seat_res['SOAP:Envelope']['SOAP:Body']['air:SeatMapRsp']['air:Rows']['air:Row'])) {
            // echo 'herre';exit;
            ...
            for ($s = 0; $s < count($arrSeatData); $s++) {
                ...
                foreach ($row_details as $key => $value) {
                    ...
                    $opt_service = $optional_service[$value['OptionalServiceRef']];
                    $total_price = str_split($opt_service['@attributes']['TotalPrice'],3);
                    $seating_array = array();
                    $seating_array['seat_attributes'] = $value;
                    $seating_array['seat_optional_service'] = $opt_service;
                   if($value['seat_type'] != 'Asile'){
                        $new = array(
                            ...
                            'Price' => $total_price[1],
                            'SeatId' => base64_encode(serialize($seating_array)),
                        );
    ...
    function hold_ticket($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if ($book_service_response['status'] == SUCCESS_STATUS) {
            ...
            $ticket_request_params['BookingId'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
    ...
    function price_recheck($params){
        ...
        if(empty($meal_xml) == false || empty($baggage_xml) == false){
            ...
            /*debug($check_price_res);
            exit();*/
            // $check_price_res = $this->CI->custom_db->get_static_response (220180);
    ...
    private function run_book_service_request($params, $price_recheck_xml, $search_id, $ach_status) {
      ...
      if(isset($params["GST"])){
           ...
           // debug($MealsDetails);
            ...
            exit();*/
                ...
                if (isset($SeatDetails) && valid_array($SeatDetails)) {
                    ...
                    $k = 0;
                    foreach ($arrAirseg as $key => $value) {
                        /*if($key == $SeatDetails[$k]['Origin'].'_'.$SeatDetails[$k]['Destination']){*/
                            $SpecificSeatAssignment .= '<SpecificSeatAssignment xmlns="http://www.travelport.com/schema/air_v50_0" BookingTravelerRef="'.$pass_key.'" SegmentRef="'.$value.'" SeatId="'.$SeatDetails[$k]['SeatCode'].'"/>';
        ...
        $payment = '<FormOfPayment Type="AgencyPayment" xmlns="http://www.travelport.com/schema/common_v50_0">
                            <AgencyPayment AgencyBillingIdentifier="IGS5186" AgencyBillingPassword="Pace@786" />
                            </FormOfPayment>';
                }
            ...
            $payment = '<FormOfPayment xmlns="http://www.travelport.com/schema/common_v50_0" Key="' . $arrPriceKey . '" Type="Cash" />';
        }
        $payment = '<FormOfPayment Type="Credit" xmlns="http://www.travelport.com/schema/common_v50_0" Key="446408">
                    <CreditCard BankCountryCode="US" CVV="123" ExpDate="2025-09" Name="TEST TRAVELPORT" Number="5200000000000007" Type="MC">
                        <BillingAddress>
    ...
    private function save_flight_ticket_details($booking_params, $book_response, $ticket_response, $app_reference, $sequence_number, $search_id){
        ...
        foreach($airsegment_list as $seg_key => $seg_value){
            ...
            $update_itinerary_data['airline_pnr'] = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
    ...
    private function save_book_response_details($booking_params, $book_response, $app_reference, $sequence_number, $search_id, $provider_code) {
        ...
        $update_data['airline_pnr'] = $airline_pnr = $book_response['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
