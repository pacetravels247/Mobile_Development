...
class Travelport extends Common_Api_Flight {
    ...
    // ini_set('display_errors', 1);
        parent::__construct(META_AIRLINE_COURSE, TRAVELPORT_FLIGHT_BOOKING_SOURCE,$config_table_origin);
...
function format_fare_rule_response($fare_rule_response) {
    ...
    private function fare_rule_request($arrSegmentData) {
        ...
        if($arrSegmentData[0]['ProviderCode'] == '1G'){
            $strRequest = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                <soapenv:Header/>
                <soapenv:Body>
                    <ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v50_0" xmlns:ns2="http://www.travelport.com/schema/air_v50_0" FareRuleType="long" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" AuthorizedBy="' . $this->config ['UserName'] . '">
            ...
            $strRequest = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                <soapenv:Header/>
                <soapenv:Body>
                    <air:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v50_0" xmlns:air="http://www.travelport.com/schema/air_v50_0" FareRuleType="long" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" AuthorizedBy="' . $this->config ['UserName'] . '">
                      <BillingPointOfSaleInfo OriginApplication="uAPI" />';
    ...
    private function format_search_data_response($low_fare_search_res_data, $search_data) {
        ...
        foreach ($low_fare_search_res_data as $key => $low_fare_search_res) {
            ...
            if(!empty($low_fare_search_res['air:LowFareSearchRsp']['air:FlightDetailsList']['air:FlightDetails'])){
                ...
                if(isset($low_fare_search_res ['air:LowFareSearchRsp'] ['air:AirPricePointList']['air:AirPricePoint'])){
                    $air_pricing_solutions = $low_fare_search_res ['air:LowFareSearchRsp'] ['air:AirPricePointList']['air:AirPricePoint'];
                }
                else{
                ...
                $arr_flight_details = array();
                $air_flight_details = array();
                foreach ($air_flight_list['air:FlightDetails'] as $k => $v) {
                    $arr_flight_details[$v['@attributes']['Key']] = $v['@attributes']; 
                }
                if(valid_array($air_flight_list['air:FlightDetails'])){
                    foreach ($air_flight_list['air:FlightDetails'] as $fd_key => $fd_value) {
                        if(valid_array($fd_value['@attributes'])){
                            if (isset($fd_value['@attributes']['OriginTerminal'])) {
                                $air_flight_details[$fd_value['@attributes']['Key']]['OriginTerminal'] = $fd_value['@attributes']['OriginTerminal'];
                            }

                            if (isset($fd_value['@attributes']['DestinationTerminal'])) {
                                $air_flight_details[$fd_value['@attributes']['Key']]['DestinationTerminal'] = $fd_value['@attributes']['DestinationTerminal'];
                            }
                ...
                for ($b = 0; $b < count($air_fare_info_list); $b++) {
                    ...
                    $fare_info_key_arrays[$air_fare_info_list[$b]['@attributes']['Key']]['FareFamily'] = @$air_fare_info_list[$b]['@attributes']['FareFamily'];
                    $fare_info_key_arrays[$air_fare_info_list[$b]['@attributes']['Key']]['FareBasis'] = $air_fare_info_list[$b]['@attributes']['FareBasis'];
                    $baggage_value ='';
                    $baggage_unit = '';
                ...
                if((isset($air_host_token_list['common_v50_0:HostToken'][0]['@attributes'])) && valid_array($air_host_token_list)){
                    foreach($air_host_token_list['common_v50_0:HostToken'] as $host_key => $host_value){
                ...
                if($search_data['trip_type'] == 'return' && empty($search_data['is_domestic']) == false){
                    foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                        // debug($pricing_array);exit;
                        // $air_flight_details = array();
                        $this->format_air_pricing_solution_domestic($pricing_array, $fare_info_key_arrays, $air_segment_list, $search_data, $airport_list, $airline_list, $air_flight_details, $flight_list_data, $arrHostTokenData, $arrBaggageData);
                        $flight_data_list = $flight_list_data;
                    }
                }
                else{
                    if (isset($air_pricing_solutions_data) && valid_array($air_pricing_solutions_data)) {
                        foreach ($air_pricing_solutions_data as $ap_key => $pricing_array) {
                            ...
                            $flight_data_list [] = $this->format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData,$airport_list,$airline_list,$hand_baggage_list,$arr_flight_details);
      ...
      function format_air_pricing_solution_domestic($pricing_array, $fare_info_key_array, $air_segment_list, $search_data, &$airport_list, &$airline_list, $air_flight_details, &$flight_list_data, $arr_host_token, $arr_bag_data){
        // debug($arr_bag_data);exit;
        // $arr_key = array_key_last ($flight_list_data);
        $flight_arr = array_keys($flight_list_data);
        $arr_key = end($flight_arr);
        
        $AirPricingInfos = force_multple_data_format($pricing_array['air:AirPricingInfo']);
        
        $flight_list_arr = array();
        $segment_list = array();
        foreach($air_segment_list as $seg_key => $seg_info){
            $segment_list[$seg_info['@attributes']['Key']] = $seg_info;
        }

        if(empty($arr_key) == true){
            $arr_key = 0;
        }
        else{
            $arr_key = $arr_key+1;
        }
        if(empty($arr_key_return) == true){
            $arr_key_return = 0;
        }
        else{
            $arr_key_return = $arr_key_return+1;
        }
        $fare_info_key = '';
        $flight_list_total = $AirPricingInfos[0]['air:FlightOptionsList']['air:FlightOption'];
        
        $flight_list_total = force_multple_data_format($flight_list_total);
        foreach($flight_list_total as $flight_keyy => $flight_infos){
            // debug($flight_infos);exit;
            $flight_list = $flight_infos['air:Option'];
            $flight_list = force_multple_data_format($flight_list);
            foreach($flight_list as $flight_key => $flight_info){
            // debug($flight_list);exit;
            $air_booking_info = force_multple_data_format($flight_info['air:BookingInfo']);
            $fare_info_key = $air_booking_info[0]['@attributes']['FareInfoRef'];
            $details = array();
            foreach($air_booking_info as $b_key => $booking_info){
                // debug($fare_info_key_array);
                // debug($booking_info);exit;
                $segment_data = $segment_list[$booking_info['@attributes']['SegmentRef']];
                // debug($segment_data);exit;
                $flight_details_ref = force_multple_data_format($segment_data['air:FlightDetailsRef']);
                // debug($flight_details_ref);exit;
                $origin_terminal = '';
                $destination_terminal = '';
                if(isset($air_flight_details[$flight_details_ref[0]['@attributes']['Key']])){
                    $origin_terminal = @$air_flight_details[$flight_details_ref[0]['@attributes']['Key']]['OriginTerminal'];
                    $destination_terminal = @$air_flight_details[$flight_details_ref[0]['@attributes']['Key']]['DestinationTerminal'];
                }
               
                // debug($origin_terminal);exit;
                $is_leg = true;
                $cabin_class = $booking_info['@attributes']['BookingCode'];
                $booking_count = $booking_info['@attributes']['BookingCount'];
                $duration = $segment_data['@attributes']['FlightTime'];
                $trip_type = '';
                $arr_flight_details = array();
                $details[0][] = $this->flight_detail_format_domestic($segment_data, $arr_key, $trip_type, $is_leg, $airport_list, $airline_list, $arr_flight_details, $search_data, $cabin_class, $booking_count, $origin_terminal, $destination_terminal, $arr_bag_data[$booking_info['@attributes']['FareInfoRef']]);
                $is_leg = false;
                $connection = '';
                if(isset($flight_info ['air:Connection'])){
                    $connection_arr = force_multple_data_format(@$flight_info ['air:Connection']);
                ...
                //For Result Token
                // $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['TraceId']                    = @$trace_id;
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['group']                     = @$segment_data ['@attributes'] ['Group'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['carrier']                   = @$segment_data ['@attributes'] ['Carrier'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['flight_number']             = @$segment_data ['@attributes'] ['FlightNumber'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['origin']                    = @$segment_data ['@attributes'] ['Origin'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['destination']               = @$segment_data ['@attributes'] ['Destination'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['departure_time']            = @$segment_data ['@attributes'] ['DepartureTime'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['arrival_time']              = @$segment_data ['@attributes'] ['ArrivalTime'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['flight_time']               = @$segment_data ['@attributes'] ['FlightTime'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['distance']                  = @$segment_data ['@attributes'] ['Distance'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['e_ticketability']           = @$segment_data ['@attributes'] ['ETicketability'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['equipment']                 = @$segment_data ['@attributes'] ['Equipment'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['change_of_plane']           = @$segment_data ['@attributes'] ['ChangeOfPlane'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['participant_level']         = @$segment_data ['@attributes'] ['ParticipantLevel'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['link_availability']         = @$segment_data ['@attributes'] ['LinkAvailability'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['polled_availability_option'] = @$segment_data ['@attributes'] ['PolledAvailabilityOption'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['optional_services_indicator']= @$segment_data ['@attributes'] ['OptionalServicesIndicator'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['availability_source']       = @$segment_data ['@attributes'] ['AvailabilitySource'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['flight_seg_key']            = $segment_data['@attributes']['Key'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['provider_code']             = @$segment_data ['air:AirAvailInfo'] ['@attributes'] ['ProviderCode'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['flight_detail_key']         = $segment_data['@attributes']['Key'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['status']                    = @$segment_data ['@attributes'] ['Status'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['suppliercode']              = @$segment_data ['@attributes'] ['SupplierCode'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['apis_req_ref']              = @$segment_data ['@attributes'] ['APISRequirementsRef'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['booking_code'] = $booking_info ['@attributes'] ['BookingCode'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['booking_counts'] = $booking_info ['@attributes'] ['BookingCount'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['cabin_class'] = $booking_info ['@attributes'] ['CabinClass'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['fare_info_ref'] = $fare_info_ref = $booking_info ['@attributes'] ['FareInfoRef'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['host_token_key'] = $host_token_key = @$booking_info ['@attributes'] ['HostTokenRef'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['fare_basis'] = @$fare_info_key_array[$booking_info['@attributes']['FareInfoRef']]['FareBasis'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['fare_info_value'] = @$segment_data['@attributes']['FareInfoRef'];
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['ProviderCode'] = "ACH";
                $flight_list_on_out ['flight_list'] [0] ['flight_detail'] [$b_key] ['host_token_value'] = @$arr_host_token[$host_token_key]['HostTokenValue'];
            ...
            // $fare_type = '';
            // echo $fare_type;exit;
            $price = $this->format_itineray_price_details($pricing_array, $fare_type);
            
            
            $Refundable = 0;
            if(isset($AirPricingInfos[0]['@attributes'])){
                if(isset($AirPricingInfos[0]['@attributes']['Refundable'])){
                    $Refundable = $AirPricingInfos[0]['@attributes']['Refundable'];
                    if ($Refundable =='true') {
                        $Refundable = 1;
                    }
                }
            }
            $flight_list_data[$arr_key] ['FlightDetails'] ['Details'] = $details;
            $flight_list_data[$arr_key] ['Price'] = $price;
            $flight_list_data[$arr_key] ['Attr']['IsRefundable'] = $Refundable;
            $flight_list_data[$arr_key] ['Attr']['AirlineRemark'] = '';
            $flight_list_data[$arr_key] ['HoldTicket'] = true;
            $token_data [0] = $flight_list_on_out;
            $token_data [0]['booking_source'] = $this->booking_source;
            $flight_list_data[$arr_key] ['ResultToken'] = serialized_data($token_data);
            $arr_key++;
        }
    }
        
    }
    private function flight_detail_format_domestic($segment, $flight_key, $trip_type, $is_leg,&$airport_list,&$airline_list, $arr_flight_details = array(), $search_data, $cabin_class, $booking_count, $origin_terminal, $destination_terminal, $baggage_data) {
        // debug($baggage_data);exit;
        $FlightDetailsRef = @$segment['air:FlightDetailsRef']['@attributes']['Key'];
        ...
        $hand_baggage ='';
        $CheckinBaggage = @$baggage_data['Baggage'];
        $CabinBaggage = '';
        if(!empty($hand_baggage)){
            $CabinBaggage = $hand_baggage;
        }
        ...
        $flight_detail_arr ['CabinClass'] = $cabin_class;
        $flight_detail_arr ['Attr']['Baggage'] = $CheckinBaggage;
        $flight_detail_arr ['Attr']['CabinBaggage'] = $CabinBaggage;
        $flight_detail_arr ['Attr']['AvailableSeats'] = $booking_count;
 ...
 /**
     ...
     */
    private function format_itineray_price_details($itineray_price, $fare_type) {
       // debug($itineray_price);exit;
        $price = array();
        $passenger_breakup = array();
        $currency_code = substr($itineray_price['@attributes']['ApproximateBasePrice'], 0, 3);
        $base_fare = substr($itineray_price['@attributes']['ApproximateBasePrice'], 3);
        $tax = substr($itineray_price['@attributes']['ApproximateTaxes'], 3);
        $fees = 0;
        if(isset($itineray_price['@attributes']['ApproximateFees'])){
            $fees = substr($itineray_price['@attributes']['ApproximateFees'], 3);
        }
        $total_fare = substr($itineray_price['@attributes']['ApproximateTotalPrice'], 3);
        $air_prciing_info = force_multple_data_format($itineray_price['air:AirPricingInfo']);
        // debug($air_prciing_info);exit;
        foreach ($air_prciing_info as $k => $v) {
            // debug($v);exit;
            $pax_types = force_multple_data_format($v['air:PassengerType']);
            $pax_type = $pax_types[0]['@attributes']['Code'];
            // echo $pax_type;exit;
            
            $pax_base_fare = substr($v['@attributes']['ApproximateBasePrice'], 3);
            $pax_tax = 0;
            if(isset($v['@attributes']['Taxes'])){
                $pax_tax = substr($v['@attributes']['Taxes'], 3);
            }
            
            $feess = 0;
            if(isset($v['@attributes']['Fees'])){
                $feess = substr($v['@attributes']['Fees'], 3);
            }
            $pax_tax += $feess;
            ...
            $passenger_breakup[$pax_type]['PassengerCount'] = count($pax_types);
        }
     ...
     * array $pricing_array
     */
    function format_air_pricing_solution($pricing_array, $fare_info_key_array, $air_segment_list1, $arrBaggageData, $arrFareruleData, $search_data, $arrHostTokenData,&$airport_list,&$airline_list,&$hand_baggage_list,$arr_flight_details = array()) {
        $CI = & get_instance();
                        ...
                        if (isset($air_segment_list) && !empty($air_segment_list)) {
                           foreach ($air_segment_list as $as_key => $segment) {
                                ...
                                if ($flight_key == $segment_ref) {
                                    ...
                                    $flight_journey ['FlightDetails'] ['Details'][$j_key][] = $this->flight_detail_format($segment, $flight_key, $trip_type, $is_leg,$airport_list,$airline_list,$arr_flight_details);
...
private function flight_detail_format($segment, $flight_key, $trip_type, $is_leg,&$airport_list,&$airline_list,$arr_flight_details = array()) {
        ...
        $flight_detail_arr ['Origin'] = array(
            ...
            'time' => @$DepartureTime1[0], // Derive H i/h i a
            'FDTV' => strtotime(@$DepartureTime1[0]),
            'OriginTerminal' => @$arr_flight_details[$FlightDetailsRef]['OriginTerminal']
        );
        ...
        $flight_detail_arr ['Destination'] = array(
            ...
            'time' => @$ArrivalTime1[0], // Derive H i/h i a
            'FATV' => strtotime(@$ArrivalTime1[0]),
            'DestinationTerminal' => @$arr_flight_details[$FlightDetailsRef]['DestinationTerminal']
        );
 ...
 /*
     ...
     */
     function flight_low_fare_search_req($search_data) {
        ...
        if($search_data['cabin_class'] == 'premiumeconomy'){
            $search_data['cabin_class'] = 'PremiumEconomy';
        }
       ...
       // $airline_list = $this->get_airline_list($search_data['is_domestic']);
        ...
        $LegModifiers = '<AirLegModifiers><PermittedCabins>
                            <CabinClass Type="' . $cabin_class . '" xmlns="http://www.travelport.com/schema/common_v50_0"></CabinClass>
                        </PermittedCabins>
                        </AirLegModifiers>';
        ...
        $segment_info .= $this->search_segment($desti_code,$origin_code,$arrival_date,$LegModifiers);
            ...
            $segment_info = $this->search_segment($origin_code,$desti_code,$depart_date,$LegModifiers,$NearestAirport);
            $circle = $this->search_segment($desti_code,$origin_code,$arrival_date,$LegModifiers,$NearestAirport);
        ...
        if($this->config['flight_type']=='BOTH' or $this->config['flight_type']=='GDS'){
            /* GDS Request */
            $travelport_request[0][] = $this->flight_type_search_request($search_method,$segment_info,$adult,$child,$infant,$prefered_carrier_GDS, $search_data);
            if ($search_data['trip_type'] == 'return' && $search_data['is_domestic'] == 1){
                $travelport_request[1][]     = $this->flight_type_search_request($search_method,$circle,$adult,$child,$infant,$prefered_carrier_GDS, $search_data);
             }
        }
        if($this->config['flight_type']=='BOTH' or $this->config['flight_type']=='LCC'){
            /* ACH Request */
            $travelport_request[0][] = $this->flight_type_search_request($search_method,$segment_info,$adult,$child_ACH,$infant,$prefered_carrier_ACH, $search_data);
            if ($search_data['trip_type'] == 'return' && $search_data['is_domestic'] == 1){
                $travelport_request[1][] = $this->flight_type_search_request($search_method,$circle,$adult,$child_ACH,$infant,$prefered_carrier_ACH, $search_data);
             }
        }
    ...
    function prefered_airline($airline_list = array(),$flight_type){
        ...
        if(valid_array($airline_list)){
            ...
            foreach ($airline_list as $key => $flight_name) {
                $prefered_airline .= '<Carrier xmlns="http://www.travelport.com/schema/common_v50_0" Code="'.$flight_name.'"/>';
            }
            $prefered_airline .= '</PermittedCarriers>';
        }

        $AirSearchModifiers = '<AirSearchModifiers MaxSolutions="100"><PreferredProviders>
                     <Provider xmlns="http://www.travelport.com/schema/common_v50_0" Code="'.$flight_type.'" />
                     </PreferredProviders>
                     </AirSearchModifiers>';
    ...
    function search_pax_type($pax_conut=array(),$pax_type){
        ...
        if(isset($pax_conut)){
            for($i=0;$i<$pax_conut;$i++) {
                $pax_type_str .= '<SearchPassenger Code="'.$pax_type.'" xmlns="http://www.travelport.com/schema/common_v50_0" />';
            }
        }
        return $pax_type_str;
    }
    ...
    function search_segment($origin_code,$destination_code,$dep_date,$LegModifiers){
        $search_segment = '<SearchAirLeg>
                            <SearchOrigin>
                                <CityOrAirport Code="' . $origin_code . '" xmlns="http://www.travelport.com/schema/common_v50_0" />
                            </SearchOrigin>
                            <SearchDestination>
                                <CityOrAirport Code="' . $destination_code . '" xmlns="http://www.travelport.com/schema/common_v50_0" />
                            </SearchDestination>
    ...
    function flight_type_search_request($search_method,$segment_info,$adult,$child,$infant,$prefered_carrier, $search_data){
        ...
        $flight_type_search_request = '<?xml version="1.0" encoding="utf-8"?>
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                ...
                <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                    <' . $search_method . ' SolutionResult="'.$solution_result.'" AuthorizedBy="user" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v50_0">
    ...
    public function format_update_fare_quote_response($air_price_response, $price_response, $search_id, $flight_details_key, $price_request,$flight_search_data, $flight_search_return_data='',$air_price_response_return='', $price_return_response='') {
        error_reporting(E_ALL);
        $search_data = $this->search_data($search_id);
        ...
        if ($status = SUCCESS_STATUS) {
            ...
            $search_price = $flight_journey['Price']['TotalDisplayFare'];
            if(valid_array($flight_search_return_data)){
                $flight_journey['FlightDetails']['Details'][1] = $flight_search_return_data['flight_search_data']['FlightDetails']['Details'][0];
                $search_price += $flight_search_return_data['flight_search_data']['Price']['TotalDisplayFare'];
            }
            ...
            if (isset($air_price_response ['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                ...
                // AirPricingSolution
                $air_pricing_sol ['AirPricingSolution'] ['@attributes'] ['xmlns'] = "http://www.travelport.com/schema/air_v50_0";
                ...
                if (isset($air_price_result ['air:AirPricingInfo']) && valid_array($air_price_result ['air:AirPricingInfo'])) {
                    ...
                    $flight_journey ['Price'] ['TotalDisplayFare'] = $fare_quote_price = $conversion_amount['conversion_rate']*$display_price;
                    $flight_journey ['Price'] ['PriceBreakup'] ['Tax'] = $conversion_amount['conversion_rate']*$total_tax;
                    $flight_journey ['Price'] ['PriceBreakup'] ['BasicFare'] = $conversion_amount['conversion_rate']*$approx_base_price;
                    $flight_journey ['Price'] ['PriceBreakup']['Tax_Details'] = tax_breakup($tax_details);
                    if(valid_array($air_price_response_return)){
                        $flight_journey['Price'] = $this->merge_flight_price($flight_journey ['Price'], $air_price_response_return);
                        $fare_quote_price = $flight_journey['Price']['TotalDisplayFare'];
                    }
                    $check_com = true;
                ...
                $flight_journey['Attr']['IsPriceChanged'] = false;
                $diff_price = $search_price-$fare_quote_price;
                if($diff_price < -2 || $diff_price > 2){
                    $flight_journey['Attr']['IsPriceChanged'] = true;
                }
                ...
                $air_price_result ['common_v50_0:HostToken'] = force_multple_data_format($air_price_result ['common_v50_0:HostToken']);
                if (isset($air_price_result ['common_v50_0:HostToken']) && valid_array($air_price_result ['air:FareNote'])) {
                    foreach ($air_price_result ['common_v50_0:HostToken'] as $af_not_k => $air_host_token) {
            ...
            if(empty($price_return_response) == false){
                $AirPriceRes = $price_return_response;
                $AirPriceRes = str_replace('SOAP:', '', $AirPriceRes);
                $AirPriceRes = str_replace('air:', '', $AirPriceRes);
                $AirPriceRes = new SimpleXMLElement($AirPriceRes);

                $AirItinerary = $AirPriceRes->Body->AirPriceRsp->AirItinerary;
                $AirItinerary_xml1 = $AirItinerary->asXML();
                $AirItinerary_xml1 = str_replace('<AirItinerary>', '', $AirItinerary_xml1);
                $AirItinerary_xml1 = str_replace('</AirItinerary>', '', $AirItinerary_xml1);
                $AirItinerary_xml =  str_replace('</AirItinerary>', '', $AirItinerary_xml);
                $AirItinerary_xml .= $AirItinerary_xml1.'</AirItinerary>';
                $AirPricingSolution = $AirPriceRes->Body->AirPriceRsp->AirPriceResult->AirPricingSolution->AirPricingInfo;
                unset($AirPricingSolution->AirSegmentRef);
                $AirPricingSolution_xml1 = $AirPricingSolution->asXML();
                $AirPricingSolution_xml1 = str_replace('common_v50_0:', '', $AirPricingSolution_xml1);
               // echo $AirPricingSolution_xml1;exit;
                $doc = new DOMDocument();

                $doc->loadXML($AirPricingSolution_xml1);
                $xp = new DOMXPath($doc);
                foreach ($xp->query('//AirPricingInfo') as $key => $tn) {
                    $tn->setAttribute('AirPricingInfoGroup','2');
                }
                $AirPricingSolution_xml1 = $doc->saveXML();
                $AirPricingSolution_xml1 = str_replace('<?xml version="1.0"?>', '', $AirPricingSolution_xml1);
                $AirPricingSolution_xml = str_replace('common_v50_0:', '', $AirPricingSolution_xml);
                $doc = new DOMDocument();
                // echo $AirPricingSolution_xml;exit;
                $doc->loadXML($AirPricingSolution_xml);
                $xps = new DOMXPath($doc);
                foreach ($xps->query('//AirPricingInfo') as $key => $tn) {
                    $tn->setAttribute('AirPricingInfoGroup','1');
                }
               
                // $AirPricingSolution = $doc->getElementsByTagName("AirPricingSolution")->item(0);
               
                foreach ($xps->query('//AirPricingSolution') as $key => $tn) {
                    
                    $attributes = $tn->attributes;
                    // debug($attributes);exit;
                    
                    // $tn->removeAttribute($attributes->item($i)->name);
                    for($i=0; $i<$attributes->length; $i++){

                        $tn->removeAttribute($attributes->item($i)->name);
                    }
                    $tn->removeAttribute('TotalPrice');
                    $tn->removeAttribute('BasePrice');
                    $tn->removeAttribute('Taxes');
                    $tn->removeAttribute('ApproximateTotalPrice');
                    $tn->removeAttribute('ApproximateTaxes');
                    // debug($attributes->item(0)->name);exit;
                     $tn->setAttribute('Key','1');
                }
               
               
                $AirPricingSolution_xml = $doc->saveXML();
                // echo $AirPricingSolution_xml;exit;
                $AirPricingSolution_xml = str_replace('<?xml version="1.0"?>', '', $AirPricingSolution_xml);
 
                $AirPricingSolution_xml_org = str_replace('</AirPricingSolution>', '', $AirPricingSolution_xml);
               
                $AirPricingSolution_xml = $AirPricingSolution_xml_org.$AirPricingSolution_xml1.'</AirPricingSolution>';
                // $AirPricingSolution_xml1 = str_replace('<AirPricingSolution>', '', $AirPricingSolution_xml1);
            }
            ...
            if($provider_code == 'ACH'){
                ...
                foreach ($xp->query('//AirPriceReq') as $key => $tn) {
                    $tn->setAttribute('xmlns','http://www.travelport.com/schema/air_v50_0');
                    $tn->setAttribute('xmlns:common_v50_0','http://www.travelport.com/schema/common_v50_0');
                }
                ...
                foreach($booking_traveller_key as $b_key => $b_value){
                    $passenger_xml .= '<SearchPassenger BookingTravelerRef="' . $b_value['Key'] . '" Code="'.$b_value['Code'].'" xmlns="http://www.travelport.com/schema/common_v50_0" ></SearchPassenger>';
                }
                ...
                foreach ($optional_sevices as $opt_key => $opt_service) {
                    if($opt_service['@attributes']['Type'] == 'MealOrBeverage'){
                        ...
                        $opt_service_xml = str_replace('common_v50_0:', '', $opt_service_xml);
                        $opt_service_xml = str_replace('air:', '', $opt_service_xml);
                        $common_sevice_data = force_multple_data_format($opt_service['common_v50_0:ServiceData']);
                        $opt_service_segment = $common_sevice_data; 
                        if(count($common_sevice_data) > 1){
                            if(isset($opt_service['common_v50_0:ServiceInfo']['common_v50_0:Description'][0])){
                                $desc1 = $opt_service['common_v50_0:ServiceInfo']['common_v50_0:Description'][1];
                                if (strpos($desc1, 'Meal is available only on the first segment in a connecting flight') !== false) {
                                    unset($opt_service['common_v50_0:ServiceData'][1]);
                                    $opt_service_segment = $opt_service['common_v50_0:ServiceData']; 
                                }
                            }
                        }
                    ...
                    if($opt_service['@attributes']['Type'] == 'Baggage'){
                        ...
                        if(count($common_sevice_data) > 1){
                            if(isset($opt_service['common_v50_0:ServiceInfo']['common_v50_0:Description'][0])){
                                ...
                                if (strpos($desc1, 'Meal is available only on the first segment in a connecting flight') !== false) {
                                    unset($opt_service['common_v50_0:ServiceData'][1]);
                                    $opt_service_segment_baggage = $opt_service['common_v50_0:ServiceData']; 
                                }
                            }
                        }
    ...
    function merge_flight_price($onward_price_list, $return_price_response){
        $return_price_list = array();
        if (isset($return_price_response ['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
            $air_price_result_arr = $return_price_response ['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'] ['air:AirPricingSolution'];
            if (isset($air_price_result_arr) && valid_array($air_price_result_arr)) {
                $air_price_result = force_multple_data_format($air_price_result_arr);
                $air_price_result = $air_price_result [0];
            }
            if (isset($air_price_result ['air:AirPricingInfo']) && valid_array($air_price_result ['air:AirPricingInfo'])) {
                ...
                $air_price_result ['air:AirPricingInfo'] = force_multple_data_format($air_price_result ['air:AirPricingInfo']);
                $pass_key1 = 0;
                // debug($air_price_result ['air:AirPricingInfo']);exit;
                foreach ($air_price_result ['air:AirPricingInfo'] as $ap_key => $air_price_sol) {
                    $BasePrice = 0;
                    $Taxes = 0;
                    $Fees = 0;
                    ...
                    $return_price_list ['PassengerBreakup'][$pass_code]['PassengerCount'] = $passenger_count;
                    $return_price_list ['PassengerBreakup'][$pass_code]['BasePrice'] = $conversion_amount['conversion_rate']*$BasePrice;
                    $return_price_list ['PassengerBreakup'][$pass_code]['Tax'] = $conversion_amount['conversion_rate']*($Taxes+$Fees);
                    $return_price_list ['PassengerBreakup'][$pass_code]['TotalPrice'] = $conversion_amount['conversion_rate']*($BasePrice + $Taxes+$Fees);
                }
                $currency          = substr($air_price_result ['@attributes'] ['ApproximateTotalPrice'], 0, 3);
                $display_price     = substr(@$air_price_result ['@attributes'] ['ApproximateTotalPrice'], 3);
                $total_tax         = substr(@$air_price_result ['@attributes'] ['ApproximateTaxes'], 3)+substr(@$air_price_result ['@attributes'] ['ApproximateFees'], 3);
                $approx_base_price = substr($air_price_result ['@attributes'] ['ApproximateBasePrice'], 3);
                $return_price_list ['Currency'] = $currency;
                $return_price_list ['TotalDisplayFare'] = $conversion_amount['conversion_rate']*$display_price;
                $return_price_list ['PriceBreakup'] ['Tax'] = $conversion_amount['conversion_rate']*$total_tax;
                $return_price_list ['PriceBreakup'] ['BasicFare'] = $conversion_amount['conversion_rate']*$approx_base_price;
                $return_price_list ['PriceBreakup']['Tax_Details'] = tax_breakup($tax_details);
            }
        }
        // debug($onward_price_list);exit;
        foreach($onward_price_list['PassengerBreakup'] as $p_key => $p_value){
            $final_price_list['PassengerBreakup'][$p_key]['PassengerCount'] =  $onward_price_list['PassengerBreakup'][$p_key]['PassengerCount'];
            $final_price_list['PassengerBreakup'][$p_key]['BasePrice'] = $onward_price_list['PassengerBreakup'][$p_key]['BasePrice']+$return_price_list['PassengerBreakup'][$p_key]['BasePrice'];
            $final_price_list['PassengerBreakup'][$p_key]['Tax'] = $onward_price_list['PassengerBreakup'][$p_key]['Tax']+$return_price_list['PassengerBreakup'][$p_key]['Tax'];
            $final_price_list['PassengerBreakup'][$p_key]['TotalPrice'] = $onward_price_list['PassengerBreakup'][$p_key]['TotalPrice']+$return_price_list['PassengerBreakup'][$p_key]['TotalPrice'];
        }
        $final_price_list['Currency'] = $onward_price_list['Currency'];
        $final_price_list['TotalDisplayFare'] = $onward_price_list['TotalDisplayFare']+$return_price_list['TotalDisplayFare'];
        $final_price_list['PriceBreakup']['Tax'] = $onward_price_list['PriceBreakup']['Tax']+$return_price_list['PriceBreakup']['Tax'];
        $final_price_list['PriceBreakup']['BasicFare'] = $onward_price_list['PriceBreakup']['BasicFare']+$return_price_list['PriceBreakup']['BasicFare'];
        foreach ($onward_price_list['PriceBreakup']['Tax_Details'] as $key => $value) {
            $final_price_list['PriceBreakup']['Tax_Details'][$key] = $onward_price_list['PriceBreakup']['Tax_Details'][$key]+$return_price_list['PriceBreakup']['Tax_Details'][$key];
        }
        $final_price_list['PriceBreakup']['BasicFare'] = $onward_price_list['PriceBreakup']['BasicFare']+$return_price_list['PriceBreakup']['BasicFare'];
        
        return $final_price_list;
       // debug($final_price_list);exit;
    }
    /**
     * Update Fare Quote
     * @param unknown_type $request
     */
    public function get_update_fare_quote($request, $search_id, $return_request='') {
        // echo "test";exit;
        ...
        $response ['message'] = ''; // Message to be returned
        $response ['data'] = array(); // Data to be returned
        $update_fare_quote_request = $this->update_fare_quote_request($fare_quote_request, $return_request, $search_id);
        ...
        if ($update_fare_quote_request['status'] == SUCCESS_STATUS) {
            ...
            // $price_response = $this->CI->custom_db->get_static_response(219318);
            // debug($price_response);exit;
            $update_fare_quote_response = Converter::createArray($price_response);
            $price_return_response = '';
            if(empty($update_fare_quote_request['return_request']) == false){
                $price_return_response = $this->process_request($update_fare_quote_request['return_request'],'','flight_pricing_return(Travelport Flight)');
               
                // $price_return_response = $this->CI->custom_db->get_static_response(219319);
            }
            if(empty($update_fare_quote_request['return_request']) == false){
                $update_fare_quote_return_response = Converter::createArray($price_return_response);
                if (valid_array($update_fare_quote_response) == true && isset($update_fare_quote_response['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult']) && valid_array($update_fare_quote_return_response) == true && isset($update_fare_quote_return_response['SOAP:Envelope'] ['SOAP:Body'] ['air:AirPriceRsp'] ['air:AirPriceResult'])) {
                    $response ['status'] = SUCCESS_STATUS;
                    $response ['data']['FareQuoteDetails'] = $this->format_update_fare_quote_response($update_fare_quote_response, $price_response, $search_id, $flight_details_key, $update_fare_quote_request['request_opt'],$request, $return_request,  $update_fare_quote_return_response, $price_return_response);
    ...
    private function update_fare_quote_request($token, $return_request, $search_id) {
        // debug($return_request);exit;
        $air_segment = array();
        $air_return_segment = array();
        $CI = & get_instance();
      //  $search_id = '6045';
        $search_data = $this->search_data($search_id);
        ...
        if (isset($token ['flight_list']) && valid_array($token['flight_list'])) {
            $onward_carrier = $token ['flight_list'][0]['flight_detail'][0]['carrier'];
            $origin_value = '';
                ...
                if (isset($flights_list ['flight_detail']) && valid_array($flights_list ['flight_detail'])) {
                    ...
                    foreach ($flights_list ['flight_detail'] as $flight_key => $flight) {
                        ...
                        if (isset($flight['host_token_key']) && !empty($flight['host_token_key'])) {
                            $host_token .= '<HostToken xmlns="http://www.travelport.com/schema/common_v50_0" Key="'.$flight['host_token_key'].'">'.$flight['host_token_value'].'</HostToken>';
        ...
        $air_return_segment = array();
      //only for domestic roundtrip
        if(valid_array($return_request)){
            if (isset($return_request ['flight_list']) && valid_array($return_request['flight_list'])) {
                $origin_value = '';
                $return_carrier = $return_request ['flight_list'][0]['flight_detail'][0]['carrier'];
                foreach ($return_request['flight_list'] as $fl_ley => $flights_list) {
                    if (isset($flights_list ['flight_detail']) && valid_array($flights_list ['flight_detail'])) {
                        foreach ($flights_list ['flight_detail'] as $flight_key => $flight) {
                            $segment = '';
                            $group = $flight ['group'];
                            ...
                            if($onward_carrier == $return_carrier){
                                $air_segment [] = $segment;
                                if ($provider_code == 'ACH') {
                                    $AirSegmentPricingModifiers1 .= '<AirSegmentPricingModifiers FareBasisCode="'.$fare_basis.'" AirSegmentRef="'.$flight_seg_key.'"></AirSegmentPricingModifiers>';
                                }
                                ...
                                $air_return_segment [] = $segment;
                                if ($provider_code == 'ACH') {
                                    $AirSegmentPricingModifiers2 .= '<AirSegmentPricingModifiers FareBasisCode="'.$fare_basis.'" AirSegmentRef="'.$flight_seg_key.'"></AirSegmentPricingModifiers>';
                                }
                            }
                        }
                    }
        ...
        $PermittedCabins = '<PermittedCabins>
                            <CabinClass xmlns="http://www.travelport.com/schema/common_v50_0" Type="'.$cabin_class.'"/>
                            </PermittedCabins>';
        $all_price_modfier ='';
        $form_of_payment ='';
        if ($provider_code == '1G') {
            $all_price_modfier = '<AirPricingModifiers FaresIndicator="AllFares">'.$PermittedCabins.'</AirPricingModifiers>';
            $form_of_payment = '<FormOfPayment xmlns="http://www.travelport.com/schema/common_v50_0"  Type="Cash"></FormOfPayment>';
        }
       
        $search_passenger = '<SearchPassenger Key="0" Code="ADT" xmlns="http://www.travelport.com/schema/common_v50_0" ></SearchPassenger>';
        ...
        $air_price_req = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
            ...
            <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                <AirPriceReq TraceId="394d96c00971c4545315e49584609ff6" CheckOBFees="All" AuthorizedBy="user" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v50_0" xmlns:common_v50_0="http://www.travelport.com/schema/common_v50_0">
                    <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v50_0"></BillingPointOfSaleInfo>
                    <AirItinerary>';
        ...
        $air_price_req .=  $host_token .'
                      ...
                      ' . $childs . '
                      ' . $infants . '
                    <AirPricingCommand xmlns="http://www.travelport.com/schema/air_v50_0" CabinClass="'.$cabin_class.'">' . $AirSegmentPricingModifiers1.'</AirPricingCommand>
                   ' . $form_of_payment . ' 
                </AirPriceReq>
            </s:Body>
        </s:Envelope>';
          $air_price_req_return = '';
          if(valid_array($air_return_segment)){
            if($onward_carrier != $return_carrier){
                $air_price_req_return = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                    <s:Header>
                        <Action s:mustUnderstand="1" xmlns="http://schemas.microsoft.com/ws/2005/05/addressing/none">http://192.168.0.25/horiizons/index.php/flight/lost</Action>
                    </s:Header>
                    <s:Body xmlns:xsi="http://www.w3.org/2001/xmlschema-instance" xmlns:xsd="http://www.w3.org/2001/xmlschema">
                        <AirPriceReq TraceId="a1HbG5zv0MFAYM797aGAkuUjKa8J3cq4" CheckOBFees="All" AuthorizedBy="' . $this->config ['UserName'] . '" TargetBranch="' . $this->config ['Target'] . '" xmlns="http://www.travelport.com/schema/air_v50_0" xmlns:common_v50_0="http://www.travelport.com/schema/common_v50_0">
                            <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v50_0"></BillingPointOfSaleInfo>
                            <AirItinerary>';
                $air_price_req_opt = '<AirPriceReq TraceId="394d96c00971c4545315e49584609ff6" CheckOBFees="All" AuthorizedBy="' . $this->config ['UserName'] . '" TargetBranch="' . $this->config ['Target'] . '" >
                            <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v50_0"></BillingPointOfSaleInfo>
                            <AirItinerary>';
                foreach ($air_return_segment as $as_key => $segmnt) {
                    $air_price_req_return .= $segmnt;
                
                }
                $air_price_req_return .=  $host_token .'
                             </AirItinerary>
                              ' . $all_price_modfier . '
                              ' . $adults . '
                              ' . $childs . '
                              ' . $infants . '
                            <AirPricingCommand xmlns="http://www.travelport.com/schema/air_v50_0">' . $AirSegmentPricingModifiers2.'</AirPricingCommand>
                            ' . $form_of_payment . ' 
        ...
        $response ['return_request'] = $air_price_req_return;
        // debug($response);exit;
        return $response;
    }
        ...
        if(!empty($Age)){
         ...
         if ($pax_count != 0) {
            for ($i = 0; $i < $pax_count; $i ++) {
                $pax_xml .= '<SearchPassenger BookingTravelerRef="' . $paxId . '" Code="'.$pax_code.'" '.$Age.' xmlns="http://www.travelport.com/schema/common_v50_0" ></SearchPassenger>';
                $paxId ++;
            }
        }
        return $pax_xml;
    ...
    public function seat_request_old($request, $search_id, $itinerary_xml) {
        ...
        for ($se = 0; $se < count($seat_data_new); $se++) {
            ...
            $seat_xml = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
                            <soapenv:Header />
                            <soapenv:Body>
                                <air:SeatMapReq TargetBranch="' . $this->config ['Target'] . '" TraceId="394d96c00971c4545315e49584609ff6" ReturnSeatPricing="true" AuthorizedBy="uAPI" xmlns:com="http://www.travelport.com/schema/common_v50_0" xmlns:univ="http://www.travelport.com/schema/universal_v50_0" xmlns:air="http://www.travelport.com/schema/air_v50_0">
                                    <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v50_0" />
    ...
    public function seat_request($request) {
        ...
        <soapenv:Header />
        <soapenv:Body>
        <air:SeatMapReq TargetBranch="' . $this->config ['Target'] . '" TraceId="394d96c00971c4545315e49584609ff6" ReturnSeatPricing="false" AuthorizedBy="uAPI" xmlns:com="http://www.travelport.com/schema/common_v50_0" xmlns:univ="http://www.travelport.com/schema/universal_v50_0" xmlns:air="http://www.travelport.com/schema/air_v50_0">
        <BillingPointOfSaleInfo OriginApplication="UAPI" xmlns="http://www.travelport.com/schema/common_v50_0" />
        <air:AirSegment Key="' . $AirSegment_Key . '" Group="' . $Group . '" Carrier="' . $Carrier . '" FlightNumber="' . $FlightNumber . '" Origin="' . $Origin . '" Destination="' . $Destination . '" DepartureTime="' . $DepartureTime . '" ArrivalTime="' . $ArrivalTime . '" FlightTime="' . $FlightTime . '" Distance="' . $Distance . '" ETicketability="Yes" Equipment="' . $Equipment . '" ChangeOfPlane="' . $ChangeOfPlane . '" ClassOfService="' . $ClassOfService . '" ParticipantLevel="' . $ParticipantLevel . '" LinkAvailability="' . $LinkAvailability . '" PolledAvailabilityOption="' . $PolledAvailabilityOption . '" OptionalServicesIndicator="' . $OptionalServicesIndicator . '" AvailabilitySource="' . $AvailabilitySource . '" AvailabilityDisplayType="' . $AvailabilityDisplayType . '" ProviderCode="' . $provider . '">
        <air:AirAvailInfo ProviderCode="' . $provider . '" />
        </air:AirSegment>
    ...
    public function seat_request_oldsss($request) {
      // debug($request);
        $seat_xml = '';
        $seat_response1 = array();
        $provider = '1G';
    ...
    private function format_send_change_request($params) {
        ...
        $CancelBookingReq = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v50_0" xmlns:univ="http://www.travelport.com/schema/universal_v50_0">
                            <soapenv:Header/>
                            <soapenv:Body>
                            <univ:UniversalRecordCancelReq AuthorizedBy="user" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" UniversalRecordLocatorCode="' . $BookingId . '" Version="1">
     ...
     */
    function process_request($request, $url = '', $remarks, $search_id='') {
        if (!empty($url)) {
            $url = $url;
        } else {
        ...
        //Previously passing serialized_data($request);
        if($remarks != 'flight_booking(Travelport Flight)'){
    ...
    function hold_ticket($booking_params, $app_reference, $sequence_number, $search_id) {
        ...
        if ($book_service_response['status'] == SUCCESS_STATUS) {
            ...
            $ticket_request_params['PNR'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
            $ticket_request_params['BookingId'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
            $ticket_request_params['TraceId'] = $book_response['universal:UniversalRecordRetrieveRsp']['@attributes']['TraceId'];
    ...
    function process_booking($booking_params, $app_reference, $sequence_number, $search_id) {
       ...
       // $provider_code = 'ACH';
        ...
        $locator_code = $book_service_response['data']['book_response']['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];

       // $url = 'https://apac.universal-api.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
        /*$url = 'https://americas.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
        $pnr_retrieve = '<?xml version="1.0" encoding="UTF-8"?>
        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v39_0" xmlns:univ="http://www.travelport.com/schema/universal_v39_0">
           <soapenv:Header />
           <soapenv:Body>
              <UniversalRecordRetrieveReq xmlns="http://www.travelport.com/schema/universal_v39_0" TraceId="394d96c00971c4545315e49584609ff6" AuthorizedBy="user" TargetBranch="'.$this->config ['Target'].'">
              <BillingPointOfSaleInfo xmlns="http://www.travelport.com/schema/common_v39_0" OriginApplication="uAPI" />
              <UniversalRecordLocatorCode>'.$locator_code.'</UniversalRecordLocatorCode>
            </UniversalRecordRetrieveReq>
           </soapenv:Body>
        </soapenv:Envelope>';
        debug($pnr_retrieve);

        $book_retrieve_response = $this->process_request($pnr_retrieve,$url,'PNR Retrieve(Travelport Flight)');
        debug($book_retrieve_response);
        exit();
        $book_retrieve_response = Converter::createArray($book_retrieve_response);*/
        /*debug($book_retrieve_response);
        exit();*/
        $ticketing_status = '';
        ...
        if ($book_service_response['status'] == SUCCESS_STATUS) {
            ...
            if ($ticketing_status == "Tikcet") {
                $ticket_request_params = array();
                $ticket_request_params['PNR'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
                if(empty($ticket_request_params['PNR'])){
                    $ticket_request_params['PNR'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['universal:ProviderReservationInfo']['@attributes']['LocatorCode'];
                }
                $ticket_request_params['AirReservationLocatoreCode'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['@attributes']['LocatorCode'];
        ...
        //debug($ticket_response); exit();
        if (valid_array($ticket_response) == true || valid_array($book_response) == true) {
            if (valid_array($ticket_response) == true) {
            	if(!isset($book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'])){
                	if(empty($book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'])){
                		$universal_locator_code = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
                		
                		$universal_record_retrieve_req = $this->universal_record_retrieve($universal_locator_code);
		                $url_universal  = 'https://americas.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
		                            
		                $book_service_response = $this->process_request($universal_record_retrieve_req ['request'],$url_universal,'universal_record_retrieve(Travelport Flight)');
		                // $book_service_response = $this->CI->custom_db->get_static_response (217904);
		                     // debug($book_service_response);exit;
		                $book_service_response = Converter::createArray($book_service_response);

		                $book_response = $book_service_response['SOAP:Envelope']['SOAP:Body'];
    ...
    private function run_book_service($booking_params, $app_reference, $sequence_number='', $pricerec_check_xml='', $search_id='', $ach_status='') {
        ...
        // debug($book_service_request);exit;
        if ($book_service_request['status'] == SUCCESS_STATUS) {
          
            $book_service_response = $this->process_request($book_service_request ['request'],'','flight_booking(Travelport Flight)', $search_id);
            
            // $book_service_response = $this->CI->custom_db->get_static_response (219150);//Static Data//159
            // debug($book_service_response);exit;
            $book_service_response = Converter::createArray($book_service_response);
           // debug($book_service_response);exit;
            if (valid_array($book_service_response) == true && isset($book_service_response['SOAP:Envelope']['SOAP:Body']['universal:AirCreateReservationRsp']['universal:UniversalRecord']['air:AirReservation']['@attributes']['LocatorCode']) == true) {
                $response ['status'] = SUCCESS_STATUS;
                $universal_locator_code = $book_service_response['SOAP:Envelope']['SOAP:Body']['universal:AirCreateReservationRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
                
                $universal_record_retrieve_req = $this->universal_record_retrieve($universal_locator_code);
                $url_universal  = 'https://americas.universal-api.pp.travelport.com/B2BGateway/connect/uAPI/UniversalRecordService';
                            
                $book_service_response = $this->process_request($universal_record_retrieve_req ['request'],$url_universal,'universal_record_retrieve(Travelport Flight)');
                // $book_service_response = $this->CI->custom_db->get_static_response (219392);
                     // debug($book_service_response);exit;
                $book_service_response = Converter::createArray($book_service_response);
                ...
                $error_message = '';
                if (isset($book_service_response['SOAP:Envelope']['SOAP:Body']['SOAP:Fault']['faultstring'])) {
                    $error_message = $book_service_response['SOAP:Envelope']['SOAP:Body']['SOAP:Fault']['faultstring'];
                }
                if (empty($error_message) == true) {
     ...
     * @param unknown_type $request
     */
    private function universal_record_retrieve($universal_locator_code) {
        $universal_record_request = '<?xml version="1.0" encoding="UTF-8"?>
                        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v50_0" xmlns:univ="http://www.travelport.com/schema/universal_v50_0">
                           <soapenv:Header />
                           <soapenv:Body>
                              <UniversalRecordRetrieveReq xmlns="http://www.travelport.com/schema/universal_v50_0" TraceId="trace" AuthorizedBy="user" TargetBranch="'. $this->config ['Target'] .'">
                              <BillingPointOfSaleInfo xmlns="http://www.travelport.com/schema/common_v50_0" OriginApplication="uAPI" />
                              <UniversalRecordLocatorCode>'.$universal_locator_code.'</UniversalRecordLocatorCode>
                            </UniversalRecordRetrieveReq>
                           </soapenv:Body>
                        </soapenv:Envelope>';
        $request ['request'] = $universal_record_request;
        // debug($request);exit;
        $request ['status'] = SUCCESS_STATUS;
        return $request;
    }
    ...
    function price_recheck($params){
        ...
        if(empty($meal_xml) == false || empty($baggage_xml) == false){
            ...
            foreach ($xp->query('//ServiceData') as $key => $tn) {
                $tn->setAttribute('xmlns','http://www.travelport.com/schema/common_v50_0');
            }
            ...
            if(!isset($check_price_res['SOAP:Envelope']['SOAP:Body']['SOAP:Fault'])){
                ...
                $air_price_xml = str_replace('common_v50_0:', '', $air_price_xml);
                ...
                $air_itinerary_xml = str_replace('common_v50_0:', '', $air_itinerary_xml);
                ...
                foreach($opt_services_list as $opt_service_data){
                    if($opt_service_data['@attributes']['ServiceStatus'] == 'Priced' && ($opt_service_data['@attributes']['Type'] == 'MealOrBeverage' || $opt_service_data['@attributes']['Type'] == 'Baggage')){
                        ...
                        $opt_service_list_xml = str_replace('common_v50_0:', '', $opt_service_list_xml);
    ...
    private function run_book_service_request($params, $price_recheck_xml, $search_id, $ach_status) {
        ...
        $AirItinerary_xml = str_replace("common_v50_0:", '', $AirItinerary_xml);
        $AirPricingSolution_xml = $AirPricingSolution_xml1 = str_replace('common_v50_0:', '', $AirPricingSolution_xml);
        // echo $AirPricingSolution_xml;exit;
        $doc = new DOMDocument();
        $doc->loadXML($AirPricingSolution_xml);
        ...
        $HostTokens = $doc->getElementsByTagName("HostToken");
        while ($HostTokens->length > 0) {
            $node = $HostTokens->item(0);
            $this->remove_node($node);
        }
        if(valid_array($price_recheck_xml)){
       ...
       // debug($AirPricingInfo);exit;
        ...
        foreach ($params['Passengers'] as $passenger) {
            ...
            if($provider == '1G'){
                ...
                $MealsDetails = $this->meal_request_details($MealsDetails);
                //Seat Start
                 if (isset($passenger['SeatId']) == true && valid_array($passenger['SeatId']) == true) {
                    $SeatDetails = $passenger['SeatId'];
                } else {
                    $SeatDetails = array();
                }
                $SeatDetails = $this->seat_request_details($SeatDetails);
            //debug($SeatDetails);die();
            //Seat End
            }
            ...
            $pax_data_xml .= '<BookingTraveler Key="' . $paxId . '" TravelerType="'.$type.'" DOB="' . $passenger['DateOfBirth'] . '" Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v50_0">
        ...
        if(isset($params["GST"])){
            $gst_det = $params["GST"];
            // $pax_data_xml .= '<SSR Type="GSTA" Carrier="AI" Status="NN" FreeText="IND/'.$gst_det["GstAddress"].'/'.$gst_det["GstState"].'"></SSR>
            // <SSR Type="GSTN" Carrier="AI" Status="NN" FreeText="IND/'.$gst_det["Gstnumber"].'/'.$gst_det["GstComapnyName"].'"></SSR>
            // <SSR Type="GSTP" Carrier="AI" Status="NN" FreeText="IND/'.$gst_det["GstPhone"].'/'.$gst_det["GstPhone"].'"></SSR>';
            $pax_data_xml .= '<SSR Key="1" Type="GSTA" Carrier="'.$air_line_pcode.'" Status="NN" FreeText="IND/SABNIS COMPLEX/OPP LINGRAJ COLLEGE ROAD/BELGAUM/590001"/>';
            $pax_data_xml .= '<SSR Key="2" Type="GSTN" Carrier="'.$air_line_pcode.'" Status="NN" FreeText="IND/29AAICP1898A1Z7/PaceTravels247"/>';
            $pax_data_xml .= '<SSR Key="3" Type="GSTP" Carrier="'.$air_line_pcode.'" Status="NN" FreeText="IND/08314207383/08314206383"/>';
            $pax_data_xml .= '<SSR Key="4" Type="GSTE" Carrier="'.$air_line_pcode.'" Status="NN" FreeText="IND/cmd//pacetravels.in"/>';
        }
            ...
            //if($provider == '1G'){
                if (isset($MealsDetails) && valid_array($MealsDetails)) {
                    ...
                    foreach ($arrAirseg as $AirKey => $AirVal) {
                        if($MealsDetails[$k]['Code'] != ''){
                        $pax_data_xml .= '<SSR Carrier="' . $air_line_pcode . '" Status="HK" SegmentRef="' . $AirVal . '" Type="' . $MealsDetails[$k]['Code'] . '"></SSR>';
                        $k++;
                        }
                    }
                }
            ...
            //Seat Start
            //debug($SeatDetails);die();
                if (isset($SeatDetails) && valid_array($SeatDetails)) {
                    $k = 0;
                    foreach ($arrAirsegSeat as $key => $value) {
                        if($key == $SeatDetails[$k]['Origin'].'_'.$SeatDetails[$k]['Destination']){
                            $SpecificSeatAssignment .= '<SpecificSeatAssignment xmlns="http://www.travelport.com/schema/air_v50_0" BookingTravelerRef="0" SegmentRef="'.$value.'" SeatId="'.$SeatDetails[$k]['Code'].'"/>';
        ...
        if ($provider == "ACH") {
           if($ach_status == 'hold'){
                ...
                if($carrier == '6E'){
                    $payment = '<FormOfPayment Type="Credit" xmlns="http://www.travelport.com/schema/common_v50_0" Key="446408">
                    ...
                    $payment = '<FormOfPayment Type="AgencyPayment" xmlns="http://www.travelport.com/schema/common_v50_0">
            ...
            $payment = '<AgencyContactInfo xmlns="http://www.travelport.com/schema/common_v50_0">
        <PhoneNumber Type="Agency" Number="8050834511" Text="Galileo Trng ref support"/>
</AgencyContactInfo>
 <CustomerID xmlns="http://www.travelport.com/schema/common_v50_0"/>
                        <FormOfPayment xmlns="http://www.travelport.com/schema/common_v50_0" Key="' . $arrPriceKey . '" Type="Cash" />';
        }
        $payment = '<FormOfPayment Type="Credit" xmlns="http://www.travelport.com/schema/common_v50_0" Key="446408">
                    <CreditCard BankCountryCode="US" CVV="123" ExpDate="2025-09" Name="TEST TRAVELPORT" Number="5200000000000007" Type="MC">
                        <BillingAddress>
        ...
        $ActionStatus = '<ActionStatus ProviderCode="' . $provider . '" TicketDate="T*" Type="ACTIVE" QueueCategory="01" xmlns="http://www.travelport.com/schema/common_v50_0" ></ActionStatus>';

        $AirCreateReservationReq = '<?xml version="1.0" encoding="utf-8"?>
        <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header/>
                        <s:Body>
                            <univ:AirCreateReservationReq xmlns:air="http://www.travelport.com/schema/air_v50_0"  xmlns:common_v50_0="http://www.travelport.com/schema/common_v50_0" xmlns:univ="http://www.travelport.com/schema/universal_v50_0" AuthorizedBy="user" TraceId="394d96c00971c4545315e49584609ff6" TargetBranch="' . $this->config ['Target'] . '" RetainReservation="Both">
                            <BillingPointOfSaleInfo  xmlns="http://www.travelport.com/schema/common_v50_0" OriginApplication="UAPI" ></BillingPointOfSaleInfo>
                            ' ;
        ...
        $AirCreateReservationReq = str_replace("HostToken Key", "HostToken xmlns='http://www.travelport.com/schema/common_v50_0' Key", $AirCreateReservationReq);
        $AirCreateReservationReq = str_replace("<AirPricingSolution", "<AirPricingSolution xmlns='http://www.travelport.com/schema/air_v50_0'", $AirCreateReservationReq);
    ...
    function BookingTraveler_Pax_XML($passenger,$air_line_pcode,$address='',$MealsDetails,$arrAirseg,$gender,$email){
        ...
        $Passenger_Firstnamep = $passenger['FirstName'];
        $Passenger_Lastnamep = $passenger['LastName'];
        $adults .= '<BookingTraveler Key="' . $paxId . '" TravelerType="ADT" DOB="' . $passenger['DateOfBirth'] . '" Gender="' . $gender . '" xmlns="http://www.travelport.com/schema/common_v50_0">
    ...
    private function run_non_lcc_ticket_service($booking_params, $app_reference, $sequence_number) {
        ...
        if ($non_lcc_ticket_service_request['status'] == SUCCESS_STATUS) {
            ...
            // $non_lcc_ticket_service_response = $this->CI->custom_db->get_static_response (217967);
            ...
            if ($this->validate_ticket_response($non_lcc_ticket_service_response) == true) {
                ...
                $error_message = '';
                if (isset($non_lcc_ticket_service_response['SOAP:Envelope'] ['SOAP:Body'] ['SOAP:Fault']['faultstring'])) {
                    $error_message = $non_lcc_ticket_service_response['SOAP:Envelope'] ['SOAP:Body'] ['SOAP:Fault']['faultstring'];
                }
                if (empty($error_message) == true) {
    ...
    private function run_non_lcc_ticket_service_request($params) {
        ...
        $ticket_request ='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v50_0" 
            xmlns:univ="http://www.travelport.com/schema/universal_v50_0">
               <soapenv:Header/>
               <soapenv:Body>
                  <air:AirTicketingReq BulkTicket="false" ReturnInfoOnFail="true" TargetBranch="' . $this->config ['Target'] . '" xmlns:air="http://www.travelport.com/schema/air_v50_0">
                     <com:BillingPointOfSaleInfo OriginApplication="UAPI"/>
    ...
    private function save_flight_ticket_details($booking_params, $book_response, $ticket_response, $app_reference, $sequence_number, $search_id){
        ...
        $book_id = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
        ...
        //update Airline PNR
        $airsegment = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['air:AirSegment'];
        ...
        foreach($airsegment_list as $seg_key => $seg_value){
            ...
            // debug($book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']);exit;
            //itinerary updated data
            $update_itinerary_data = array();
            $update_itinerary_data['airline_pnr'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator']['@attributes']['SupplierLocatorCode'];
        ...
        if(isset($ticket_response['SOAP:Envelope']['SOAP:Body']['air:AirTicketingRsp']['air:ETR']['air:Ticket']['@attributes']['TicketNumber'])){
            ...
            $ticket_number[0]['ticket_id'] = $ticket_response['SOAP:Envelope']['SOAP:Body']['air:AirTicketingRsp']['air:ETR']['air:Ticket']['@attributes']['TicketNumber'];
        }
        else{
    ...
    private function save_book_response_details($booking_params, $book_response, $app_reference, $sequence_number, $search_id, $provider_code) {
        ...
        $update_data['book_id'] = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['@attributes']['LocatorCode'];
        $air_reservation_data = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['air:AirReservation']['common_v50_0:SupplierLocator'];
        $air_reservation_data = force_multple_data_format($air_reservation_data);
        $update_data['airline_pnr'] = $air_reservation_data[0]['@attributes']['SupplierLocatorCode'];
        // debug($update_data);exit;
        foreach($air_reservation_data as $air_key => $reservation){
            $update_pnr_data['airline_pnr'] = $reservation['@attributes']['SupplierLocatorCode'];
            $update_pnr_condition['airline_code'] = $reservation['@attributes']['SupplierCode'];
           
            $this->CI->custom_db->update_record('flight_booking_itinerary_details', $update_pnr_data, $update_pnr_condition);
        }
        $gds_pnr_data = $book_response['universal:UniversalRecordRetrieveRsp']['universal:UniversalRecord']['universal:ProviderReservationInfo'];
