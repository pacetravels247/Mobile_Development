...
class Multi_Curl
{
    ...
    public function execute_multi_curl($curl_params)
	{
		...
		if(valid_array($booking_sources) == true && valid_array($requests) == true && 
            ...
            //debug($booking_sources);die();
            $current_response =  '';
			foreach ($booking_sources as $k => $v){
                if($v==TRAVELPORT_FLIGHT_BOOKING_SOURCE || $v==SABARE_FLIGHT_BOOKING_SOURCE){
                ...
                //$this->CI->api_model->update_api_response("Request time out. Please try again!", $request_insert_id[$k],$curl_details['total_time']);
			}
		}
